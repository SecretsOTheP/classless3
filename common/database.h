...
public:
	...
	bool	MoveCharacterToZone(uint32 iCharID, const char* iZonename, float x = -1.f, float y = -1.f, float z = -1.f);
	...
	std::map<uint32, MercCharacter_Struct> GetCharactersOnAccount(uint32 AccID);
	...
	bool AddClientToInstance(uint32 instance_id, uint32 char_id);
	uint32 CharacterInInstanceGroup(uint32 group_id);
	bool CheckInstanceExists(uint32 instance_id);
	bool CheckInstanceExpired(uint32 instance_id);
	bool CreateInstance(uint32 instance_id, uint32 zone_id, uint32 version, uint32 duration, uint32 group_id, int64 scale_cap = -1);
	bool GetUnusedInstanceID(uint32 &instance_id);
	bool GlobalInstance(uint32 instance_id);
	bool RemoveClientFromInstance(uint32 instance_id, uint32 char_id);
	bool RemoveClientsFromInstance(uint32 instance_id);
	bool VerifyInstanceAlive(uint32 instance_id, uint32 char_id);
	bool VerifyZoneInstance(uint32 zone_id, uint32 instance_id);

	uint16 GetInstanceID(const char* zone, uint32 charid, int16 version);
	uint16 GetInstanceID(uint32 zone, uint32 charid, int16 version);
	uint16 GetInstanceVersion(uint32 instance_id);
	uint32 GetInstanceGroupID(uint32 instance_id);
	int64 GetInstanceScaleCap(uint32 instance_id);
	uint32 GetTimeRemainingInstance(uint32 instance_id, bool &is_perma);
	uint32 VersionFromInstanceID(uint32 instance_id);
	uint32 ZoneIDFromInstanceID(uint32 instance_id);

	void AssignGroupToInstance(uint32 gid, uint32 instance_id);
	void AssignRaidToInstance(uint32 rid, uint32 instance_id);
	void BuryCorpsesInInstance(uint32 instance_id);
	void DeleteInstance(uint32 instance_id);
	...
	void GetCharactersInInstance(uint32 instance_id, std::list<uint32> &charid_list);
	void PurgeExpiredInstances();
	void SetInstanceDuration(uint32 instance_id, uint32 new_duration);
	...
	bool    SaveHackerProcesses(std::string processName, uint32 accountId, std::string accountName, std::string ipAddr);
	...
	std::list<uint64>	GetZoneSize(const char* zonename);
