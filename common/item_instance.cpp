...
EQ::ItemInstance::ItemInstance(SharedDatabase *db, uint32 item_id, int16 charges) {
	m_use_type = ItemInstNormal;
	ItemData dbitem = db->GetItem(item_id);
	if(dbitem.ID) {
		m_item = new ItemData(dbitem);
	}
	else {
		m_item = nullptr;
	}
...
bool EQ::ItemInstance::IsEquipable(uint16 race, uint16 class_, bool allowclassless) const
{
	if (!m_item || (m_item->Slots == 0))
		return false;

	if(class_ == 17 && allowclassless)
		return true;
