...
{
	// Delete cursor items
	std::string query = StringFormat("DELETE FROM inventory WHERE charid = %i AND slotid = %i OR (slotid >= %i AND slotid <= %i)",
...
bool SharedDatabase::GetSharedBank(uint32 id, EQ::InventoryProfile *inv, bool is_charid)
{
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		if (!item.ID) {
			LogError("Warning: [{}] [{}] has an invalid item_id [{}] in inventory slot [{}]", ((is_charid == true) ? "charid" : "acctid"), id, item_id, slot_id);
			continue;
		}
...
EQ::ItemData SharedDatabase::GetItem(uint32 id)
{
	EQ::ItemData blank;
	memset(&blank, 0, sizeof(EQ::ItemData));

	if (id == 0 || id==(uint32)-1) {
		return blank;
	}

	if (!items_hash || id > items_hash->max_key() && id < 10000000) {
		return blank;
	}

	if (items_hash->exists(id) && items_hash->at(id).ID == id) {
		return (items_hash->at(id));
	}
	else if (id >= 10000000)
	{
		EQ::ItemData itemPtr;
		itemPtr = GetItem(199000);
		int itemId = id;

		if (itemPtr.ID == 0)
			return blank;

		EQ::ItemData item = itemPtr;
...
void SharedDatabase::ClearTempItems() {

	for (auto itemPtr : hackitems)
	{
		//free the memory back into the great unknown
		safe_delete(itemPtr.second);
	}
...
void SharedDatabase::LoadLootDrops(void *data, uint32 size) {
    ...
    for (auto row = results.begin(); row != results.end(); ++row) {
        uint32 id = static_cast<uint32>(atoul(row[0]));
		EQ::ItemData item_data = GetItem(static_cast<uint32>(atoul(row[1])));
		
		if(item_data.ItemType == 20)
		{
			continue;
		}
