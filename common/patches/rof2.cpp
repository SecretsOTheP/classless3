...
namespace RoF2
{
	...
	void SerializeItem(EQ::OutBuffer& ob, const EQ::ItemInstance *inst, int16 slot_id, uint8 depth, ItemPacketType packet_type, uint8 classless = 0);
	...
	ENCODE(OP_CharInventory)
	{
		...
		*p = nullptr;
		...
		for (int index = 0; index < item_count; ++index, ++eq) {
			SerializeItem(ob, (const EQ::ItemInstance*)eq->inst, eq->slot_id, 0, ItemPacketCharInventory, eq->classless);
...
#undef PutFieldN
	...
	ENCODE(OP_ItemPacket)
	{
		...
		*p = nullptr;
		...
		SerializeItem(ob, (const EQ::ItemInstance*)int_struct->inst, int_struct->slot_id, 0, old_item_pkt->PacketType, int_struct->classless);
		if (ob.tellp() == last_pos) {
	...
	ENCODE(OP_PlayerProfile)
	{
		...
		*p = nullptr;
		...
		outapp->WriteUInt8(emu->class_ >= 17 ? 5 : emu->class_);	// Class
		...
		uint32 char_deity = emu->class_ >= 17 ? 396 : emu->deity;

		outapp->WriteUInt32(char_deity);
	...
	ENCODE(OP_RespondAA)
	{
		...
		eq->aa_spent = emu->aapoint_assigned;
	...
	ENCODE(OP_SendCharInfo)
	{
		...
		for (int counter = 0; counter < character_count; ++counter) {
			...
			eq_cse->Class = emu_cse->Class >= 17 ? 1 : emu_cse->Class;
...
// file scope helper methods
	...
	void SerializeItem(EQ::OutBuffer& ob, const EQ::ItemInstance *inst, int16 slot_id_in, uint8 depth, ItemPacketType packet_type, uint8 classless)
	{
		...
		ibs.Deity = classless ? 0 : item->Deity;
		...
		if (SubSlotNumber != EQ::invbag::SLOT_INVALID) {
			for (uint32 index = EQ::invbag::SLOT_BEGIN; index <= EQ::invbag::SLOT_END; ++index) {
				...
				SerializeItem(ob, sub, SubSlotNumber, (depth + 1), packet_type, classless);
				++subitem_count;
			}
