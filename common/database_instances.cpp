 ...
 * @return
 */
bool Database::AddClientToInstance(uint32 instance_id, uint32 char_id)
{
...
uint32 Database::CharacterInInstanceGroup(uint32 group_id)
{

	std::string query = StringFormat("SELECT id FROM instance_list where group_id=%u", group_id);
	auto results = QueryDatabase(query);
...
bool Database::CheckInstanceExists(uint32 instance_id) {
	std::string query = StringFormat(
		"SELECT "
		"`id`  "
		"FROM  "
...
bool Database::CheckInstanceExpired(uint32 instance_id)
{
...
bool Database::CreateInstance(uint32 instance_id, uint32 zone_id, uint32 version, uint32 duration, uint32 group_id, int64 scale_cap)
{
	std::string query = StringFormat(
		"INSERT INTO instance_list (id, zone, version, start_time, duration, group_id, scale_cap)"
		" values (%u, %u, %u, UNIX_TIMESTAMP(), %u, %u, %llu)",
		instance_id,
		zone_id,
		version,
		duration,
...
bool Database::GetUnusedInstanceID(uint32& instance_id)
{
...
bool Database::GlobalInstance(uint32 instance_id)
{
	std::string query = StringFormat(
		"SELECT "
		"is_global "
...
bool Database::RemoveClientFromInstance(uint32 instance_id, uint32 char_id)
{
...
bool Database::RemoveClientsFromInstance(uint32 instance_id)
{
...
bool Database::VerifyInstanceAlive(uint32 instance_id, uint32 char_id)
{
	//we are not saved to this instance so set our instance to 0
	if (!GlobalInstance(instance_id) && instance_id != CharacterInInstanceGroup(char_id))
		return false;
...
bool Database::VerifyZoneInstance(uint32 zone_id, uint32 instance_id)
{
...
uint16 Database::GetInstanceVersion(uint32 instance_id) {
	if (instance_id == 0)
		return 0;
...
uint32 Database::GetInstanceGroupID(uint32 instance_id) {
	if (instance_id == 0)
		return 0;

	std::string query = StringFormat("SELECT group_id FROM instance_list where id=%u", instance_id);
	auto results = QueryDatabase(query);
...
int64 Database::GetInstanceScaleCap(uint32 instance_id) {
	if (instance_id == 0)
		return 0;

	std::string query = StringFormat("SELECT scale_cap FROM instance_list where id=%llu", instance_id);
	auto results = QueryDatabase(query);
...
uint32 Database::GetTimeRemainingInstance(uint32 instance_id, bool &is_perma)
{
	uint32 start_time = 0;
	uint32 duration = 0;
	uint32 never_expires = 0;
...
uint32 Database::VersionFromInstanceID(uint32 instance_id)
{
...
uint32 Database::ZoneIDFromInstanceID(uint32 instance_id)
{
...
void Database::BuryCorpsesInInstance(uint32 instance_id) {
	std::string query = StringFormat(
...
void Database::DeleteInstance(uint32 instance_id)
{
	std::string query;
	...
	query = StringFormat("DELETE FROM cust_ext_instances WHERE inst_id=%u", instance_id);
	QueryDatabase(query);
...
void Database::GetCharactersInInstance(uint32 instance_id, std::list<uint32> &charid_list) {
...
void Database::PurgeExpiredInstances()
{
	...
	QueryDatabase(fmt::format("DELETE FROM instance_list", imploded_instance_ids));
	QueryDatabase(fmt::format("DELETE FROM instance_list_player", imploded_instance_ids));
	QueryDatabase(fmt::format("DELETE FROM respawn_times WHERE instance_id > 0", imploded_instance_ids));
	QueryDatabase(fmt::format("DELETE FROM spawn_condition_values WHERE instance_id > 0)", imploded_instance_ids));
	QueryDatabase(fmt::format("UPDATE character_corpses SET is_buried = 1, instance_id = 0 WHERE instance_id > 0", imploded_instance_ids));
	QueryDatabase(fmt::format("ALTER TABLE `instance_list` AUTO_INCREMENT = 1"));

	
}

void Database::SetInstanceDuration(uint32 instance_id, uint32 new_duration)
{
