...
bool Database::LoadFileSizes() {


	zonename_size_array.clear();

	std::string query("SELECT size, file_name, should_check FROM zone_sizes");
	...
	for (auto row = results.begin(); row != results.end(); ++row)
	{
		uint32 fileSize = atoi(row[0]);
		std::string fileName = row[1];
		uint32 shouldbeused = atoi(row[2]);

		ZoneSize_Struct zone_size;
		zone_size.shouldCheck = shouldbeused;
		zone_size.size = fileSize;
		zonename_size_array.insert(std::pair<std::string,ZoneSize_Struct>(fileName, zone_size));
	}
...
std::list<uint64> Database::GetZoneSize(const char* fileName) {

	if (fileName == nullptr)
		return std::list<uint64>();

	std::list<uint64> allowed_zone_sizes;

	for (auto iter = zonename_size_array.begin(); iter != zonename_size_array.end(); ++iter)
		if (strcasecmp(iter->first.c_str(), fileName) == 0 && iter->second.shouldCheck == 1)
			allowed_zone_sizes.push_back(iter->second.size);

	return allowed_zone_sizes;
}
...
bool Database::MoveCharacterToZone(uint32 iCharID, const char* iZonename, float x, float y, float z) { 
	std::string query = StringFormat("UPDATE `character_data` SET `zone_id` = %i, `x` = %f, `y` = %f, `z` = %f WHERE `id` = %i", GetZoneID(iZonename), iCharID, x, y, z);
	auto results = QueryDatabase(query);
...
std::map<uint32, MercCharacter_Struct> Database::GetCharactersOnAccount(
	uint32 AccID)
{
	...
	std::string query = StringFormat(
		"SELECT `id`, `level`, `class`, `name` FROM `character_data` WHERE `account_id` = '%d' AND `deleted_at` IS NULL", AccID);
	...
	for (auto row = results.begin(); row != results.end(); ++row)
	{
		MercCharacter_Struct charStruct;
		memset(&charStruct, 0, sizeof(MercCharacter_Struct));
		charStruct.level = atoi(row[1]);
		charStruct.slot = i;
		if(row[3][0])
			strcpy(&charStruct.name[0], row[3]);
		charStruct.mercClass = atoi(row[2]);
		retList[atoi(row[0])] = charStruct;
		i++;
	}
...
void Database::ClearAllGroups(void)
{
	...
	std::string query2("ALTER TABLE `group_id` AUTO_INCREMENT = 1");
...
bool Database::SaveHackerProcesses(std::string processName, uint32 accountId, std::string accountName, std::string ipAddr)
{
	char tmpProc[256];
	DoEscapeString(tmpProc, processName.c_str(), strlen(processName.c_str()));

	std::string query = StringFormat("REPLACE INTO hackers_processes set process_name = '%s', account_id = %d, account_name = '%s', occurtime = %d, ipAddr='%s' ", tmpProc, accountId, accountName.c_str(), time(0), ipAddr.c_str());
	auto results = QueryDatabase(query);
