...
bool EQ::InventoryProfile::SwapItem(
	...
	if (source_item_instance) {
		...
		if ((destination_slot >= invslot::EQUIPMENT_BEGIN && destination_slot <= invslot::EQUIPMENT_END)) {
			...
			if (race_id && class_id && !source_item->IsEquipable(race_id, class_id, true)) {
				fail_state = swapRaceClass;
				return false;
			}
			if (deity_id && class_id != 17 && source_item->Deity && !(deity::ConvertDeityTypeToDeityTypeBit((deity::DeityType)deity_id) & source_item->Deity)) {
	...
	if (destination_item_instance) {
		...
		if ((source_slot >= invslot::EQUIPMENT_BEGIN && source_slot <= invslot::EQUIPMENT_END)) {
			...
			if (race_id && class_id && !destination_item->IsEquipable(race_id, class_id, true)) {
				fail_state = swapRaceClass;
				return false;
			}
