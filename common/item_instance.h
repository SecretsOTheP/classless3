...
namespace EQ
{
	...
	public:
		...
		bool IsEquipable(uint16 race, uint16 class_, bool allowclassless = false) const;
		...
		std::string Serialize(int16 slot_id, uint8 classless = 0) const { InternalSerializedItem_Struct s; s.slot_id = slot_id; s.classless = classless; s.inst = (const void*)this; std::string ser; ser.assign((char*)&s, sizeof(InternalSerializedItem_Struct)); return ser; }
		void Serialize(OutBuffer& ob, int16 slot_id, uint8 classless = 0) const { InternalSerializedItem_Struct isi; isi.slot_id = slot_id;  isi.classless = classless; isi.inst = (const void*)this; ob.write((const char*)&isi, sizeof(isi)); }
	...
	protected:
		...
		ItemData*		m_item;		// Ptr to item data
