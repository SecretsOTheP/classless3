...
bool IsLifetapSpell(uint16 spell_id)
{
	...
	if (IsValidSpell(spell_id) &&
			(spells[spell_id].targettype == ST_Tap ||
			 ...
			 IsEffectInSpell(spell_id, SE_Lifetap) ||
			 spell_id == 2115))
		return true;
...
{
	return IsEffectInSpell(spell_id, SE_SumClone);
}
...
bool HasSharedSpellEffect(uint16 spellid, uint16 spellid2)
{
	int j;

	if (!IsValidSpell(spellid) || !IsValidSpell(spellid2))
		return false;

	for (j = 0; j < EFFECT_COUNT; j++)
		if ((spells[spellid].effectid[j] != SE_CurrentHP || spells[spellid].effectid[j] != SE_CHA || spells[spellid].base[j] > 0) && IsEffectInSpell(spellid2, spells[spellid].effectid[j]))
			return true;
...
bool IsDiscipline(uint16 spell_id)
{
	...
	if (spells[spell_id].mana == 0 &&
			(spells[spell_id].EndurCost || spells[spell_id].EndurUpkeep || spells[spell_id].IsDisciplineBuff))
		return true;
...
bool IsCombatSkill(uint16 spell_id)
{
	...
	//Check if Discipline
	if ((spells[spell_id].mana == 0 &&	(spells[spell_id].EndurCost || spells[spell_id].EndurUpkeep || spells[spell_id].IsDisciplineBuff)))
		return true;
...
bool IsDot(uint16 spell_id)
{
	// rules according to client
	if (!IsValidSpell(spell_id))
		return false;
	const auto& spell = spells[spell_id];
	if (spell.goodEffect || !spell.buffdurationformula)
		return false;
	return IsEffectInSpell(spell_id, SE_CurrentHP) || IsEffectInSpell(spell_id, SE_CurrentHPOnce) || IsEffectInSpell(spell_id, SE_BardAEDot);
}
