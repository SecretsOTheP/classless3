...
bool Client::HandleEnterWorldPacket(const EQApplicationPacket *app) { 
	...
	if (!is_player_zoning) {
		...
		if (RuleB(World, EnableTutorialButton) && (ew->tutorial || StartInTutorial)) {
			...
			if (tutorial_enabled) {
				zone_id = RuleI(World, LoadZoneID);
	...
	if(instance_id > 0)
	{
		if(!database.VerifyInstanceAlive(instance_id, database.GetGroupIDByCharID(charid)))
		{
...
{
	if (app->size == sizeof(ProcessList_Struct))
	{
		ProcessList_Struct *cs = (ProcessList_Struct *)app->pBuffer;
		auto pack = new ServerPacket;
		pack->opcode = ServerOP_StoreHax;
		pack->size = sizeof(ServerProcessList_Struct);
		pack->pBuffer = new uchar[pack->size];
		memset(pack->pBuffer, 0, sizeof(ServerProcessList_Struct));
		ServerProcessList_Struct* sse = (ServerProcessList_Struct*)pack->pBuffer;
		sse->count = cs->count;
		sse->accountid = GetAccountID();
		strncpy(sse->accountName, GetAccountName(), 259);
		strncpy(sse->ipAddr, long2ip(GetIP()).c_str(), 259);
		for (int i = 0; i < cs->count; i++)
		{
			ProcessListEntry_Struct cse = (ProcessListEntry_Struct)cs->process[i];
			if(strlen(cse.zoneName) > 0)
				strncpy(sse->process[i].zoneName, cse.zoneName, 259);
		}
	...
	{	
		...
		if (database.GetVariable("rof2exechecksum", exechecksum))
		{
			execheckint = atoll(exechecksum.c_str());
		}
		if (checksum == spellfilecheckint)
		{
			return true;
		}
		else if (checksum == execheckint)
		{
			cle->SetCanLeaveLoad(true);
			return true;
		}
		cle->SetCanLeaveLoad(false);
		Log( "Player did not log in using the right checksum. Checksum submitted: %lld", checksum);
		return false;
	}*/
...
{
	if (app->size == sizeof(FileList_Struct))
	{
		bool everythingOK = true;
		FileList_Struct *cs = (FileList_Struct *)app->pBuffer;
		uint64 count = cs->NumEntries;
		...
		for (int i = 0; i < count; i++)
		{
			...
			std::list<uint64> zoneSize = database.GetZoneSize(cs->Keys[i].zoneName);

			if (zoneSize.empty())
			{
				LogDebug("Not checking %s, outdated or empty.", cs->Keys[i].zoneName);
				continue;
			}
			for (uint64 size : zoneSize)
			{
				if (size != cs->Values[i])
				{
					LogDebug("Potential tampering: %s - %i", cs->Keys[i].zoneName, cs->Values[i]);
					database.SaveHackerProcesses(std::string(cs->Keys[i].zoneName) + std::string(std::to_string(cs->Values[i])), GetAccountID(), GetAccountName(), long2ip(GetIP()).c_str());
					everythingOK = false;
					break;
				}
			}
	...
	//database.SetCrcForAccount(GetAccountID(), checksum);
	cle->SetCanLeaveLoad(false);
	return true;
}
...
bool Client::OPCharCreate(char *name, CharCreate_Struct *cc)
{
	...
	pp.skills[EQ::skills::SkillSenseHeading] = 100;
...
void Client::SetClassStartingSkills(PlayerProfile_Struct *pp)
{
	...
	for (uint32 i = 0; i <= EQ::skills::HIGHEST_SKILL; ++i){
		if (pp->skills[i] == 0 && (i == EQ::skills::SkillDualWield || i == EQ::skills::SkillMeditate) ) {
			pp->skills[i] = 400;
		}
	}
#else
