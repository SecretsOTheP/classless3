...
uint32 WorldDatabase::GetMaxLevelOnAccount(uint32 accountID)
{
	...
	std::string cquery = StringFormat(
		...
		"FROM                       "
		"character_data             "
		"WHERE `account_id` = %i ORDER BY `name`", accountID);
...
bool WorldDatabase::GetStartZone(PlayerProfile_Struct* in_pp, CharCreate_Struct* in_cc,bool isTitanium)
{
	...
	else
	{
		...
		in_pp->z = 0;
		in_pp->heading = 0;
		in_pp->zone_id = RuleI(World, LoadZoneID);
...
bool WorldDatabase::GetCharSelInventory(uint32 account_id, char *name, EQ::InventoryProfile *inv)
{
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		EQ::ItemData item = GetItem(item_id);
		if (!item.ID)
			continue;
