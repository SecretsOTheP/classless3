...
void ZSList::Process() {
	...
	if (reminder && reminder->Check() && shutdowntimer) {
		SendEmoteMessage(0, 0, 0, Chat::System, "<SYSTEMWIDE MESSAGE>:SYSTEM MSG:World coming down, everyone log out now. World will shut down in %i minutes...", ((shutdowntimer->GetRemainingTime() / 1000) / 60));
	}
}
