...
public:
	...
	inline std::map<uint16, PetInfo>& GetPetInfo() {
		return m_petinfo;
	}
	...
	bool AutoFireEnabled() const { return false; }
	...
	*/
	...
	int64 GetMaxStat() const;
	int32 GetMaxResist() const;
	int64 GetMaxSTR() const;
	int64 GetMaxSTA() const;
	int64 GetMaxDEX() const;
	int64 GetMaxAGI() const;
	int64 GetMaxINT() const;
	int64 GetMaxWIS() const;
	int64 GetMaxCHA() const;
	...
	inline virtual int64 GetHeroicSTR() const { return itembonuses.HeroicSTR; }
	inline virtual int64 GetHeroicSTA() const { return itembonuses.HeroicSTA; }
	inline virtual int64 GetHeroicDEX() const { return itembonuses.HeroicDEX; }
	inline virtual int64 GetHeroicAGI() const { return itembonuses.HeroicAGI; }
	inline virtual int64 GetHeroicINT() const { return itembonuses.HeroicINT; }
	inline virtual int64 GetHeroicWIS() const { return itembonuses.HeroicWIS; }
	inline virtual int64 GetHeroicCHA() const { return itembonuses.HeroicCHA; }
	...
	void CheckHasInstrument(int16 SlotID, EQ::ItemInstance* inst);
	void SendClickyRecast(int16 SlotID, EQ::ItemInstance* inst);
	void CheckWornClickies(int16 SlotID, EQ::ItemInstance* inst);
	...
	inline const int64 GetBaseHP() const { return base_hp; }
	...
	inline uint8 GetEndurancePercent() { return (uint8)(current_endurance / max_end * 100.0); }
	virtual void SetEndurance(int64 newEnd); //This sets the current endurance to the new value
	...
	void SendFakeMovementPacket(Mob* mob);
	void SendCorrectStationaryMovementPacket(Mob* mob);
	...
	bool CheckLootLockout(uint32 npctype_id);
	...
	virtual int32 GetEquipmentMaterial(uint8 material_slot) const { return Mob::GetEquipmentMaterial(material_slot); }
	...
	void LevelChangeInstances();

	float GetMitigationForSlot(uint16 slot = EQ::invslot::slotCharm);

	EQ::textures::MaterialType GetMitigationTextureType(EQ::textures::MaterialType material = EQ::textures::MaterialType::materialInvalidType);
	...
	bool IsItemRecastExpired(int16 slot);
	...
	int FindMemSlotBySpellID(uint16 spellid);
	int GetNextAvailableSpellBookSlot(int starting_slot = 0);
	int GetNextAvailableSpellMemSlot(int starting_slot = 0);
	inline uint32 GetSpellByBookSlot(int book_slot) { return m_pp.spell_book[book_slot]; }
	inline uint32 GetSpellByMemSlot(int slot) { return m_pp.mem_spells[slot]; }
	...
	inline bool XTargettingAvailable() const { return false; }
	...
	inline uint32 GetMercID() const { return m_mercinfo[mercSlot].mercid; }
	inline uint8 GetMercSlot() const { return mercSlot; }
	void SetMercID( uint32 newmercid) { m_mercinfo[mercSlot].mercid = newmercid; }
	void SetMercSlot( uint8 newmercslot) { mercSlot = newmercslot; }
	Merc* GetMerc();
	int8 GetMercIndexByCharacterID(uint32 character_id);
	MercInfo& GetMercInfo(uint8 slot) { return m_mercinfo[slot]; }
	MercInfo& GetMercInfo() { return m_mercinfo[mercSlot]; }
	uint8 GetNumMercs() {	return validMercCharacters.size(); }
	...
	int64 GetActSTR() { return( GetSTR() ); }
	int64 GetActSTA() { return( GetSTA() ); }
	int64 GetActDEX() { return( GetDEX() ); }
	int64 GetActAGI() { return( GetAGI() ); }
	int64 GetActINT() { return( GetINT() ); }
	int64 GetActWIS() { return( GetWIS() ); }
	int64 GetActCHA() { return( GetCHA() ); }
	...
	int64 mod_bindwound_percent(int64 max_percent, Mob* bindmob);
	int64 mod_bindwound_hp(int64 bindhps, Mob* bindmob);
	...
	uint8 GetEdgeClass(uint8 index = 0);

	bool HasEdgeClass(uint8 edge_class);

	void SetEdgeClass(uint8 edge_class, uint8 index = 0);

	uint32 trapid; //ID of trap player has triggered. This is cleared when the player leaves the trap's radius, or it despawns.

	std::map<uint16, PetInfo> m_petinfo; // current pet data, used while loading from and saving to DB
	...
	std::map<uint32, MercCharacter_Struct>& GetValidMercenaries() { return validMercCharacters; };

	bool HasLootLockouts(uint32 npctype_id);
...
private:
	...
	double GetEXPForLevel(uint16 level);

	double MobLevelToExp(int level);
	...
	std::map<uint32, MercCharacter_Struct> validMercCharacters;

	bool checkedValidAltCharacters;
