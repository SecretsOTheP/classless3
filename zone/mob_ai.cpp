...
bool NPC::AICastSpell(Mob* tar, uint8 iChance, uint32 iSpellTypes, bool bInnates) {
	...
	if (IsNPC() && CastToNPC()->GetNPCScaleTier() > 2)
		checked_los = true;
			...
			if (
				...
				switch (AIspells[i].type) {
					...
					case SpellType_Pet: {
						...
						if (!IsPet() && petlist.empty() && (bInnates || zone->random.Roll(25))) {
							AIDoSpellCast(i, tar, mana_cost);
							return true;
						}
						break;
...
bool NPC::AIDoSpellCast(uint8 i, Mob* tar, int32 mana_cost, uint32* oDontDoAgainBefore) {
	...
	const auto& aispell = AIspells[casting_spell_AIindex];

	bool result = false;

	// manacost has special values, -1 is no mana cost, -2 is instant cast (no mana)
	int32 manaCost = spells[aispell.spellid].mana;
	int32 endurCost = spells[aispell.spellid].EndurCost;


	float dist2 = 0;

	if (aispell.type & SpellType_Escape) {
		dist2 = 0;
	}
	else
		dist2 = DistanceSquared(m_Position, tar->GetPosition());
	...
	if (
		((spells[aispell.spellid].targettype == ST_GroupTeleport
			|| aispell.type == SpellType_Heal
			|| spells[aispell.spellid].targettype == ST_AECaster
			|| spells[aispell.spellid].targettype == ST_Group
			|| spells[aispell.spellid].targettype == ST_AEBard && dist2 <= spells[aispell.spellid].aoerange * spells[aispell.spellid].aoerange)
			|| dist2 <= spells[aispell.spellid].range * spells[aispell.spellid].range
			|| aispell.type == SpellType_Pet
			|| spells[aispell.spellid].targettype == ST_Self
			|| spells[aispell.spellid].targettype == ST_TargetOptional && tar == this)
		&&
		(endurCost > 0 && (endurCost <= GetEndurance()) || manaCost > 0 && (manaCost <= GetMana()) || mana_cost == -2)
		)
	{
		StopNavigation();
		result = CastSpell(aispell.spellid, tar->GetID(), EQ::spells::CastingSlot::Gem2, -1, manaCost > 0 ? manaCost : endurCost, oDontDoAgainBefore, -1, -1, 0, 0);
	}
	return result;
}
...
void Client::AI_Process()
{
	...
	else
	{
		...
		if (IsPet()) {
			...
			Mob* supposedOwner = nullptr;
			Mob* tmpPrevious = nullptr;
			for (auto pet : owner->petlist)
			{
				if (pet == this)
				{
					if (supposedOwner == nullptr && tmpPrevious != nullptr)
						supposedOwner = tmpPrevious;
				}
				tmpPrevious = pet;
			}

			glm::vec4 pet_owner_position = supposedOwner ? supposedOwner->GetPosition() : owner->GetPosition();

			float dist = DistanceSquared(m_Position, pet_owner_position);
			if (dist >= 200) { // >=20
				if (AI_movement_timer->Check()) {
					if (dist >= 300) {
		...
		if (is_combat_range) {
			...
			if (target && !IsStunned() && !IsMezzed() && GetAppearance() != eaDead && !IsMeleeDisabled()) {
				...
				if (attack_timer.Check()) {
					...
					if (GetSpecialAbility(SPECATK_PET_AREA_RAMPAGE) && !specialed) {
						int rampage_chance = GetSpecialAbilityParam(SPECATK_PET_AREA_RAMPAGE, 0);
						rampage_chance = rampage_chance > 0 ? rampage_chance : 25;
						if (zone->random.Roll(rampage_chance)) {
							ExtraAttackOptions opts;
							int                cur = GetSpecialAbilityParam(SPECATK_PET_AREA_RAMPAGE, 3);
							if (cur > 0) {
								opts.damage_flat = cur;
							}

							cur = GetSpecialAbilityParam(SPECATK_PET_AREA_RAMPAGE, 4);
							if (cur > 0) {
								opts.armor_pen_percent = cur / 100.0f;
							}

							cur = GetSpecialAbilityParam(SPECATK_PET_AREA_RAMPAGE, 5);
							if (cur > 0) {
								opts.armor_pen_flat = cur;
							}

							cur = GetSpecialAbilityParam(SPECATK_PET_AREA_RAMPAGE, 6);
							if (cur > 0) {
								opts.crit_percent = cur / 100.0f;
							}

							cur = GetSpecialAbilityParam(SPECATK_PET_AREA_RAMPAGE, 7);
							if (cur > 0) {
								opts.crit_flat = cur;
							}

							AreaRampageOnPets(&opts);
							specialed = true;
						}
					}
			...
			if (IsPet()) {
				...
				switch (pStandingPetOrder) {
					case SPO_Follow: {
						...
						if (distance_to_owner >= 100 || z_distance > 100) {
...
void NPC::AI_DoMovement() {
	 ...
	 */
	if (roambox_min_x != 0.0f && roambox_min_y != 0.0f && roambox_max_x != 0.0f && roambox_max_y != 0.0f) {
		...
		if (!IsMoving() && time_until_can_move < Timer::GetCurrentTime()) {
			...
			auto move_x = zone->random.Real(0, movedist);
			auto move_y = movedist - move_x;
			move_x = sqrtf(move_x);
			move_y = sqrtf(move_y);
			move_x *= zone->random.Int(0, 1) ? 1 : -1;
			move_y *= zone->random.Int(0, 1) ? 1 : -1;
			roambox_destination_x = GetX() + move_x;
			roambox_destination_y = GetY() + move_y;
			//Try to calculate new coord using distance.
			if (roambox_destination_x > roambox_max_x || roambox_destination_x < roambox_min_x)
				roambox_destination_x -= move_x * 2;
			if (roambox_destination_y > roambox_max_y || roambox_destination_y < roambox_min_y)
				roambox_destination_y -= move_y * 2;
			...
			if (roambox_destination_x > roambox_max_x || roambox_destination_x < roambox_min_x)
				roambox_destination_x = zone->random.Real(roambox_min_x + 1, roambox_max_x - 1);
			if (roambox_destination_y > roambox_max_y || roambox_destination_y < roambox_min_y)
				roambox_destination_y = zone->random.Real(roambox_min_y + 1, roambox_max_y - 1);
			/**
...
void NPC::AI_Event_SpellCastFinished(bool iCastSucceeded, uint16 slot) {	// if the spell wasn't casted, then take back any extra mana that was given to the bot to cast that spell
	...
	if (!IsMerc())
	{
		if (!iCastSucceeded) {
			...
			if (casting_spell_AIindex < AIspells.size()) {
				if (spells[AIspells[casting_spell_AIindex].spellid].mana && GetMana() - spells[AIspells[casting_spell_AIindex].spellid].mana >= 0)
					SetMana(GetMana() - spells[AIspells[casting_spell_AIindex].spellid].mana);
				else
					SetMana(0);

				if (spells[AIspells[casting_spell_AIindex].spellid].EndurCost && GetEndurance() - spells[AIspells[casting_spell_AIindex].spellid].EndurCost >= 0)
					SetEndurance(GetEndurance() - spells[AIspells[casting_spell_AIindex].spellid].EndurCost);
				else
					SetEndurance(0);
			}
		}
...
void Mob::AreaRampageOnPets(ExtraAttackOptions *opts)
{
	int index_hit = 0;
	int rampage_targets = GetSpecialAbilityParam(SPECATK_PET_AREA_RAMPAGE, 1);
	rampage_targets = rampage_targets > 0 ? rampage_targets : -1;
	m_specialattacks = eSpecialAttacks::AERampage;
	index_hit = hate_list.AreaRampageOnPets(this, GetTarget(), rampage_targets, opts);
...
bool IsSpellEffectInList(DBnpcspellseffects_Struct* spelleffect_list, int32 iSpellEffectID, int32 base, int32 limit, int32 max);
...
#endif
	int32 attack_proc_spell = -1;
	int8 proc_chance = 3;
	int32 range_proc_spell = -1;
	int32 rproc_chance = 0;
	int32 defensive_proc_spell = -1;
	int32 dproc_chance = 0;
...
void NPC::AddSpellEffectToNPCList(int32 iSpellEffectID, int32 base, int32 limit, int32 max)
{
