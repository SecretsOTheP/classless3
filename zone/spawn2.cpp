...
Spawn2::Spawn2(uint32 in_spawn2_id, uint32 spawngroup_id,
	...
	uint32 respawn, uint32 original_respawn, uint32 variance, uint32 timeleft, uint32 grid,
...
: timer(100000), killcount(0)
{
	...
	heading = in_heading;
	respawn_ = respawn;
	original_respawn_ = original_respawn;
	variance_ = variance;
	grid_ = grid;
...
bool ZoneDatabase::PopulateZoneSpawnListClose(uint32 zoneid, LinkedList<Spawn2*> &spawn2_list, int16 version, const glm::vec4& client_position, uint32 repop_distance)
{
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		uint32 Respawn = atoi(row[6]);

		uint32 OriginalRespawn = atoi(row[6]);

		if (zone && zone->IsHotzone() > 0 && Respawn > 0)
		{
			Respawn = Respawn / EQ::Clamp((zone->IsHotzone() + 1), 1, 4);
		}
		...
		new_spawn = new Spawn2(							   // 
			...
			Respawn, // uint32 respawn
			OriginalRespawn, //temp variable
...
Spawn2* ZoneDatabase::LoadSpawn2(LinkedList<Spawn2*> &spawn2_list, uint32 spawn2id, uint32 timeleft) {
    ...
    auto newSpawn = new Spawn2(atoi(row[0]), atoi(row[1]), atof(row[2]), atof(row[3]), atof(row[4]), atof(row[5]),
		Respawn, OriginalRespawn, atoi(row[7]), timeleft, atoi(row[8]), atoi(row[9]), atoi(row[10]),
