...
inline std::string GetMobAttributeByString(Mob *mob, const std::string &attribute)
{
	...
	if (attribute == "mp_min_max") {
		return commify(std::to_string(mob->GetMana())) + " / " +
			commify(std::to_string(mob->GetMaxMana())) + " (" +
	...
	if (attribute == "power") {
		return std::to_string((mob->m_POWER));
	}

	if (attribute == "normal") {
		return std::to_string((mob->m_POWER_NORMAL));
	}

	if (attribute == "gear_normal") {
		return std::to_string((mob->m_GEAR_NORMAL));
	}

	if (attribute == "power_difficulty") {
		return std::to_string((mob->m_POWER_DIFFICULTY));
	}
	...
	if (attribute == "stat_total")
	{
		return commify(std::to_string(mob->GetSTR() + mob->GetSTA() + mob->GetAGI() + mob->GetDEX() + mob->GetINT() + mob->GetWIS() + mob->GetCHA()));
	}

	if (attribute == "melee_haste") {
		return commify(std::to_string((int)(mob->GetHaste() - 100))) + scaling_modified;
	}

	if (attribute == "spell_haste") {
		return commify(std::to_string((int)(mob->GetHaste(EQ::skills::SkillEvocation) - 100))) + scaling_modified;
	}

	if (attribute == "healing_haste") {
		return commify(std::to_string((int)(mob->GetHaste(EQ::skills::SkillMend) - 100))) + scaling_modified;
	}

	if (attribute == "melee_crit") {
		return commify(std::to_string((int)mob->GetCriticalChanceBonus(EQ::skills::Skill1HBlunt))) + scaling_modified;
	}

	if (attribute == "spell_crit") {
		return commify(std::to_string((int)mob->GetCriticalChanceBonus(EQ::skills::SkillEvocation))) + scaling_modified;
	}

	if (attribute == "healing_crit") {
		return commify(std::to_string((int)mob->GetCriticalChanceBonus(EQ::skills::SkillMend))) + scaling_modified;
	}
	...
	if (attribute == "crit") {
		return std::to_string(mob->GetCriticalChanceBonus(-1));
	}
	...
	if (attribute == "defense") {
		return std::to_string(mob->mitigation());
	}
	...
	if (attribute == "melee_offense") {
		return std::to_string(mob->offense(EQ::skills::SkillHandtoHand));
	}

	if (attribute == "spell_offense") {
		return std::to_string(mob->offense(EQ::skills::SkillEvocation));
	}

	if (attribute == "healing_offense") {
		return std::to_string(mob->offense(EQ::skills::SkillMend));
	}

	if (attribute == "mitigation_ac") {
		return std::to_string(mob->GetACBonus());
	}

	if (attribute == "armor_class") {
		return commify(std::to_string(mob->GetACBonus()));
	}

	if (attribute == "attack") {
		return commify(std::to_string(mob->GetATKBonus()));
	}
	...
	if (mob->IsNPC()) {
		...
		if (attribute == "loottable") {
			return std::to_string(!npc->GetLoottableID().empty() ? npc->GetLoottableID()[0] : 0);
		}
	...
	if (mob->IsClient()) {
		...
		if (attribute == "spell_1") {
			auto spellid = client->GetSpellByMemSlot(0);

			return GetSpellName(spellid);
		}
		if (attribute == "spell_2") {
			auto spellid = client->GetSpellByMemSlot(1);

			return GetSpellName(spellid);
		}
		if (attribute == "spell_3") {
			auto spellid = client->GetSpellByMemSlot(2);

			return GetSpellName(spellid);
		}
		if (attribute == "spell_4") {
			auto spellid = client->GetSpellByMemSlot(3);

			return GetSpellName(spellid);
		}
		if (attribute == "spell_5") {
			auto spellid = client->GetSpellByMemSlot(4);

			return GetSpellName(spellid);
		}
		if (attribute == "spell_6") {
			auto spellid = client->GetSpellByMemSlot(5);

			return GetSpellName(spellid);
		}
		if (attribute == "spell_7") {
			auto spellid = client->GetSpellByMemSlot(6);

			return GetSpellName(spellid);
		}
		if (attribute == "spell_8") {
			auto spellid = client->GetSpellByMemSlot(7);

			return GetSpellName(spellid);
		}
		if (attribute == "spell_9") {
			auto spellid = client->GetSpellByMemSlot(8);

			return GetSpellName(spellid);
		}
		if (attribute == "spell_10") {
			auto spellid = client->GetSpellByMemSlot(9);

			return GetSpellName(spellid);
		}
		if (attribute == "spell_11") {
			auto spellid = client->GetSpellByMemSlot(10);

			return GetSpellName(spellid);
		}
		if (attribute == "spell_12") {
			auto spellid = client->GetSpellByMemSlot(11);
...
void Mob::DisplayInfo(Mob *mob)
{
	...
	if (this->IsClient()) {
		...
		if (mob->IsClient()) {
			...
			std::vector<std::string> mod_spells = {
			"spell_1",
			"spell_2",
			"spell_3",
			"spell_4",
			...
			window_text += WriteDisplayInfoSection(mob, "Spells", mod_spells, 1, true);
