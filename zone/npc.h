...
public:
	...
	void LevelScale();
	void RescaleNPC(float modifier = 1.f);
	...
	virtual int		GetAttackDelay() const {return attack_delay;}
	...
	void	AddLootDrop(const EQ::ItemData *item2, ItemList* itemlistconst, int16 charges, uint8 minlevel, uint8 maxlevel, bool equipit, bool wearchange = false, bool quest = false, bool pet = false, bool force_equip = false, uint32 aug1 = 0, uint32 aug2 = 0, uint32 aug3 = 0, uint32 aug4 = 0, uint32 aug5 = 0, uint32 aug6 = 0);
	...
	virtual uint32 		GetEquipmentColor(uint8 material_slot) const { return Mob::GetEquipmentColor(material_slot); }
	...
	void AddSpellEffectToNPCList(int32 iSpellEffectID, int32 base, int32 limit, int32 max);
	...
	virtual int _GetWalkSpeed() { return Mob::_GetWalkSpeed(); }
	virtual int _GetRunSpeed() { return Mob::_GetRunSpeed(); }
	virtual int _GetFearSpeed() { return Mob::_GetFearSpeed(); }
	...
	bool IgnoreScaling() { return ignore_scaling; }

	uint32 GetNPCScaleTier() { return scale_tier; }
	uint32 GetNPCScaleDifficulty() { return scale_difficulty; }
...
protected:
	...
	int32 GetFocusEffect(focusType type, int32 spell_id);
	uint32 innate_proc_spell_id;
