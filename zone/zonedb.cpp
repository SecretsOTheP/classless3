...
bool ZoneDatabase::GetZoneCFG(
	...
	*map_filename = new char[100];
	...
	std::string query = StringFormat(
		...
		"npc_scale_rate, "		 // 61
		"max_movement_update_range, " // 62
		"underworld_teleport_index "	// 63
	...
	zone_data->NPCZoneDifficulty = atoi(row[61]);
	...
	max_movement_update_range = atof(row[62]);
...
bool ZoneDatabase::LoadCharacterData(uint32 character_id, PlayerProfile_Struct* pp, ExtendedProfile_Struct* m_epp){
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		m_epp->edge_pri_class_id = atoi(row[r]); r++;						 // "`edge_pri_class_id`		"
		m_epp->edge_sec_class_id = atoi(row[r]); r++;						 // "`edge_sec_class_id`		"
		m_epp->edge_ter_class_id = atoi(row[r]); r++;						 // "`edge_ter_class_id`		"
		m_epp->edge_merc_character_id = atoi(row[r]); r++;						 // "`edge_merc_character_id`		"
...
bool ZoneDatabase::LoadCharacterLanguages(uint32 character_id, PlayerProfile_Struct* pp){
	for (auto i = 0; i < MAX_PP_LANGUAGE; ++i)
		pp->languages[i] = 100;
...
bool ZoneDatabase::LoadCharacterLeadershipAA(uint32 character_id, PlayerProfile_Struct* pp){
	...
	for (unsigned int i = 0; i < MAX_GROUP_LEADERSHIP_AA_ARRAY * 2; i++)
	pp->leader_abilities.ranks[i] = 5;

	pp->leader_abilities.group.groupAAHealthEnhancement = 0;
	pp->leader_abilities.group.groupAAManaEnhancement = 0;
	pp->leader_abilities.group.groupAAOffenseEnhancement = 0;
	pp->leader_abilities.raid.raidAAHealthEnhancement = 0;
	pp->leader_abilities.raid.raidAAManaEnhancement = 0;
	pp->leader_abilities.raid.raidAAOffenseEnhancement = 0;
	return true;
}
...
bool ZoneDatabase::LoadCharacterSkills(uint32 character_id, PlayerProfile_Struct* pp){
	...
	if(pp->skills[EQ::skills::SkillApplyPoison] == 0)
		pp->skills[EQ::skills::SkillApplyPoison] = 1;
	if (pp->skills[EQ::skills::SkillForage] == 0)
		pp->skills[EQ::skills::SkillForage] = 1;
	if (pp->skills[EQ::skills::SkillFishing] == 0)
		pp->skills[EQ::skills::SkillFishing] = 1;
	if (pp->skills[EQ::skills::SkillSenseHeading] != 400)
		pp->skills[EQ::skills::SkillSenseHeading] = 400;
	if (pp->skills[EQ::skills::SkillDualWield] != 400)
		pp->skills[EQ::skills::SkillDualWield] = 400;
	if (pp->skills[EQ::skills::SkillMeditate] != 400)
		pp->skills[EQ::skills::SkillMeditate] = 400;
	return true;
}
...
bool ZoneDatabase::LoadCharacterBandolier(uint32 character_id, PlayerProfile_Struct* pp)
{
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		EQ::ItemData item_data = database.GetItem(atoi(row[r]));
		if (item_data.ID) {
...
bool ZoneDatabase::LoadCharacterPotions(uint32 character_id, PlayerProfile_Struct *pp)
{
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		i = atoi(row[0]);
		EQ::ItemData item_data = database.GetItem(atoi(row[1]));
		if (!item_data.ID)
			continue;
...
bool ZoneDatabase::SaveCharacterData(uint32 character_id, uint32 account_id, PlayerProfile_Struct* pp, ExtendedProfile_Struct* m_epp){
	...
	std::string query = StringFormat(
		...
		m_epp->edge_pri_class_id,
		m_epp->edge_sec_class_id,
		m_epp->edge_ter_class_id,
		m_epp->edge_merc_character_id,
		mail_key.c_str()
	);
	auto results = database.QueryDatabase(query);
...
const NPCType *ZoneDatabase::LoadNPCTypesData(uint32 npc_type_id, bool bulk_load /*= false*/)
{
	...
	std::string query = StringFormat(
		...
		"npc_types.model, "
		"npc_types.scale_tier, "
		"npc_types.scale_difficulty, "
		"npc_types.flymode, "
		"npc_types.hp_modifier, "
		"npc_types.ignore_scaling, "
		"npc_types.always_aggro, "
		"npc_types.loot_lockout, "
		"npc_types.loot_lockout_timer "
		"FROM npc_types %s",
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		temp_npctype_data->max_hp             = atoll(row[5]);
		temp_npctype_data->current_hp         = temp_npctype_data->max_hp;
		temp_npctype_data->Mana               = atoll(row[6]);
		...
		temp_npctype_data->hp_regen        = atoll(row[47]);
		temp_npctype_data->mana_regen      = atoll(row[48]);
		...
		temp_npctype_data->scale_tier = atoi(row[111]);
		temp_npctype_data->scale_difficulty = atoi(row[112]);
		temp_npctype_data->flymode          = atoi(row[113]);
		temp_npctype_data->hp_modifier = atof(row[114]);
		temp_npctype_data->ignore_scaling = atoi(row[115]) != 0;
		temp_npctype_data->always_aggro	        = atoi(row[116]);
		temp_npctype_data->loot_lockout = atoi(row[117]) == 1 ? true : false;
		temp_npctype_data->loot_lockout_timer = atoi(row[118]) == 1 ? true : false;
...
bool ZoneDatabase::LoadMercInfo(Client *client) {
	...
	std::string query = StringFormat("SELECT MercID, Slot, Name, TemplateID, SuspendedTime, "
                                    ...
                                    "FROM mercs WHERE OwnerCharacterID = '%i'", client->CharacterID());
    auto results = QueryDatabase(query);
    if (!results.Success())
        return false;
    ...
    for (auto row = results.begin(); row != results.end(); ++row) {
        ...
        client->GetMercInfo(slot).mercid = 1;
        client->GetMercInfo(slot).slot = 0;
...
bool ZoneDatabase::SaveMerc(Merc *merc) {
	...
	if(!owner)
		...
		// New merc record
		std::string query = StringFormat("REPLACE INTO mercs "
		...
		"'%u', '%f', '%u', '%u', '%u', '%u', '%i', '%i', '%i', "
		"'%i', '%i', '%i', '%i', '%i', '%i', '%i')",
		merc->GetMercCharacterID(), 0, merc->GetCleanName(), 1,
		owner->GetMercInfo().SuspendedTime, merc->IsSuspended(), owner->GetMercInfo().MercTimerRemaining,
		merc->GetGender(), merc->GetSize(), (int)merc->GetStance(), merc->GetHP(),
...
void ZoneDatabase::SavePetInfo(Client *client)
{
	...
	query = StringFormat("DELETE FROM `character_pet_info` WHERE `char_id` = %u", client->CharacterID());
	results = database.QueryDatabase(query);
	if (!results.Success())
		return;

	auto pets = client->m_petinfo;
	...
	for (auto pet : pets) {
		...
		if (IsEffectInSpell(petinfo.SpellID, 507))
			continue;
		...
		query = StringFormat("INSERT INTO `character_pet_info` "
				...
				"`hp` = %u, `mana` = %u, `size` = %f",
				client->CharacterID(), i, petinfo.Name, petinfo.petpower, petinfo.SpellID,
		...
		for (int index = 0; index < max_slots; index++) {
			...
			if (query.length() == 0)
				query = StringFormat("INSERT INTO `character_pet_buffs` "
						...
						client->CharacterID(), i, index, petinfo.Buffs[index].spellid,
		...
		for (int index = EQ::invslot::EQUIPMENT_BEGIN; index <= EQ::invslot::EQUIPMENT_END; index++) {
			...
			if (query.length() == 0)
				query = StringFormat("INSERT INTO `character_pet_inventory` "
						...
						"VALUES (%u, %u, %u, %u)",
						client->CharacterID(), i, index, petinfo.Items[index]);
			else
				query += StringFormat(", (%u, %u, %u, %u)", client->CharacterID(), i, index, petinfo.Items[index]);
		}
		database.QueryDatabase(query);
		i++;
	}
...
void ZoneDatabase::RemoveFactions(Client *client) {
...
void ZoneDatabase::LoadPetInfo(Client *client)
{
	...
	std::map<uint16, PetInfo> petinfo = client->GetPetInfo();
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		if (client->m_petinfo.find(i) == client->m_petinfo.end())
			continue;
		...
		client->m_petinfo[i].Buffs[slot_id].spellid = spell_id;
		client->m_petinfo[i].Buffs[slot_id].level = caster_level;
		client->m_petinfo[i].Buffs[slot_id].player_id = caster_id;
		client->m_petinfo[i].Buffs[slot_id].effect_type = 2; // Always 2 in buffs struct for real buffs

		client->m_petinfo[i].Buffs[slot_id].duration = ticsremaining;
		client->m_petinfo[i].Buffs[slot_id].counters = counters;
		client->m_petinfo[i].Buffs[slot_id].bard_modifier = bard_mod;
	}
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		if (client->m_petinfo.find(pet) == client->m_petinfo.end())
			continue;
		...
		client->m_petinfo[pet].Items[slot] = atoul(row[2]);
	}
}
