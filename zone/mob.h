...
public:
	...
	float offense(EQ::skills::SkillType skill, bool instant = false, bool cancrit = true);
	float mitigation();
	float AntiMitigation(bool ignoreCharms = false);
	Mob* GetScaleTarget();
	...
	double RollD20(float offense, float mitigation, bool rand = true); // CALL THIS FROM THE DEFENDER
	...
	virtual int64 GetActDSDamage(uint16 spell_id, int64 value);
	virtual int64 GetActSpellCorrection(uint16 spell_id, int64 value, bool isproc = false, bool istic = false, EQ::skills::SkillType skill = EQ::skills::Skill1HBlunt);
	virtual float GetActSpellSynergy(uint16 spell_id, bool usecache = true);
	virtual int64 GetActSpellDamage(uint16 spell_id, int64 value, Mob* target = nullptr, bool isproc = false);
	virtual int64 GetActDoTDamage(uint16 spell_id, int64 value, Mob* target, bool isproc = false);
	virtual int64 GetActSpellHealing(uint16 spell_id, int64 value, Mob* target = nullptr, bool isproc = false);
	virtual int64 GetActHoTHealing(uint16 spell_id, int64 value, Mob* target, bool isproc = false);
	virtual int64 GetActSpellCost(uint16 spell_id, int64 cost){ return cost;}
	...
	virtual bool CastSpell(uint16 spell_id, uint16 target_id, EQ::spells::CastingSlot slot = EQ::spells::CastingSlot::Item, int32 casttime = -1,
		int64 mana_cost = -1, uint32* oSpellWillFinish = 0, uint32 item_slot = 0xFFFFFFFF,
	...
	virtual bool DoCastSpell(uint16 spell_id, uint16 target_id, EQ::spells::CastingSlot slot = EQ::spells::CastingSlot::Item, int32 casttime = -1,
		...
		uint32 aa_id = 0);
	void CastedSpellFinished(uint16 spell_id, uint32 target_id, EQ::spells::CastingSlot slot, int64 mana_used,
		uint32 inventory_slot = 0xFFFFFFFF, int16 resist_adjust = 0);
	bool SpellFinished(uint16 spell_id, Mob *target, EQ::spells::CastingSlot slot = EQ::spells::CastingSlot::Item, int64 mana_used = 0,
	...
	virtual bool SpellEffect(Mob* caster, uint16 spell_id, float partial = 100, int level_override = -1, bool isproc = false);
	...
	void BeamDirectional(uint16 spell_id, int16 resist_adjust, bool isproc = false);
	void ConeDirectional(uint16 spell_id, int16 resist_adjust, bool isproc = false);
	...
	void BuffFadeBeneficialByCaster(Mob *caster);
	...
	int64 RuneAbsorb(int64 damage, uint16 type);
	...
	inline virtual void SetHP(int64 hp) { if (hp >= max_hp) current_hp = max_hp; else current_hp = hp;}
	bool ChangeHP(Mob* other, int64 amount, uint16 spell_id = 0, int8 buffslot = -1, bool iBuffTic = false);
	inline void SetOOCRegen(int32 newoocregen) {ooc_regen = newoocregen;}
	virtual void Heal();
	virtual void HealDamage(int64 ammount, Mob* caster = nullptr, uint16 spell_id = SPELL_UNKNOWN);
	...
	virtual inline double GetHPRatio() const { return max_hp == 0 ? 0 : ((double)current_hp/max_hp*100); }
	virtual inline int64 GetIntHPRatio() const { return max_hp == 0 ? 0 : static_cast<int64>(current_hp * 100 / max_hp); }
	inline int32 GetAC() const { return AC; }
	inline int32 GetACBonus() const { return itembonuses.AC + spellbonuses.AC; }
	inline virtual int32 GetATK() const { return ATK + itembonuses.ATK + spellbonuses.ATK; }
	inline virtual int32 GetATKBonus() const { return itembonuses.ATK + spellbonuses.ATK; }
	inline virtual int64 GetSTR() const { return (int64)(std::max((double)((STR) * m_POWER_NORMAL * m_POWER_DIFFICULTY + itembonuses.STR + spellbonuses.STR), 1.0)); }
	inline virtual int64 GetSTA() const { return (int64)(std::max((double)((STA) * m_POWER_NORMAL * m_POWER_DIFFICULTY + itembonuses.STA + spellbonuses.STA), 1.0)); }
	inline virtual int64 GetDEX() const { return (int64)(std::max((double)((double)(DEX) * m_POWER_NORMAL * m_POWER_DIFFICULTY + itembonuses.DEX + spellbonuses.DEX), 1.0)); }
	inline virtual int64 GetAGI() const { return (int64)(std::max((double)((AGI) * m_POWER_NORMAL * m_POWER_DIFFICULTY + itembonuses.AGI + spellbonuses.AGI), 1.0)); }
	inline virtual int64 GetINT() const { return (int64)(std::max((double)((INT) * m_POWER_NORMAL * m_POWER_DIFFICULTY + itembonuses.INT + spellbonuses.INT), 1.0)); }
	inline virtual int64 GetWIS() const { return (int64)(std::max((double)((WIS) * m_POWER_NORMAL * m_POWER_DIFFICULTY + itembonuses.WIS + spellbonuses.WIS), 1.0)); }
	inline virtual int64 GetCHA() const { 
		if(IsNPC() && !IsMerc())
		{
			float target = m_POWER * m_POWER_NORMAL * m_POWER_DIFFICULTY;
			target -= GetSTR() + GetSTA() + GetDEX() + GetAGI() + GetINT() + GetWIS() + (CHA * m_POWER_NORMAL * m_POWER_DIFFICULTY + itembonuses.CHA + spellbonuses.CHA);
			return std::max((int64)target, (int64)1); 
		}
		else
			return std::max((double)((CHA) * m_POWER_NORMAL * m_POWER_DIFFICULTY + itembonuses.CHA + spellbonuses.CHA), 1.0);
	}
	...
	inline virtual int64 GetHeroicSTR() const { return 0; }
	inline virtual int64 GetHeroicSTA() const { return 0; }
	inline virtual int64 GetHeroicDEX() const { return 0; }
	inline virtual int64 GetHeroicAGI() const { return 0; }
	inline virtual int64 GetHeroicINT() const { return 0; }
	inline virtual int64 GetHeroicWIS() const { return 0; }
	inline virtual int64 GetHeroicCHA() const { return 0; }
	inline virtual int64 GetMaxSTR() const { return GetSTR(); }
	inline virtual int64 GetMaxSTA() const { return GetSTA(); }
	inline virtual int64 GetMaxDEX() const { return GetDEX(); }
	inline virtual int64 GetMaxAGI() const { return GetAGI(); }
	inline virtual int64 GetMaxINT() const { return GetINT(); }
	inline virtual int64 GetMaxWIS() const { return GetWIS(); }
	inline virtual int64 GetMaxCHA() const { return GetCHA(); }
	...
	virtual int64 CalcMaxEndurance();
	virtual int64 CalcBaseHP();
	virtual int64 CalcBaseMana();
	virtual int64 CalcBaseEndurance();
	inline int64 GetMaxMana() const { return max_mana; }
	inline int64 GetMana() const { return current_mana; }
	virtual int64 GetEndurance() const { return current_endurance; }
	virtual int64 GetMaxEndurance() const { return max_end; }
	virtual void SetEndurance(int64 newEnd) { current_endurance = newEnd; }
	int64 GetItemHPBonuses();
	int64 GetSpellHPBonuses();
	virtual const int64& SetMana(int64 amount);
	inline double GetManaRatio() const { return max_mana == 0 ? 100 :
	...
	int GetWalkspeed() { return(_GetWalkSpeed()); }
	int GetRunspeed() { return(_GetRunSpeed()); }
	...
	float GetMovespeed() { return (float)(IsRunning() ? GetRunspeed() : GetWalkspeed()); }
	bool IsRunning() const { return m_is_running; }
	void SetRunning(bool val) { m_is_running = val; }
	virtual void GMMove(float x, float y, float z, float heading = 0.01, bool SendUpdate = true, bool SaveGuardSpot = true);
	...
	void HalveAggro(Mob *other) { uint64 in_hate = GetHateAmount(other); SetHateAmountOnEnt(other, (int64)(in_hate > 1 ? in_hate / 2.0 : (int64)1)); }
	void DoubleAggro(Mob *other) { uint64 in_hate = GetHateAmount(other); SetHateAmountOnEnt(other, (int64)(in_hate ? in_hate * 2.0: (int64)1)); }
	...
	virtual Mob* MakePet(uint16 spell_id, const char* pettype, const char *petname = nullptr);
	virtual Mob* MakePoweredPet(uint16 spell_id, const char* pettype, int16 petpower, const char *petname = nullptr, float in_size = 0.0f);
	virtual Mob* MakePoweredNPCPet(uint16 spell_id, uint32 npcTypeId, int16 petpower, const char *petname = nullptr, float in_size = 0.0f);
	virtual Mob* MakePoweredPCPet(uint16 spell_id, Client* affected, int16 petpower, const char *petname = nullptr, float in_size = 0.0f);
	...
	float GetCriticalChanceBonus(int16 skill);
	...
	int64 ApplySpellEffectiveness(int16 spell_id, int64 value, bool IsBard = false, uint16 caster_id=0);
	...
	std::vector<Mob*> GetSwarmPets();
	uint32 GetPetCount();
	void RemovePet(Mob* ptr);
	void RemoveAllPets();
	virtual Mob* GetOwner();
	virtual void SetOwner(Mob* newOwner) { owner = newOwner; }
	virtual Mob* GetOwnerOrSelf();
	Mob* GetUltimateOwner();
	void AddPet(Mob* NewPet);
	...
	inline virtual bool HasOwner() { if (GetOwner() == nullptr) { return false; } return owner; }
	inline virtual bool IsPet() { return(HasOwner() && !IsMerc()); }
	bool HasPet() {
		return !petlist.empty();
	}
	...
	virtual float GetHaste(int16 skill = EQ::skills::Skill1HBlunt);
	...
	void DoSpecialAttackDamage(Mob *who, EQ::skills::SkillType skill, int64 base_damage, int64 min_damage = 0, int64 hate_override = -1, int ReuseTime = 10);
	...
	void AreaRampageOnPets(ExtraAttackOptions *opts);
	...
	bool IsFleeing() { return (flee_mode); } // This returns true if the mob is feared or fleeing due to low HP
	...
	static int64 GetLevelHP(uint8 tlevel);
	uint32 GetZoneID() const; //for perl
	virtual int64 CheckAggroAmount(uint16 spell_id, Mob *target, bool isproc = false);
	virtual int64 CheckHealAggroAmount(uint16 spell_id, Mob *target, uint64 heal_possible = 0);
	...
	int64 CalcSpellEffectValue_formula(int formula, int64 base, int64 max, int caster_level, uint16 spell_id, int ticsremaining = 0);
	...
	// HP Event
	inline int64 GetNextHPEvent() const { return nexthpevent; }
	void SetNextHPEvent( int64 hpevent );
	void SendItemAnimation(Mob *to, const EQ::ItemData *item, EQ::skills::SkillType skillInUse, float velocity = 4.0);
	inline int64& GetNextIncHPEvent() { return nextinchpevent; }
	...
	bool AddAA(uint32 id, uint32 points, uint32 charges = 0);
	...
	int GetWeaponDamage(Mob *against, const EQ::ItemInstance *weapon_item, int64 *hate = nullptr);

	int64 GetHPRegen() const;
	void SetHPRegen(int64 in_hp_regen) { hp_regen = in_hp_regen; }
...
public:
	...
	int64 WIS;
	int64 CHA;
	float m_SYNERGY[EQ::spells::SPELL_GEM_COUNT];
	float m_POWER;
	float m_POWER_NORMAL;
	float m_GEAR_NORMAL;
	float m_POWER_DIFFICULTY;
	float m_HP_MODIFIER;
	float m_Melee_CritRoll;
	float m_Range_CritRoll;
	int32 MR;
	int32 CR;
	int32 FR;
	int32 DR;
	...
	int32 Corrup;
	int32 PhR;
	std::list<Mob*> petlist;
	bool QuestSpawned;
	bool QuestLootEntity;
