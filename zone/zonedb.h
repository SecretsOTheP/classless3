...
public:
	...
	//Edge custom load
	bool LoadCharacterLootLockouts(std::map<uint32, LootLockout>& loot_lockout_list, uint32 account_id, uint32 ip_addr);
	...
	/* Zone related   */
	bool		GetZoneCFG(uint32 zoneid, uint16 instance_id, NewZone_Struct *data, bool &can_bind, bool &can_combat, bool &can_levitate, bool &can_castoutdoor, bool &is_city, uint8 &is_hotzone, bool &allow_mercs, double &max_movement_update_range, uint8 &zone_type, int &ruleset, char **map_filename);
	...
	uint32	GetZoneForage(uint32 ZoneID, uint32 skill); /* for foraging */
	uint32	GetZoneFishing(uint32 ZoneID, uint32 skill, uint32 &npc_id, uint8 &npc_chance);
