	...
	while (it != mob_list.end()) {
		...
		if(mob_dead) {
			if(mob->IsMerc()) {
				...
				if (mob && mob->IsClient() && mob->CastToClient()->IsTrader() && mob->CastToClient()->CharacterID() != 0)
					database.DeleteTraderItem(mob->CastToClient()->CharacterID());

				entity_list.BuffFadeByCasterDeconstructor(mob);
...
void EntityList::SendZoneCorpsesBulk(Client *client)
{
	...
	for (auto it = corpse_list.begin(); it != corpse_list.end(); ++it) {
		...
		if (spawn && spawn->InZone()) {
			auto cAccessList = spawn->GetCorpseAccessList();
			if(cAccessList.find(client) != cAccessList.end())
			{
				memset(&ns, 0, sizeof(NewSpawn_Struct));
				spawn->FillSpawnStruct(&ns, client);
				bzsp->AddSpawn(&ns);
		...
		bool ignore_sender, float dist, Mob *SkipThisMob, bool ackreq, eqFilterType filter)
{
	...
	for (auto &e : GetCloseMobList(sender, dist)) {
		Mob *mob = e.second;
		...
		if ((!ignore_sender || client != sender) && (client != SkipThisMob)) {
			...
			if (
				...
				client->QueuePacket(app, ackreq, Client::CLIENT_CONNECTED);
			}
		}
	}
}
...
void EntityList::QueueSpawnForClients(Mob *sender, const EQApplicationPacket *app,
	bool ignore_sender, bool ackreq)
{
	auto it = client_list.begin();
	while (it != client_list.end()) {
		...
		//if (zone && zone->GetInstanceID() == 0 && app->size == sizeof(NewSpawn_Struct))
		//{
		//	NewSpawn_Struct *ns = (NewSpawn_Struct *)app->pBuffer;
		//	ns->spawn.level = ent->GetLevel() <= ns->spawn.level ? ent->GetLevel() : ns->spawn.level;
		//}
...
void EntityList::QueueClientsGuildBankItemUpdate(const GuildBankItemUpdate_Struct *gbius, uint32 GuildID)
{
	...
	EQ::ItemData Item = database.GetItem(gbius->ItemID);
	...
	while (it != client_list.end()) {
		...
		if (client->IsInGuild(GuildID)) {
			if (Item.ID && (gbius->Permissions == GuildBankPublicIfUsable))
...
void EntityList::Genocide(Mob *who, uint32 hate)
{
	auto it = npc_list.begin();
	while (it != npc_list.end()) {
		if(it->second)
			it->second->Damage(who, it->second->GetMaxHP(), SPELL_UNKNOWN, EQ::skills::SkillOffense);
		//it->second->AddToHateList(who, hate);
		++it;
	}
}
...
//check a limit over the entire zone.
//returns true if the limit has not been reached
bool EntityList::LimitCheckTypeTier(uint32 npc_type)
{
	auto tmpList = npc_list;

	for (auto npc : npc_list) {
		if (npc.second && npc.second->GetNPCScaleTier() > 1 && npc.second->GetNPCTypeID() == npc_type) {
				return false;
		}
	}
	return true;
...
void EntityList::HideCorpses(Client *c, uint8 CurrentMode, uint8 NewMode)
{
	...
	while (it != corpse_list.end()) {
		...
		if (b && (b->GetCharID() != c->CharacterID()) && !b->IsEmpty(c)) {
...
void EntityList::BuffFadeByCaster(Mob* caster) {
	for (auto it : mob_list) {
		if (it.second)
			it.second->BuffFadeBeneficialByCaster(caster);
	}

	for (auto it : mob_list) {
		if (it.second)
		{
			if(!it.second->GetUltimateOwner()->IsClient())
				it.second->BuffFadeDetrimentalByCaster(caster);
		}
	}
...
void EntityList::BuffFadeByCasterDeconstructor(Mob* caster) {
	for (auto it : mob_list) {
		if (it.second && it.second != caster)
			it.second->BuffFadeBeneficialByCaster(caster);
	}
...
void EntityList::BuffFadeBySpellIDAndCaster(uint16 spell_id, uint16 caster) {
	for (auto it : mob_list) {
		if (it.second)
			it.second->BuffFadeBySpellIDAndCaster(spell_id, caster);
	}
}
