...
public:
	...
	void	CastGroupSpell(Mob* caster,uint16 spellid, uint32 gid, bool isproc = false);
	void	SplitExp(uint32 exp, Mob* other);
	uint64	GetTotalRaidDamage(Mob* other);
	void	BalanceHP(int64 penalty, uint32 gid, float range = 0, Mob* caster = nullptr, int64 limit = 0);
	void	BalanceMana(int64 penalty, uint32 gid,  float range = 0, Mob* caster = nullptr, int64 limit = 0);
	void	HealGroup(int64 heal_amt, Mob* caster, uint32 gid, float range = 0);
	void	SmartHealGroup(int64 heal_amt, Mob* caster, float range = 0);
