...
public:
	...
	virtual bool IsTypePet()		const { return false; }
...
public:
	...
	void	BuffFadeByCaster(Mob* caster);
	void	BuffFadeByCasterDeconstructor(Mob* caster);
	void	BuffFadeBySpellIDAndCaster(uint16 spellid, uint16 caster);
	...
	void	QueueSpawnForClients(Mob* sender, const EQApplicationPacket* app, bool ignore_sender = false, bool ackreq = true);
	...
	void	AEAttack(Mob *attacker, float dist, int Hand = EQ::invslot::slotPrimary, int count = 0, bool IsFromSpell = false);
	void	AETaunt(Client *caster, float range=0, int32 bonus_hate=0);
	void	AESpell(Mob *caster, Mob *center, uint16 spell_id, bool affect_caster = true, int16 resist_adjust = 0, int *max_targets = nullptr, bool isproc = false);
	void	MassGroupBuff(Mob *caster, Mob *center, uint16 spell_id, bool affect_caster = true, bool isproc = false);
	...
	void	Genocide(Mob* who, uint32 hate = 0);
	...
	bool	LimitCheckTypeTier(uint32 npc_type);
