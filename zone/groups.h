...
public:
	...
	std::list<std::string>&	GetClientNames() const;
#ifdef BOTS
	...
	void	CastGroupSpell(Mob* caster,uint16 spellid, bool isproc = false);
	...
	uint64	GetTotalGroupDamage(Mob* other);
	...
	bool	LearnMembers();
	void	VerifyGroup();
	void	BalanceHP(int64 penalty, float range = 0, Mob* caster = nullptr, int64 limit = 0);
	void	BalanceMana(int64 penalty, float range = 0, Mob* caster = nullptr, int64 limit = 0);
	void	HealGroup(int64 heal_amt, Mob* caster, float range = 0);
