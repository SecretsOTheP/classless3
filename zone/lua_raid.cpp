...
void Lua_Raid::BalanceHP(int64 penalty, uint32 group_id) {
	Lua_Safe_Call_Void();
...
Lua_Client Lua_Raid::GetMember(int index) {
	...
	Client* c = self->members[index].member ? (self->members[index].member->IsClient() ? self->members[index].member : nullptr) : nullptr;
...
int Lua_Raid::GetGroupNumber(int index) {
	...
	int number = self->members[index].GroupNumber ? (self->members[index].member->IsClient() ? self->members[index].GroupNumber : -1) : -1;
...
luabind::scope lua_register_raid() {
	return luabind::class_<Lua_Raid>("Raid")
		...
		.def("BalanceHP", (void(Lua_Raid::*)(int,int64))&Lua_Raid::BalanceHP)
