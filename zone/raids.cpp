...
void Raid::RemoveMember(const char *characterName)
{
	...
	for (int x = 0; x < MAX_RAID_MEMBERS; x++)
	{
		if (strlen(members[x].membername) == 0 || !members[x].member)
			continue;
		else
			members[x].member->CalcBonuses();
	}
...
void Raid::CastGroupSpell(Mob* caster, uint16 spellid, uint32 gid, bool isproc)
{
	...
	for(int x = 0; x < MAX_RAID_MEMBERS; x++)
	{
		if(members[x].member == caster) {
			caster->SpellOnTarget(spellid, caster, false, false, 0, isproc);
		...
		else if(members[x].member != nullptr)
		{
			if(members[x].GroupNumber == gid){
				...
				if(distance <= range2){
					caster->SpellOnTarget(spellid, members[x].member, false, false, 0, isproc);
#ifdef GROUP_BUFF_PETS
					for (auto pet : members[x].member->petlist)
					{
						pet_iterator_list.push_back(pet);
					}
#endif
	...
	{
		caster->SpellOnTarget(spellid, petiter, false, false, 0, isproc);
	}

}


uint64 Raid::GetTotalRaidDamage(Mob* other)
{
	uint64 total = 0;
...
void Raid::HealGroup(int64 heal_amt, Mob* caster, uint32 gid, float range)
{
	if (!caster)
		return;
...
void Raid::BalanceHP(int64 penalty, uint32 gid, float range, Mob* caster, int64 limit)
{
	if (!caster)
		return;
...
void Raid::BalanceMana(int64 penalty, uint32 gid, float range, Mob* caster, int64 limit)
{
	if (!caster)
		return;
...
void Raid::GroupBardPulse(Mob* caster, uint16 spellid, uint32 gid){
...
#ifdef GROUP_BUFF_PETS
					for (auto pet : members[z].member->petlist)
					{
						pet_iterator_list.push_back(pet);
					}
#endif
	...
	for (auto petiter : pet_iterator_list)
	{
		petiter->BardPulse(spellid, caster);
	}
}
...
Mob* Raid::GetHighest()
{
	...
	for (i = 0; i < MAX_RAID_MEMBERS; i++)
	{
		if (members[i].member)
		{
			if(members[i].member->IsMerc())
			{
				if(members[i].member->m_POWER * members[i].member->m_POWER_NORMAL > power)
				{
					power = members[i].member->m_POWER * members[i].member->m_POWER_NORMAL;
					highest = members[i].member;
				}
			}
			else
			{
				if(members[i].member->m_POWER > power)
				{
					power = members[i].member->m_POWER;
					highest = members[i].member;
				}
			}
		}
