...
int Mob::_GetWalkSpeed() {

	if (IsRooted() || IsStunned() || IsMezzed() || IsClient() && CastToClient()->GetFeigned())
		return 0;
...
int Mob::_GetRunSpeed() {
	if (IsRooted() || IsStunned() || IsMezzed() || IsPseudoRooted() || IsClient() && CastToClient()->GetFeigned())
		return 0;
...
int64 Mob::CalcBaseHP()
{

}

int64 Mob::CalcBaseMana()
{
	...
	//int32 baseManaCalc = ((GetINT() + GetWIS() + (CharmingPersonality ? (GetCHA() - 10) / 2 + 10 : GetCHA()) - 20) * 10);
	int64 baseManaCalc = 500.0f * sqrt(( + (double)GetWIS() + (CharmingPersonality ? ((double)GetCHA()) / 2.0 : GetCHA()))/10.0);

	return std::max((int64)100, baseManaCalc);
}

int64 Mob::CalcBaseEndurance()
{

}

int64 Mob::CalcMaxEndurance()
{
	max_end = CalcBaseEndurance() + itembonuses.Endurance + spellbonuses.Endurance;
...
void Mob::FillSpawnStruct(NewSpawn_Struct* ns, Mob* ForWho)
{
	...
	ns->spawn.petOwnerId	= owner ? owner->GetID() : 0;
...
void Mob::CreateHPPacket(EQApplicationPacket* app)
{
	...
	if (IsNPC() && (GetNextIncHPEvent() > 0))
	{
		...
		{
			char buf[16];
			snprintf(buf, 9, "%lli", GetNextIncHPEvent());
			buf[9] = '\0';
			SetNextIncHPEvent(-1);
		...
		if (current_hp != last_hp || force_update_all) {
			...
			if (this->CastToClient()->hp_self_update_throttle_timer.Check() || force_update_all) {
				...
				if (!skip_self || this->CastToClient()->ClientVersion() >= EQ::versions::ClientVersion::SoD) {
					...
					hp_packet_client->cur_hp = std::min(GetHP(), (int64)2147483646);
					hp_packet_client->spawn_id = GetID();
					hp_packet_client->max_hp   = std::min(GetMaxHP(), (int64)2147483646);
...
void Mob::ShowStats(Client* client)
{
	if (IsClient()) {
		...
		if (client->Admin() >= 100)
			client->Message(Chat::White, "  EntityID: %i  OwnerID: %i AIControlled: %i Targetted: %i", GetID(), GetOwner(), IsAIControlled(), targeted);
		...
		if (IsNPC()) {
			...
			client->Message(Chat::White, "  NPCID: %u  SpawnGroupID: %u Grid: %i NPCType_LootTable: %u FactionID: %i SpellsID: %u ", GetNPCTypeID(),spawngroupid, n->GetGrid(), n->GetLoottableID().empty() ? 0 : n->GetLoottableID().at(0), n->GetNPCFactionID(), n->GetNPCSpellsID());
...
Mob* Mob::GetScaleTarget()
{
    Mob *lead = nullptr;
    Mob *self = this;

    //hack fix for trader crash :(
    if (zone && zone->GetZoneID() == RuleI(World, LoadZoneID))
        return this;

    if((self->IsMerc() || self->IsPet()) && self->GetUltimateOwner()->IsClient())
    {
        self = self->GetUltimateOwner();
    }

    if(self->IsClient())
    {
        Raid *kr = entity_list.GetRaidByClient(self->CastToClient());
        if(kr)
        {
            lead = kr->GetHighest();
        }
        if(lead == nullptr)
        {
            Group *kg = entity_list.GetGroupByClient(self->CastToClient());
            if(kg)
            {
                lead = kg->GetHighest() ;
            }
...
void Mob::GMMove(float x, float y, float z, float heading, bool SendUpdate, bool bSaveGuardSpot) {
	m_Position.x = x;
	m_Position.y = y;
	m_Position.z = z;
	SetHeading(heading);
...
void Mob::ChangeSize(float in_size = 0, bool bNoRestriction) {
	...
	if (!bNoRestriction)
	{
		if (this->IsClient() || !petlist.empty())
			if (in_size < 3.0)
				in_size = 3.0;
...
bool Mob::HateSummon() {
	...
	// validate hp
	int64 hp_ratio = GetSpecialAbilityParam(SPECATK_SUMMON, 1);
	hp_ratio = hp_ratio > 0 ? hp_ratio : 97;
	if(GetHPRatio() > static_cast<double>(hp_ratio)) {
		return false;
	}
...
// hp event
void Mob::SetNextHPEvent( int64 hpevent )
{
	nexthpevent = hpevent;
}
...
int64 Mob::GetLevelHP(uint8 tlevel)
{
	int multiplier = 0;
	if (tlevel < 10)
	{
...
int32 Mob::GetActSpellCasttime(uint16 spell_id, int32 casttime) {
	...
	casttime /= GetHaste(IsBeneficialSpell(spell_id) ? EQ::skills::SkillMend : spells[spell_id].skill)/100.f;
	return casttime;
}
...
float Mob::GetHaste(int16 skill)
{
...
float Mob::GetCriticalChanceBonus(int16 skill)
{
