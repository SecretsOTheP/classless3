...
bool Client::Process() {
	...
	if (Connected() || IsLD()) {
		...
		if (dead && dead_timer.Check()) {
			database.MoveCharacterToZone(GetName(), database.GetZoneName(RuleI(World, LoadZoneID)));
		...
		//Anti-AFK bard code - kept here for reference.
		//if (bardsong_stoptimer.Check() && bardsong_stoptimer.Enabled() && bardsong != 0)
		//{
		//	_StopSong();
		...
		bool may_use_ranged_attacks = false;
		/*
		...
		*/
		...
		if (auto_attack) {
			if (!IsAIControlled() && !dead
				...
				may_use_ranged_attacks = true;

			if (may_use_ranged_attacks && attack_timer.Enabled()) {
				//if the range timer is enabled, we need to consider it
				if (!attack_timer.Check(false)) {
					//the ranged timer has not elapsed, cannot attack.
					may_use_ranged_attacks = false;
				}
			}

			if (may_use_ranged_attacks && attack_dw_timer.Enabled()) {
				//if the range timer is enabled, we need to consider it
				if (!attack_dw_timer.Check(false)) {
					//the ranged timer has not elapsed, cannot attack.
					may_use_ranged_attacks = false;
				}
			}
		...
		if (auto_attack &&  auto_attack_target != nullptr && may_use_ranged_attacks && !CombatRange(auto_attack_target)) {
			AutoFire();
		}
		...
		if (has_loaded_group && zone->GetInstanceID() > 0)
		{
			auto pGroup = GetGroup();
			if (pGroup)
			{
				if (zone->GetInstanceGroupID() != pGroup->GetID())
				{
					Kill();
				}
			}
		...
		else if (auto_attack && may_use_attacks && auto_attack_target != nullptr
...
void Client::BulkSendMerchantInventory(int merchant_id, int npcid, bool bSpecialLogic) {
	EQ::ItemData handyitem;
	memset(&handyitem, 0, sizeof(handyitem));
	...
	for (itr = merlist.begin(); itr != merlist.end() && i <= numItemSlots; ++itr) {
		...
		if (item.ItemType == 20 && GetClass() == CLASSLESS)
			continue;

		if (!(ml.classes_required & (1 << (GetClass() - 1))) && GetClass() != CLASSLESS)
			continue;
	...
	switch(memspell->scribing)
	{
		case memSpellScribing:	{	// scribing spell to book
			...
			if (inst && inst->IsClassCommon())
			{
				...
				if (RuleB(Character, RestrictSpellScribing) && !item->IsEquipable(GetBaseRace(), GetClass())) {
	...
	switch(mc->from_slot)
	{
		...
		case 2:	// bank
		{
			...
			if(zone && zone->GetZoneID() != RuleI(World, LoadZoneID) && (!banker || distance > USE_NPC_RANGE2))
			{
				...
				database.SetMQDetectionFlag(AccountName(), GetName(), hacked_string.c_str(), zone->GetShortName());
				return;
			}
...
void Client::DoHPRegen() {
	...
	if (GetCharmEffectID(EQ::invslot::slotCharm) == CharmRegeneration
		&& IsItemRecastExpired(EQ::invslot::slotCharm))
	{
		hpRegenDiv -= 25;
	}
	...
	{
		SetHP(GetHP() + 1 + CalcHPRegen() + GetMaxHP() / hpRegenDiv);
	}
...
void Client::DoManaRegen() {
	if (AggroCount <= 0)
	{
		SetMana(GetMana() + 1 + CalcManaRegen() + GetMaxMana() / 10);
		CheckManaEndUpdate();
	}
}
...
void Client::DoEnduranceRegen()
{
	...
	{
		SetEndurance(GetMaxEndurance() / 2 + CalcEnduranceRegen());
	}
	else
	{
		SetEndurance(GetEndurance() + baseregen * (diff > (int64)0 ? (int64)1 : (int64)-1) + CalcEnduranceRegen());
	}
}
...
void Client::CalcRestState()
{
	...
	if (!bWasRested)
	{
		...
		a->spellid = 0;
		a->type = 0;
		a->hitType = 0; // no type, // 1 = crit, 2 = overtime, 4 = heal
