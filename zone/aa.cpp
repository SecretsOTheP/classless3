...
void Client::SendAlternateAdvancementRank(int aa_id, int level) {
	...
	if(!(ability->classes & (1 << GetClass())) && GetClass() != 17) {
		return;
	}
...
void Client::SendAlternateAdvancementStats() {
	...
	aps->unspent = m_pp.aapoints > 30000 ? 30000 : m_pp.aapoints;
...
void Client::SendAlternateAdvancementPoints() {
	...
	aa2->aapoint_assigned = m_pp.aapoints + m_pp.aapoints_spent;
	QueuePacket(outapp);
	safe_delete(outapp);
}
...
bool ZoneDatabase::LoadAlternateAdvancement(Client *c) {
	...
	c->AddAA(331, 1); // EDGE Darksign AA
	c->AddAA(32050, 1); // EDGE Remove Augment AA
	//c->AddAA(32051, 1); // EDGE Toggle PVP AA
	c->AddAA(41000, 1); // EDGE Toggle PVP AA

	c->AddAA(195, 3); // Animation Control AA

	if(c->GetLevel() >= 10)  
	{
		c->AddAA(1071, EQ::Clamp(c->GetLevel() / 10, 0, 4));  // EDGE Mnemonic Retention AA
		c->AddAA(141, EQ::Clamp(c->GetLevel() / 10, 0, 6));  // Craft Mastery AA
	}
...
bool Mob::AddAA(uint32 id, uint32 points, uint32 charges) {
	for(int i = 1; i <= points; ++i) {
		auto ability_rank = zone->GetAlternateAdvancementAbilityAndRank(id, i);
		auto ability = ability_rank.first;
		auto rank = ability_rank.second;
		...
		SetAA(rank->id, rank->current_value, charges);
	}
	return true;
}
...
bool Mob::CanUseAlternateAdvancementRank(AA::Rank *rank) {
	...
	race = race > 16 ? 1 : race;
	if(!(ability->races & (1 << (race - 1))) && GetClass() != 17) {
		return false;
	}

	auto deity = GetDeityBit();
	if(!(ability->deities & deity) && GetClass() != 17) {
		return false;
	}
...
bool Mob::CanPurchaseAlternateAdvancementRank(AA::Rank *rank, bool check_price, bool check_grant) {
	...
	if(ability->charges > 0 && current_charges > 0 && rank->id > 36) {
		return false;
	}
