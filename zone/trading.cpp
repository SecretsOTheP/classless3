				...
				if(GetGM() || (inst->IsAttuned() == false && 
					...
					if (item.IsClassBag() && item.BagSlots > 0) {
						for (int16 bslot = EQ::invbag::SLOT_BEGIN; bslot < item.BagSlots; bslot++) {
							...
							if (baginst) {
								EQ::ItemData bagitem = database.GetItem(baginst->GetID());
...
void Client::SendTraderItem(const EQ::ItemInstance* iinst, int16 Quantity) {
	...
	EQ::ItemData item = database.GetItem(iinst->GetID());
...
std::list<BazaarSearchResultsNew_Struct> Client::QueryBazaar(uint32 TraderID, uint32 Class_, uint32 Race, uint32 ItemStat, uint32 Slot, uint32 Type,
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		EQ::ItemData item = database.GetItem(atoi(row[1]));

		EQ::ItemData aug1item = database.GetItem(atoi(row[7]));
		EQ::ItemData aug2item = database.GetItem(atoi(row[8]));
		EQ::ItemData aug3item = database.GetItem(atoi(row[9]));
		EQ::ItemData aug4item = database.GetItem(atoi(row[10]));
		EQ::ItemData aug5item = database.GetItem(atoi(row[11]));
		EQ::ItemData aug6item = database.GetItem(atoi(row[12]));
...
void Client::SellToBuyer(const EQApplicationPacket *app) {
	...
	if(!item.ID || !Quantity || !Price || !QtyBuyerWants) return;
