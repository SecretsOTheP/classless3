...
void NPC::CalcBonuses()
{
	...
	//Calc pet stats for sub-NPCs after.
	CalcPetBonuses();
}
...
void Client::CalcSynergy()
{
	for (unsigned int i = 0; i < EQ::spells::SPELL_GEM_COUNT; ++i) {
		m_SYNERGY[i] = IsValidSpell(CastToClient()->m_pp.mem_spells[i]) ? GetActSpellSynergy(CastToClient()->m_pp.mem_spells[i], false) : 1.0f;
	}
}

void Mob::CalcPetBonuses()
{
...
void Client::ProcessItemCaps()
{
	...
	itembonuses.ManaRegen = std::min((int64)itembonuses.ManaRegen, CalcManaRegenCap());
...
void Client::AddFakeItemBonuses(const EQ::ItemInstance *inst, StatBonuses *newbon, bool isAug, const EQ::ItemInstance *baseinst)
{
	if (!inst || !baseinst) {
		return;
	}
...
void Client::AddItemBonuses(const EQ::ItemInstance *inst, StatBonuses *newbon, bool isAug, bool isTribute, int rec_override, bool ammo_slot_item)
{
	...
	if (!ammo_slot_item) {
		if (GetLevel() >= rec_level) {
			...
			newbon->Corrup += CalcRecommendedLevelBonus(lvl, rec_level, (item->SVCorruption));
		}
			     ...
			     bool IsAISpellEffect, uint16 effect_id, int32 se_base, int32 se_limit, int32 se_max)
{
	...
	for (i = 0; i < EFFECT_COUNT; i++)
	{
		...
		switch (effectid)
		{
			...
			case SE_DamageShield:
			{
				new_bonus->DamageShield += GetActDSDamage(spell_id, effect_value);
			...
			case SE_MeleeMitigation:
			{
				int t1 = new_bonus->MeleeMitigationEffect / 1000;
				int t2 = new_bonus->MeleeMitigationEffect % 1000;
				if (effect_value > 1000 && effect_value / 1000 > t1)
				{
					new_bonus->MeleeMitigationEffect = effect_value + t2;
				}
				else if (effect_value < 1000 && effect_value > t2)
				{
					new_bonus->MeleeMitigationEffect = t1 * 1000 + effect_value;
				}
