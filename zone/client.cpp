/*	EQ::: Everquest Server Emulator
	Copyright (C) 2001-2016 EQ:: Development Team (http://EQ::lator.org)
  ...
  tmSitting(0)
{
	...
	mercid = 0;
	mercSlot = 0;
	checkedValidAltCharacters = false;
	InitializeMercInfo();
	SetMerc(0);
	has_loaded_group = false;
...
Client::~Client() {
	...
	//if(zone && zone->GetInstanceID() > 0 && zone->maxLevelInZone == GetLevel())
	//{
	//	auto other_clients = entity_list.GetClientList();
	//	int max_level = 1;
	//	for(auto client : other_clients)
	//	{
	//		if (client.second)
	//		{
	//			if (client.second->GetLevel() > max_level)
	//				max_level = client.second->GetLevel();
	//		}
	//	}

	//	zone->maxLevelInZone = max_level;

	//	if (zone->maxLevelInZone != GetLevel())
	//	{
	//		auto npcs = entity_list.GetNPCList();
	//		for (auto npc : npcs)
	//		{
	//			if (npc.second && !npc.second->IsPetOwnerClient())
	//			{
	//				npc_scale_manager->ScaleNPC(npc.second, GetLevel());
	//			}
	//		}
	//	}
	...
	//entity_list.BuffFadeByCasterDeconstructor(this);
...
void Client::SendZoneInPackets()
{
	...
	if(GetLevel() == 1 && GetNextAvailableSpellBookSlot() == 0)
		SmartScribeSpells();
}
...
bool Client::Save(uint8 iCommitNow) {
	...
	/*std::vector<Mob*> targetPets = GetPets();
	int i = 0;
	m_petinfo.clear();
	for (auto pet : targetPets)
	{
		if (m_petinfo.find(i) == m_petinfo.end())
		{
			PetInfo info;
			memset(&info, 0, sizeof(struct PetInfo));
			m_petinfo[i] = info;
		}

		if (pet && pet->IsNPC() && pet->CastToNPC()->GetPetSpellID() && !dead) {
			NPC *petnpc = pet->CastToNPC();
			m_petinfo[i].SpellID = petnpc->CastToNPC()->GetPetSpellID();
			m_petinfo[i].HP = petnpc->GetHP();
			m_petinfo[i].Mana = petnpc->GetMana();
			petnpc->GetPetState(m_petinfo[i].Buffs, m_petinfo[i].Items, m_petinfo[i].Name);
			m_petinfo[i].petpower = petnpc->GetPetPower();
			m_petinfo[i].size = petnpc->GetSize();
		}
		else {
			memset(&m_petinfo[i], 0, sizeof(struct PetInfo));
		}
		++i;
	}
...
void Client::SetSkill(EQ::skills::SkillType skillid, uint16 value) {	
	...
	if (skillid == EQ::skills::SkillAlchemy || skillid == EQ::skills::SkillFishing || skillid == EQ::skills::SkillTailoring || skillid == EQ::skills::SkillBlacksmithing
		|| skillid == EQ::skills::SkillFletching || skillid == EQ::skills::SkillBrewing || skillid == EQ::skills::SkillBaking || skillid == EQ::skills::SkillPottery || skillid == EQ::skills::SkillJewelryMaking || skillid == EQ::skills::SkillTinkering || skillid == EQ::skills::SkillForage || skillid == EQ::skills::SkillMakePoison || skillid == EQ::skills::SkillApplyPoison)
	{
...
void Client::CheckManaEndUpdate() {
	...
	if (last_reported_mana != current_mana || last_reported_endurance != current_endurance) {
		...
		if (this->GetEndurancePercent() != last_reported_endurance_percent) {
			...
			endurance_update->cur_end = (uint32)GetEndurance();
			endurance_update->max_end = (uint32)GetMaxEndurance();
...
void Client::SendManaUpdate()
{
	...
	mana_update->cur_mana = std::min(GetMana(), (int64)2147483646);
	mana_update->max_mana = std::min(GetMaxMana(), (int64)2147483646);
	if(mana_update->cur_mana == mana_update->max_mana) { mana_update->cur_mana -= 1; }
	mana_update->spawn_id = GetID();
...
void Client::SendEnduranceUpdate()
{
	...
	endurance_update->cur_end = std::min(GetEndurance(), (int64)2147483646);
	endurance_update->max_end = std::min(GetMaxEndurance(), (int64)2147483646);
	endurance_update->spawn_id = GetID();
	QueuePacket(end_app);
	safe_delete(end_app);
...
bool Client::HasLootLockouts(uint32 npctype_id)
{
	auto entryItr = loot_lockouts.find(npctype_id);

	if (entryItr == loot_lockouts.end())
	{
		return false;
	}
	...
	if (currTime >= entry.expirydate)
		return true;

	if (entry.ip_addr == GetIP())
		return true;
...
bool Client::CheckIncreaseSkill(EQ::skills::SkillType skillid, Mob *against_who, int chancemodi) {
	...
	if(skillid == EQ::skills::SkillFishing)
	{
		maxskill = 400;
		if (skillval >= GetLevel() * 8)
		{
	...
	if (skillval < maxskill)
	{
		...
		if(Chance < 2)
			Chance = 2; // Make it always possible
...
{
	EQ::ItemData item = database.GetItem(item_id);
	if (item.ID && item.IsClassCommon()) {
...
/*	this code helps figure out the filter IDs in the packet if needed
	...
	for(r = 0; r < (sizeof(SetServerFilter_Struct)/4); r++) {
		if(*o != *n)
			LogFile->write(EQ::Log::Debug, "Filter %d changed from %d to %d", r, *o, *n);
		o++; n++;
	}
...
void Client::SetEndurance(int64 newEnd)
{
...
void Client::KeyRingList()
{
	...
	for (auto iter = keyring.begin(); iter != keyring.end(); ++iter) {
		if ((item = database.GetItem(*iter)).ID!=0) {
...
//	/*	Atk		*/	indP << "<c \"#CCFF00\">tohit: " << compute_tohit(skill) << " / " << GetTotalToHit(skill, 0) << "</c><br>" <<
	/*	Atk2	*/	indP << "- Melee Modifier: " << offense(skill) << " | Item: " << itembonuses.ATK << " (" << RuleI(Character, ItemATKCap) << ")~Used: " << (itembonuses.ATK * 1.342) << " | Spell: " << spellbonuses.ATK << "<br>" <<
//	/*	AC		*/	indP << "<c \"#CCFF00\">mitigation AC: " << GetMitigationAC() << "</c><br>" <<
	/*	AC2		*/	indP << "- Mitigation Modifier: " << compute_defense() << " / " << GetTotalDefense() << " | Spell: " << spellbonuses.AC << " | Shield: " << shield_ac << "<br>" <<
	...
	/*	Crit	*/	indP << "<c \"#CCFF00\">Crit: " << GetCriticalChanceBonus(-1) << "</c><br>" <<
	...
	/*	Mod2s	*/	mod2_field << "<br>" <<
	/*	HealAmt	*/	indP << "Heal Modifier: " << GetHealAmt() << " / " << RuleI(Character, ItemHealAmtCap) << "<br>" <<
	/*	SpellDmg*/	indP << "Spell Modifer: " << GetSpellDmg() << " / " << RuleI(Character, ItemSpellDmgCap) << "<br>" <<
	...
	if (client->Admin() >= 100) {
		client->Message(Chat::White, "  CharID: %i  EntityID: %i  OwnerID: %i  AIControlled: %i  Targetted: %i", CharacterID(), GetID(), GetOwner() ? GetOwner()->GetID() : 0, IsAIControlled(), targeted);	
	}
}
...
void Client::SendAltCurrencies() {
	if (ClientVersion() >= EQ::versions::ClientVersion::SoF) {
		...
		while(iter != zone->AlternateCurrencies.end()) {
			EQ::ItemData item = database.GetItem((*iter).item_id);
...
void Client::SendMercPersonalInfo()
{
	...
	if (mercCount > 0)
	{
			...
			mdus->MercCount = mercCount;
				mdus->MercData[i].MercID = 1;
				mdus->MercData[i].MercType = 1;
				mdus->MercData[i].MercSubType = 1;
				mdus->MercData[i].PurchaseCost = 0;
				mdus->MercData[i].UpkeepCost = 0;
				mdus->MercData[i].Status = 0;
				mdus->MercData[i].AltCurrencyCost = 0;
				mdus->MercData[i].AltCurrencyUpkeep = 0;
				mdus->MercData[i].AltCurrencyType = altCurrentType;
				mdus->MercData[i].MercUnk01 = 0;
				mdus->MercData[i].TimeLeft = GetMercInfo().MercTimerRemaining;
				mdus->MercData[i].MerchantSlot = i + 1;
				mdus->MercData[i].MercUnk02 = 1;
				mdus->MercData[i].StanceCount = 2;
				...
				mdus->MercData[i].Stances[0].StanceIndex = 0;
				mdus->MercData[i].Stances[0].Stance = EQ::constants::StanceType::stancePassive;
				mdus->MercData[i].Stances[1].StanceIndex = 1;
				mdus->MercData[i].Stances[1].Stance = EQ::constants::StanceType::stanceAggressive;
...
FACTION_VALUE Client::GetReverseFactionCon(Mob* iOther) {
	if (GetUltimateOwner() != this) {
		return GetUltimateOwner()->GetReverseFactionCon(iOther);
	}

	iOther = iOther->GetUltimateOwner();
...
FACTION_VALUE Client::GetFactionLevel(uint32 char_id, uint32 npc_id, uint32 p_race, uint32 p_class, uint32 p_deity, int32 pFaction, Mob* tnpc)
{
	...
	if (IsInvisible(tnpc))
		return FACTION_INDIFFERENT;
	if (tnpc && tnpc->GetOwner() != nullptr) // pets con amiably to owner and indiff to rest
	{
		if (tnpc->GetUltimateOwner() == this)
...
void Client::ForceMerchantWindow(uint32 entityId, uint8 type, uint32 merchant_id)
{
	...
	if (merchantNpc)
	{
		...
		{
			BulkSendMerchantInventory(merchantNpc->MerchantType, merchantNpc->GetNPCTypeID(), merchant_id == 0 ? 0 : 1);
		}
	}
}

uint8 Client::GetEdgeClass(uint8 index)
{
	if (index == 0)
	{
		return m_epp.edge_pri_class_id;
	}
	else if (index == 1)
	{
		return m_epp.edge_sec_class_id;
	}
	else if (index == 2)
	{
		return m_epp.edge_ter_class_id;
	}

	return 0;
}

bool Client::HasEdgeClass(uint8 classidx)
{

	if ((m_epp.edge_pri_class_id == classidx ||
		m_epp.edge_sec_class_id == classidx ||
		m_epp.edge_ter_class_id == classidx))
	{
		return true;
	}
...
void Client::SetEdgeClass(uint8 classidx, uint8 index)
{

	if ((m_epp.edge_pri_class_id == classidx ||
		m_epp.edge_sec_class_id == classidx ||
		m_epp.edge_ter_class_id == classidx) && classidx != 0 )
	{
		return;
	}
	...
	if (index == 0)
	{
		m_epp.edge_pri_class_id = classidx;
	}
	else if (index == 1)
	{
		m_epp.edge_sec_class_id = classidx;
...
void Client::SendFakeMovementPacket(Mob* mob)
{
	if (mob)
	{
		...
		spu->spawn_id = mob->GetID();
		spu->x_pos = FloatToEQ19(-999999.f);
		spu->y_pos = FloatToEQ19(-999999.f);
		spu->z_pos = FloatToEQ19(-999999.f);
		spu->heading = FloatToEQ12(mob->GetHeading());
...
void Client::SendCorrectStationaryMovementPacket(Mob* mob)
{
	if (mob)
	{
		EQApplicationPacket outapp(OP_ClientUpdate, sizeof(PlayerPositionUpdateServer_Struct));
		...
		memset(spu, 0x00, sizeof(PlayerPositionUpdateServer_Struct));
		spu->spawn_id = mob->GetID();
		spu->x_pos = FloatToEQ19(mob->GetX());
		spu->y_pos = FloatToEQ19(mob->GetY());
		spu->z_pos = FloatToEQ19(mob->GetZ());
		spu->heading = FloatToEQ12(mob->GetHeading());
		spu->delta_x = FloatToEQ13(0);
		spu->delta_y = FloatToEQ13(0);
		spu->delta_z = FloatToEQ13(0);
...
void Client::LevelChangeInstances()
{
	//if (zone && GetLevel() > zone->maxLevelInZone && zone->GetInstanceID() > 0)
	//{
	//	zone->maxLevelInZone = GetLevel();
	//	auto npcs = entity_list.GetNPCList();
	//	for (auto npc : npcs)
	//	{
	//		if (npc.second && !npc.second->IsPetOwnerClient())
...
void Client::SendToGuildHall()
{
	...
	if (instance_id <= 0) {
		...
		if (!database.CreateInstance(instance_id, zone_id, 1, expiration_time, 0)) {
...
void Client::SetMercCharacterID(int character_id)
{
	// Handle the dismiss here...
	if(GetMercInfo().mercid)
		DismissMerc(GetMercInfo().mercid);

	auto mercResult = validMercCharacters.find(character_id);
	if (mercResult != validMercCharacters.end())
	{
		m_epp.edge_merc_character_id = character_id;
		Merc* merc = Merc::LoadMercFromCharacterID(this, character_id);
		if (merc)
		{
			SpawnMerc(merc, true);
			merc->Save();
...
void Client::OutputAccountCharacters()
{
	...
	if (validMercCharacters.empty())
	{
		Message(Chat::System, "You must create an alternate character to utilize this feature!");
		return;
	}

	auto mercResult = validMercCharacters.find(GetMercCharacterID());


	Message(Chat::Who, "----------------------------------- ");
	std::string currentlyActiveName = "Currently Active Alternative Character Name: "; 
	currentlyActiveName += mercResult != validMercCharacters.end() ? mercResult->second.name : "None";
	...
	for (auto &mercChar : validMercCharacters)
	{
		if (mercChar.first == CharacterID())
			continue;

		std::string charLinkName = mercChar.second.name;
		charLinkName += " [";
		charLinkName += std::to_string(mercChar.second.level);
		charLinkName += " ";
		charLinkName += GetClassIDName(mercChar.second.mercClass);
		charLinkName += "]";
