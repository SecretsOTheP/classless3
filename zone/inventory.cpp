...
bool Client::SummonItem(uint32 item_id, int16 charges, uint32 aug1, uint32 aug2, uint32 aug3, uint32 aug4, uint32 aug5, uint32 aug6, bool attuned, uint16 to_slot, uint32 ornament_icon, uint32 ornament_idfile, uint32 ornament_hero_model) {
	...
	// check to make sure we are augmenting an augmentable item
		...
		LogInventory( "Player %s on account %s attempted to augment an augment or a non-common class item.\n(Item: %u, Aug1: %u, Aug2: %u, Aug3: %u, Aug4: %u, Aug5: %u, Aug5: %u)\n",
	...
	*/
	...
	for (int iter = EQ::invaug::SOCKET_BEGIN; iter <= EQ::invaug::SOCKET_END; ++iter) {
		EQ::ItemData augtest = database.GetItem(augments[iter]);

		if(augtest.ID == 0) {
			if(augments[iter]) {
				Message(Chat::Red, "Augment %u (Aug%i) does not exist.", augments[iter], iter + 1);
				LogInventory( "Player %s on account %s attempted to create an augment (Aug%i) with an invalid id.\n(Item: %u, Aug1: %u, Aug2: %u, Aug3: %u, Aug4: %u, Aug5: %u, Aug6: %u)\n",
			...
			// check that augment is an actual augment
				...
				LogInventory( "Player %s on account %s attempted to use a non-augment item (Aug%i) as an augment.\n(Item: %u, Aug1: %u, Aug2: %u, Aug3: %u, Aug4: %u, Aug5: %u, Aug6: %u)\n",
			...
			*/
			...
			if(enforcewear) {
				...
				if(item.AugSlotVisible[iter] == 0) {
					...
					LogInventory( "Player %s on account %s attempted to augment an unevolved item with augment type (Aug%i).\n(Item: %u, Aug1: %u, Aug2: %u, Aug3: %u, Aug4: %u, Aug5: %u, Aug6: %u)\n",
						GetName(), account_name, (iter + 1), item.ID, aug1, aug2, aug3, aug4, aug5, aug6);
			...
			if(enforcerestr) {
				...
				if(restrictfail) {
					...
					LogInventory( "Player %s on account %s attempted to augment an item with a restricted augment (Aug%i).\n(Item: %u, Aug1: %u, Aug2: %u, Aug3: %u, Aug4: %u, Aug5: %u, Aug6: %u)\n",
						GetName(), account_name, (iter + 1), item.ID, aug1, aug2, aug3, aug4, aug5, aug6);
			...
			if(enforceusable) {
				...
				if(item.Classes && !(classes &= augtest.Classes)) {
					...
					LogInventory("Player %s on account %s attempted to create an item unusable by any class.\n(Item: %u, Aug1: %u, Aug2: %u, Aug3: %u, Aug4: %u, Aug5: %u, Aug6: %u)\n",
						GetName(), account_name, item.ID, aug1, aug2, aug3, aug4, aug5, aug6);
				...
				if(item.Races && !(races &= augtest.Races)) {
					...
					LogInventory("Player %s on account %s attempted to create an item unusable by any race.\n(Item: %u, Aug1: %u, Aug2: %u, Aug3: %u, Aug4: %u, Aug5: %u, Aug6: %u)\n",
						GetName(), account_name, item.ID, aug1, aug2, aug3, aug4, aug5, aug6);
				...
				if(item.Slots && !(slots &= augtest.Slots)) {
					...
					LogInventory("Player %s on account %s attempted to create an item unusable in any slot.\n(Item: %u, Aug1: %u, Aug2: %u, Aug3: %u, Aug4: %u, Aug5: %u, Aug6: %u)\n",
						GetName(), account_name, item.ID, aug1, aug2, aug3, aug4, aug5, aug6);
	...
	if (enforceusable && (to_slot >= EQ::invslot::EQUIPMENT_BEGIN && to_slot <= EQ::invslot::EQUIPMENT_END)) {
		...
		if(!(slots & ((uint32)1 << slottest))) {
			...
			LogInventory("Player %s on account %s attempted to equip an item unusable in slot %u - moved to cursor.\n(Item: %u, Aug1: %u, Aug2: %u, Aug3: %u, Aug4: %u, Aug5: %u, Aug6: %u)\n",
...
void Client::DropItemQS(EQ::ItemInstance* inst, bool pickup) {
	if (RuleB(QueryServ, PlayerDropItems)) {
		...
		if (inst) {
			...
			detail->aug_1 = inst->GetAugmentItemID(0);
			detail->aug_2 = inst->GetAugmentItemID(1);
			detail->aug_3 = inst->GetAugmentItemID(2);
			detail->aug_4 = inst->GetAugmentItemID(3);
			detail->aug_5 = inst->GetAugmentItemID(4);
			...
			if (inst->IsClassBag()) {
				for (uint8 sub_slot = EQ::invbag::SLOT_BEGIN; (sub_slot <= EQ::invbag::SLOT_END); ++sub_slot) { // this is to catch ALL items
					...
					if (bag_inst) {
						...
						detail->aug_1 = bag_inst->GetAugmentItemID(0);
						detail->aug_2 = bag_inst->GetAugmentItemID(1);
						detail->aug_3 = bag_inst->GetAugmentItemID(2);
						detail->aug_4 = bag_inst->GetAugmentItemID(3);
						detail->aug_5 = bag_inst->GetAugmentItemID(4);
						event_details.push_back(detail);
					}
				}
			}
...
void Client::DeleteItemInInventory(int16 slot_id, int8 quantity, bool client_update, bool update_db) {
	...
	if(RuleB(QueryServ, PlayerLogDeletes)) {
		...
		qsaudit->items[parent_offset].aug_1		= m_inv[slot_id]->GetAugmentItemID(0);
		qsaudit->items[parent_offset].aug_2		= m_inv[slot_id]->GetAugmentItemID(1);
		qsaudit->items[parent_offset].aug_3		= m_inv[slot_id]->GetAugmentItemID(2);
		qsaudit->items[parent_offset].aug_4		= m_inv[slot_id]->GetAugmentItemID(3);
		qsaudit->items[parent_offset].aug_5		= m_inv[slot_id]->GetAugmentItemID(4);
		...
		if (m_inv[slot_id]->IsClassBag()) {
			for (uint8 bag_idx = EQ::invbag::SLOT_BEGIN; bag_idx < m_inv[slot_id]->GetItem()->BagSlots; bag_idx++) {
				...
				if(bagitem) {
					...
					qsaudit->items[parent_offset].aug_1			= bagitem->GetAugmentItemID(0);
					qsaudit->items[parent_offset].aug_2			= bagitem->GetAugmentItemID(1);
					qsaudit->items[parent_offset].aug_3			= bagitem->GetAugmentItemID(2);
					qsaudit->items[parent_offset].aug_4			= bagitem->GetAugmentItemID(3);
					qsaudit->items[parent_offset].aug_5			= bagitem->GetAugmentItemID(4);
				}
			}
		}
...
bool Client::TryAutoVendor(uint32 new_item_id, int32 count, uint8 type) {
	...
	EQ::ItemData item_data = database.GetItem(item_id);
	for (i = EQ::invslot::GENERAL_BEGIN; i <= EQ::invslot::GENERAL_END; i++) {
		if (((uint64)1 << i) & GetInv().GetLookup()->PossessionsBitmask == 0)
			continue;
		...
		if(bag_inst != nullptr){
			...
			if(bag_inst->GetItem()->EdgeBagType == EQ::item::EdgeBagType::EdgeBagVoidHole) {
				for (uint8 j = 0; j < slots; j++) {
					...
					if(tmp_inst && tmp_inst->GetItem()->ID == item_id)
					{
						...
						int ourCopper = value % 100;
						int ourSilver = value / 100 % 100;
						int ourGold = value / 10000 % 100;
						int ourPlat = value / 1000000;
...
bool Client::TryForceStack(uint32 new_item_id, int32 count, uint8 type) {
	...
	for (i = EQ::invslot::GENERAL_BEGIN; i <= EQ::invslot::GENERAL_END; i++) {
		...
		if(bag_inst != nullptr){
			...
			if(bag_inst->GetItem()->EdgeBagType == EQ::item::EdgeBagType::EdgeBagStickySack && (new_item_id < 199900 || new_item_id > 200000)) {
				for (uint8 j = 0; j < slots; j++) {
					uint16 slotid = EQ::InventoryProfile::CalcSlotId(i, j);
					EQ::ItemInstance* tmp_inst = m_inv.GetItem(slotid);
					if(tmp_inst == nullptr )
					{
						...
						database.SaveInventory(this->CharacterID(), new_inst, slotid);
			...
			if(bag_inst->GetItem()->EdgeBagType == EQ::item::EdgeBagKnickknackPack && item_data.Tradeskills && (new_item_id < 199900 || new_item_id > 200000)) {
				for (uint8 j = 0; j < slots; j++) {
					uint16 slotid = EQ::InventoryProfile::CalcSlotId(i, j);
					EQ::ItemInstance* tmp_inst = m_inv.GetItem(slotid);
					if(tmp_inst == nullptr)
			...
			if(bag_inst->GetItem()->EdgeBagType == EQ::item::EdgeBagType::EdgeBagRunePack && item_data.Tradeskills && new_item_id >= 199900 && new_item_id < 200000) {
				for (uint8 j = 0; j < slots; j++) {
					uint16 slotid = EQ::InventoryProfile::CalcSlotId(i, j);
					EQ::ItemInstance* tmp_inst = m_inv.GetItem(slotid);
					if(tmp_inst == nullptr)
			...
			if(bag_inst->GetItem()->EdgeBagType == 11 && (new_item_id < 199900 || new_item_id > 200000)) {
				for (uint8 j = 0; j < slots; j++) {
					uint16 slotid = EQ::InventoryProfile::CalcSlotId(i, j);
					EQ::ItemInstance* tmp_inst = m_inv.GetItem(slotid);
					if(tmp_inst == nullptr)
...
{
	// #1: Try to auto equip
	if (try_worn && inst.IsEquipable(GetBaseRace(), GetClass(), true) && inst.GetItem()->ReqLevel <= level && (!inst.GetItem()->Attuneable || inst.IsAttuned()) && inst.GetItem()->ItemType != EQ::item::ItemTypeAugmentation) {
...
bool Client::SwapItem(MoveItem_Struct* move_in) {
	...
	if(IsBankSlot(src_slot_id) || IsBankSlot(dst_slot_id) || IsBankSlot(src_slot_check) || IsBankSlot(dst_slot_check)) {
		...
		if(zone && zone->GetZoneID() != RuleI(World, LoadZoneID) && (!banker || distance > USE_NPC_RANGE2))
		{
	...
	if (move_in->number_in_stack > 0) {
		...
		if (src_inst) {
			CheckHasInstrument(src_slot_id, src_inst);
		}
		if (dst_inst)
		{
			CheckHasInstrument(dst_slot_id, dst_inst);
		}
	...
	//Special edge logic for sending recast timers on equip for charms/power sources

	EQ::ItemInstance *itm = CastToClient()->GetInv().GetItem(dst_slot_id);
	if (itm && itm->GetItem()->Click.Effect > 0 && itm->GetItem()->RecastDelay > 0 && itm->GetItem()->MaxCharges <= 0 && dst_slot_id < EQ::invslot::slotCursor) 
	{
		auto recast_type = itm->GetItem()->RecastType;
		auto recast_delay = 60;
		GetPTimers().Start((pTimerItemStart + recast_type), recast_delay);
		database.UpdateItemRecastTimestamps(
			CharacterID(), recast_type,
			GetPTimers().Get(pTimerItemStart + recast_type)->GetReadyTimestamp());
		if(casting_spell_id)
			InterruptSpell(itm->GetItem()->Click.Effect);
		...
		ird->unknown008 = recast_type;
		QueuePacket(outapp);
		safe_delete(outapp);
		if (dst_slot_id == EQ::invslot::slotCharm)
			Message(15, "You begin to focus the mystical power of your Charm.");
		if (dst_slot_id == EQ::invslot::slotPowerSource)
			Message(15, "You begin to focus the mystical power of your Power Source.");
		src_inst->SetRecastTimestamp(GetPTimers().Get(pTimerItemStart + recast_type)->GetReadyTimestamp());
	}
...
void Client::SwapItemResync(MoveItem_Struct* move_slots) {
	...
	if (move_slots->from_slot >= EQ::invslot::EQUIPMENT_BEGIN && move_slots->from_slot <= EQ::invbag::CURSOR_BAG_END) {
		...
		if (IsValidSlot(resync_slot) && resync_slot != INVALID_INDEX) {
			...
			EQ::ItemData token_struct = database.GetItem(22292); // 'Copper Coin'
...
void Client::DisenchantSummonedBags(bool client_update)
{
	...
	while (!m_inv.CursorEmpty()) {
		...
		if (!inst) { break; }
		if (!IsSummonedBagID(inst->GetID())){ break; }
...
uint32 Client::GetEquipmentColor(uint8 material_slot) const
{
	...
	EQ::ItemData item = database.GetItem(GetEquippedItemFromTextureSlot(material_slot));
	if(item.ID != 0)
...
void Client::CheckWornClickies(int16 SlotID, EQ::ItemInstance* inst)
{
	if (!inst || SlotID == -1)
		return;

	auto spell_id = inst->GetItem()->Click.Effect;

	auto click_type = inst->GetItem()->Click.Type;

	if (spell_id > 0 && click_type == 4)
	{
		entity_list.BuffFadeBySpellIDAndCaster(spell_id, GetID());

		std::list<Mob*> petsToRemove;
		...
		for (Mob* pet : petlist)
		{
			...
			if (pet->CastToNPC()->GetPetSpellID() == spell_id)
				petsToRemove.push_back(pet);
		}

		for (auto petptr : petsToRemove)
		{
			RemovePet(petptr);
		}
	}
...
void Client::SendClickyRecast(int16 SlotID, EQ::ItemInstance* inst)
{

	if (SlotID >= EQ::invslot::slotCursor)
		return;

	if (inst && inst->GetItem()->Click.Effect > 0 && inst->GetItem()->RecastDelay > 0 && inst->GetItem()->MaxCharges <= 0)
	{
		auto recast_type = inst->GetItem()->RecastType;
		auto recast_delay = 60;
		GetPTimers().Start((pTimerItemStart + recast_type), recast_delay);
		database.UpdateItemRecastTimestamps(
			CharacterID(), recast_type,
			GetPTimers().Get(pTimerItemStart + recast_type)->GetReadyTimestamp());
		if (casting_spell_id)
			InterruptSpell(inst->GetItem()->Click.Effect);
		auto outapp = new EQApplicationPacket(OP_ItemRecastDelay, sizeof(ItemRecastDelay_Struct));
		ItemRecastDelay_Struct *ird = (ItemRecastDelay_Struct *)outapp->pBuffer;
		ird->recast_delay = recast_delay;
		ird->recast_type = recast_type;
		...
		QueuePacket(outapp);
		safe_delete(outapp);
		if (SlotID == EQ::invslot::slotCharm)
			Message(15, "You begin to focus the mystical power of your Charm.");
		if (SlotID == EQ::invslot::slotPowerSource)
			Message(15, "You begin to focus the mystical power of your Power Source.");
		inst->SetRecastTimestamp(GetPTimers().Get(pTimerItemStart + recast_type)->GetReadyTimestamp());
	}
}

void Client::CheckHasInstrument(int16 SlotID, EQ::ItemInstance* inst)
{
	if (!inst || SlotID == -1)
		return;

	CheckWornClickies(SlotID, inst);
	...
	if (spellID > 0)
	{
		...
		if (IsInstrument && SlotID == EQ::invslot::slotSecondary || IsInstrument && SlotID == EQ::invslot::slotSecondary || IsInstrument && SlotID == EQ::invslot::slotPrimary || IsInstrument && SlotID == EQ::invslot::slotPrimary)
			_StopSong();
		if (IsInstrument)
		{
			entity_list.BuffFadeBySpellIDAndCaster(spellID, this->GetID());
		}
	}
	return;
}
			...
			if (slot != INVALID_INDEX) {
				...
				if (((BandolierSlot == bandolierAmmo) || (BandolierSlot == bandolierRange)) &&
					...
					if (Charges > 1) {
						...
						CheckHasInstrument(slot, BandolierItems[BandolierSlot]);
						SendClickyRecast(slot, BandolierItems[BandolierSlot]);
					}
				...
				if (slot == INVALID_INDEX) {
					...
					if (InvItem) {
						...
						LogInventory( "returning item %s in weapon slot %i to inventory",
							InvItem->GetItem()->Name, WeaponSlot);
						LogInventory( "returning item %s in weapon slot %i to inventory", InvItem->GetItem()->Name, WeaponSlot);
		...
		if(m_pp.bandoliers[bss->Number].Items[BandolierSlot].ID) {
			...
			if(BandolierItems[BandolierSlot]) {
				...
				if(InvItem) {
					...
					{
						SendClickyRecast(slot, InvItem);
						CheckHasInstrument(slot, InvItem);
					}
					safe_delete(InvItem);
				}
			}
