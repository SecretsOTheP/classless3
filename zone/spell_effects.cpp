...
bool Mob::SpellEffect(Mob* caster, uint16 spell_id, float partial, int level_override, bool isproc)
{

	int64 effect_value;
	int caster_level, buffslot, effect, i;
...
#define _EDLEN	200
	...
	for (int i = 0; i < buff_count; i++)
	{
		...
		if (buffs[i].spellid == spell_id && IsEffectInSpell(buffs[i].spellid, SE_Toggle))
		{
	...
	//edge change: always use caster, we don't care what it was across zones tbh
	...
	{
		uint32 halved_mod = instrument_mod - 10;
		halved_mod /= 2;
		instrument_mod -= halved_mod;
	}
		...
		switch(effect)
		{
			case SE_CurrentHP:	// nukes, heals; also regen/dot if a buff
			{
				...
				if(dmg < 0)
				{
					...
					if(caster) {
						dmg = caster->GetActSpellDamage(spell_id, dmg, this, isproc);
					...
					if (!PassCastRestriction(false, spells[spell_id].base2[i], false))
						break;
					if(caster)
						dmg = caster->GetActSpellHealing(spell_id, dmg, this, isproc);
			...
			case SE_PercentalHeal:
			{
...
#endif
				int64 val = GetMaxHP() * spell.base[i] / 100;
				...
				if (val < 0) {
					...
					//if (caster)
					//	val = caster->GetActSpellDamage(spell_id, val, this, isproc);
				...
				else
				{
					...
					//if(caster)
					//	val = caster->GetActSpellHealing(spell_id, val, this, isproc);
				}
			...
			case SE_CurrentMana:
			{
				...
				if(IsManaTapSpell(spell_id)) {
					if(GetCasterClass() != 'N') {
						...
						if (caster)
							caster->SetEndurance(caster->GetEndurance() + std::abs(effect_value));
...
//					break;
//				}
//				else if (!caster->GetPets().empty() && caster->IsClient() && !caster->CastToClient()->GetGM()) {
...
//			}
			...
			case SE_Fear:
			{
				...
				if(IsClient())
				{
					if(buffs[buffslot].ticsremaining > 3)
						buffs[buffslot].ticsremaining = 3;
				}
			...
			case SE_Gate:
			{
				...
				if(!spellbonuses.AntiGate){
					...
					if(zone->random.Roll((int32)effect_value))
			...
			case SE_CancelMagic:
			{
				...
				if (GetUltimateOwner()->IsClient() && caster && caster->GetUltimateOwner()->IsClient() && caster->GetUltimateOwner() != GetUltimateOwner())
				{
					auto r1 = GetUltimateOwner()->CastToClient()->GetRaid();
					auto r2 = caster->GetUltimateOwner()->CastToClient()->GetRaid();					
					auto g1 = GetUltimateOwner()->CastToClient()->GetGroup();
					auto g2 = caster->GetUltimateOwner()->CastToClient()->GetGroup();
			...
			case SE_DispelBeneficial:
			{
				...
				if (GetUltimateOwner()->IsClient() && caster && caster->GetUltimateOwner()->IsClient())
				{
					auto r1 = GetUltimateOwner()->CastToClient()->GetRaid();
					auto r2 = caster->GetUltimateOwner()->CastToClient()->GetRaid();
					auto g1 = GetUltimateOwner()->CastToClient()->GetGroup();
			...
			case SE_SummonItem:
			{
				EQ::ItemData item = database.GetItem(spell.base[i]);
#ifdef SPELL_EFFECT_SPAM
			...
			case SE_Familiar:
			{
				...
				for (Mob* pet : petlist)
				{
					if (pet->CastToNPC()->GetPetSpellID() == spell_id && spell_id != 43300)
						returnearly = true;
					else if (pet->CastToNPC()->GetPetSpellID() == 43300)
						pet_spell_counts++;
				}

				if (spell_id == 43300 && pet_spell_counts >= 2)
					returnearly = true;
				...
				if (returnearly)
				{
					if (IsClient())
					{
						Message(Chat::Red, "You have too many pets! Cancelling pet summon spell.");
					}
					break;
				}
				else
			...
			case SE_SpinTarget:
			{
				...
				else
				{
					...
					if(IsStunned())
					{
						if(zone->random.Real(0.0, 100.0) > 25.0) // 75% chance to resist while already stunned
						{
							MessageString(Chat::Stun, SHAKE_OFF_STUN);
							break;
						}
					}
					if(effect_value > 2000) // standard 2 second stun duration cap
					{
						effect_value = 2000;
					}
			...
			case SE_ReclaimPet:
			{
				...
				if
					(
						...
						caster == GetOwner() && // and it's my master
						...
						)
				{
					...
					caster->SetEndurance(caster->GetEndurance() + pet_ActSpellCost);

					if (caster->IsClient())
					{
						caster->CastToClient()->RemoveAllPets();
					}
					break;
				}
				break;
			...
			case SE_CallPet:
			{
				...
				{
						pet->CastToNPC()->GMMove(GetX(), GetY(), GetZ(), GetHeading(), true, false);
				}
				break;
			}
			...
			case SE_SkillAttack:
			{
				...
				*/
				...
				if(caster->IsClient())
				{
					auto Hands = caster->CastToClient()->GetInv().GetItem(EQ::invslot::slotHands);
					if(Hands)
					{
						switch(Hands->GetID())
						{
				...
				case EQ::skills::SkillThrowing:
					caster->DoThrowingAttackDmg(this, nullptr, nullptr, base, spells[spell_id].base2[i], focus,  ReuseTime);
					break;
				case EQ::skills::SkillArchery:
					caster->DoArcheryAttackDmg(this, nullptr, nullptr, base, spells[spell_id].base2[i],focus,  ReuseTime);
					break;
				default:
					caster->DoMeleeSkillAttackDmg(this, base, spells[spell_id].skill, spells[spell_id].base2[i], focus, false, ReuseTime);
					break;
				}
				break;
			}
			...
			case SE_BardAEDot:
			{
				...
				if(dmg < 0)
				{
					...
					if(caster)
						dmg = caster->GetActSpellDamage(spell_id, dmg, nullptr, isproc);
			...
			case SE_Leap:
			{
				...
				if (caster && caster->GetTarget()) {
					...
					if ((caster->CheckLosFN(caster->GetTarget())))
					{
						...
						if (PlotPositionAroundTarget(caster, newX, newY, newZ, true))
						{
							if (caster->IsClient())
								caster->CastToClient()->MovePC(zone->GetZoneID(), zone->GetInstanceID(), newX, newY, newZ, GetReciprocalHeading(GetPosition()));
							else
								caster->GMMove(my_x, my_y, new_ground, caster->GetHeading(), true, false);
						}
					}
				}
				break;
			...
			case SE_PositionBehind:
			{
				...
				if (caster && caster->GetTarget()) {
					...
					if ((caster->CheckLosFN(caster->GetTarget())))
					{
						...
						if (PlotPositionOnArcBehindTarget(caster, newX, newY, newZ, 5.0f))
						{
							if (caster->IsClient())
								caster->CastToClient()->MovePC(zone->GetZoneID(), zone->GetInstanceID(), newX, newY, newZ, GetReciprocalHeading(GetPosition()));
							else
...
*/
			...
			case SE_ManaBurn:
			{
				...
				if (caster){
					...
					if(IsDetrimentalSpell(spell_id)) {
						dmg = -dmg;
						Damage(caster, dmg * mitigation(), spell_id, spell.skill, false, buffslot, false);
					} else {
						HealDamage(dmg * mitigation(), caster);
					}
				}
			...
			{
				if (IsNPC()){
					int64 new_hate = CastToNPC()->GetHateAmount(caster) * (100 + spell.base[i]) / 100;
					if (new_hate <= 0)
						new_hate = 1;
			...
			case SE_SmartHeal: {
				...
				int64 value = spell.base[i];
				uint64 heal_amt = caster->GetActSpellHealing(spell_id, value, this, isproc);
			...
			case SE_SwapPosition: {
				...
				caster->GMMove(thisPos.x, thisPos.y, thisPos.z, thisPos.w, true, false);
				GMMove(casterPos.x, casterPos.y, casterPos.z, casterPos.w, true, false);
			...
			case SE_SumClone:
			{
				...
				if (IsNPC())
				{	
					if (CastToNPC()->GetNPCScaleTier() > 1)
					{
						if (caster->IsClient())
						{
				...
				std::list<uint32> pet_spell_counts;
				for (Mob* pet : caster->petlist)
				{
					if (pet->CastToNPC()->GetPetSpellID() == spell_id && spell_id != 2813)
					{
				...
				{
					caster->MakePoweredPCPet(spell_id, this->CastToClient(), 0, "Illusionary_Ally", GetBaseSize());
				}
				else if (IsNPC())
					caster->MakePoweredNPCPet(spell_id, CastToNPC()->npctype_id, 0, "Illusionary_Ally", GetBaseSize());
				else
					break;
				// TODO: we need to sync the states for these clients ...
				// Will fix buttons for now
			...
			case SE_PetsMoveToTarget:
			{
				...
				for (Mob* pet : caster->petlist)
				{
						pet->GMMove(GetX(), GetY(), GetZ(), GetHeading(), true, false);
						pet->WipeHateList();
			...
			case SE_PetSkillAttack:
			{
				...
				for (auto pet : caster->petlist)
				{
					iterator_list.push_back(pet);
				}

				for (auto petiter : iterator_list)
				{
					switch (spells[spell_id].skill) {
					case EQ::skills::SkillThrowing:
						petiter->DoThrowingAttackDmg(this, nullptr, nullptr, spells[spell_id].base[i], spells[spell_id].base2[i]);
						break;
					case EQ::skills::SkillArchery:
						petiter->DoArcheryAttackDmg(this, nullptr, nullptr, spells[spell_id].base[i], spells[spell_id].base2[i]);
						break;
					default:
						petiter->DoMeleeSkillAttackDmg(this, spells[spell_id].base[i], spells[spell_id].skill, spells[spell_id].base2[i]);
						break;
					}
				}
			...
			case SE_PetSpellAttack:
			{
				...
				for (auto pet : iterator_list)
				{
					uint32 spellidToCast = spells[spell_id].base[i];
					if (pet->IsCasting())
					{
						pet->InterruptSpell();
					}
			...
			case SE_Vacuum:
			{
				...
				GMMove(caster->GetTarget()->GetX(), caster->GetTarget()->GetY(), caster->GetTarget()->GetZ(), caster->GetHeading(), true, false);
					...
					if ((caster->CheckLosFN(caster->GetTarget())))
					{
						...
						if (caster->PlotPositionAroundTarget(this, newX, newY, newZ, true))
						{
							if (IsClient())
								...
								GMMove(newX, newY, newZ, GetHeading(), true, false);
						}
					}
				}
			...
			case SE_AugmentItem:
			{
				...
				std::map<uint32, const EQ::ItemInstance*> instToDestroy;
				std::vector<uint32> aug_to_grant;
				bool success = false;
				EQ::InventoryProfile& user_inv = caster->CastToClient()->GetInv();
				const EQ::ItemInstance* tmpInst = user_inv.GetItem(EQ::invslot::slotCursor);
				if (!tmpInst)
				{
					caster->CastToClient()->Message(4, "You cannot remove an augment from nothing...");
					break;
				}

				EQ::ItemData item = database.GetItem(tmpInst->GetItem()->ID);
				if (!item.ID)
					break;

				instToDestroy[EQ::invslot::slotCursor] = tmpInst;

				for (auto instance : instToDestroy)
				{
					std::vector<uint32> aug_list;
					for (int i = EQ::invaug::SOCKET_BEGIN; i <= EQ::invaug::SOCKET_END; ++i)
					{
						if (!instance.second->GetAugment(i))
							continue;
						aug_list.push_back(instance.second->GetAugmentItemID(i));
					}

					if (aug_list.size() > 0)
						aug_to_grant.push_back(aug_list[zone->random.Int(0, aug_list.size() - 1)]);
				}

				for (auto instance : instToDestroy)
				{
					//if (instance.second) {
					//	caster->CastToClient()->DeleteItemInInventory(EQ::invslot::slotCursor, 0, true);
					//}
					while (!caster->CastToClient()->GetInv().CursorEmpty()) { 
						caster->CastToClient()->DeleteItemInInventory(EQ::invslot::slotCursor, 0, true);
					}
				}
				for (auto aug : aug_to_grant)
				{
					caster->CastToClient()->SummonItem(aug);
					success = true;
				}

				if (success)
					caster->CastToClient()->Message(4, "You have created... something.");
				else
				{
					caster->CastToClient()->Message(4, "You cannot remove an augment from whatever is on your cursor!");
				}
				break;
			}

			case SE_AugmentHighest:
			{
				if (!caster || !caster->IsClient())
					break;

				if (caster->CastToClient()->m_pp.aapoints < 10)
				{
					caster->Message(Chat::SpellFailure, "You cannot use this ability; it requires 10 AA points unspent.");
					break;
				}
				...
				for (auto instance : instToDestroy)
				{
					...
					if (aug_list.size() > 0)
					{
						...
						for (auto aug : aug_list)
						{
							uint32 valueOfAug = aug - 10000000;
							int value = valueOfAug / 10;
							if (aug % 10 < 3)
							{
								value = value / 10;
							}
				...
				if (success)
				{
					caster->CastToClient()->m_pp.aapoints -= 10;
					caster->CastToClient()->Message(4, "You have created... something.");
					caster->CastToClient()->SendAlternateAdvancementPoints();
					CalcBonuses();
					caster->CastToClient()->SendAlternateAdvancementStats();
					Save();
				}
				else
				{
			...
			case SE_TogglePvP:
			{
				...
				caster->CastToClient()->SetPVP(!caster->CastToClient()->GetPVP());

				if(zone && zone->GetInstanceID() > 0)
				{
					caster->CastToClient()->Message(4, "The abrupt change in morality proves lethal!");
					caster->Kill();
				}
				else
				{
			...
			case SE_FactionReset:
			{
				if (!caster || !caster->IsClient())
					break;
				database.RemoveFactions(caster->CastToClient());
				database.LoadCharacterFactionValues(caster->CastToClient()->CharacterID(), caster->CastToClient()->factionvalues);
				
				caster->Message(Chat::BrightBlue, "Your enemies and friends have forgotten you!");
				break;
			}
			...
			case SE_RaceChange:
			{
				if (!caster || !caster->IsClient())
					break;
				caster->CastToClient()->SetBaseRace(spell.base[i]);
			...
			case SE_ItemGraphic:
			{
				...
				std::string graphicID = std::string("IT") + itoa(spell.base[i]);
			...
			case SE_SummonServerGroup:
			{
				...
				auto groupNames = caster->GetGroup()->GetClientNames();

				for (auto gMem : caster->GetGroup()->GetClientNames())
				{
			      ...
			      int ticsremaining, uint16 caster_id)
{
	...
	int64 formula, base, max, effect_value;
	...
	if (EQ::skills::IsBardInstrumentSkill(spells[spell_id].skill) &&
		...
		int64 oval = effect_value;
		int64 mod = ApplySpellEffectiveness(spell_id, instrument_mod, true, caster_id);
...
int64 Mob::CalcSpellEffectValue_formula(int formula, int64 base, int64 max, int caster_level, uint16 spell_id, int ticsremaining)
{
/*
...
*/
	...
	int64 result = 0, updownsign = 1, ubase = base;
	if(ubase < 0)
		ubase = 0 - ubase;
...
void Mob::DoBuffTic(const Buffs_Struct &buff, int slot, Mob *caster)
{
	...
	if (!caster || caster->GetHP() <= 0) // && IsBeneficialSpell(buff.spellid))
	{
		BuffFadeBySpellID(buff.spellid);
		return;
	}
	...
	if (IsNPC()) {
		...
		// if buff slot, use instrument mod there, otherwise calc it
	//edge change: always use caster, we don't care what it was across zones tbh
	uint32 instrument_mod = /*buffslot > -1 ? buffs[buffslot].instrument_mod : */ caster ? caster->GetInstrumentMod(buff.spellid) : 10;
	if (instrument_mod > 10)
	{
		uint32 halved_mod = instrument_mod - 10;
		halved_mod /= 2;
	...
	for (int i = 0; i < EFFECT_COUNT; i++) {
		...
		case SE_CurrentHP: {
			if (!PassCastRestriction(false, spells[buff.spellid].base2[i], true))
				break;
			effect_value = CalcSpellEffectValue(buff.spellid, i, buff.casterlevel, instrument_mod,
			...
			if (caster && effect_value < 0) {
				...
				if(caster)
					effect_value = caster->GetActHoTHealing(buff.spellid, effect_value, this);
		...
		case SE_HealOverTime: {
			effect_value = CalcSpellEffectValue(buff.spellid, i, buff.casterlevel, instrument_mod);
			if ((caster && caster->IsNPC() && !caster->IsMerc()) || (caster && caster->IsCorpse()))
				effect_value = 5;
		...
		case SE_SmartHeal: {
			...
			int64 value = spell.base[i];
			uint64 heal_amt = caster->GetActHoTHealing(buff.spellid, value, this);
		...
		case SE_BardAEDot: {
			effect_value =
			    CalcSpellEffectValue(buff.spellid, i, buff.casterlevel, instrument_mod, caster);
			...
			if (effect_value < 0) {
				...
				// healing spell...
				if (caster) {
					caster->GetActHoTHealing(buff.spellid, effect_value, this);
				}
		...
		case SE_AddHateOverTimePct: {
			if (IsNPC()) {
				uint64 new_hate = CastToNPC()->GetHateAmount(caster) * (100 + spell.base[i]) / 100;
				if (new_hate <= 0)
					new_hate = 1;
	 ...
	 */
	if (degenerating_effects || bSendMovement)
		CalcBonuses();
...
void Mob::BuffFadeBySlot(int slot, bool iRecalcBonuses)
{
	...
	if (iRecalcBonuses || bSendMovement)
		CalcBonuses();
...
int32 NPC::GetFocusEffect(focusType type, int32 spell_id) {
	...
	if (RuleB(Spells, NPC_UseFocusFromItems) && itembonuses.FocusEffects[type]){
		...
		EQ::ItemData TempItem;
		EQ::ItemData UsedItem;
		int32 UsedFocusID = 0;
		int32 Total = 0;
		int32 focus_max = 0;
		...
		for (int i = EQ::invslot::EQUIPMENT_BEGIN; i <= EQ::invslot::EQUIPMENT_END; i++){
			EQ::ItemData cur = database.GetItem(equipment[i]);
			...
			if (TempItem.ID && TempItem.Focus.Effect > 0 && TempItem.Focus.Effect != SPELL_UNKNOWN) {
				if(rand_effectiveness) {
					focus_max = CalcFocusEffect(type, TempItem.Focus.Effect, spell_id, true);
					if (focus_max > 0 && focus_max_real >= 0 && focus_max > focus_max_real) {
						focus_max_real = focus_max;
		...
		if(UsedItem.ID && rand_effectiveness && focus_max_real != 0)
			realTotal = CalcFocusEffect(type, UsedFocusID, spell_id);
	}
...
int64 Mob::ApplySpellEffectiveness(int16 spell_id, int64 value, bool IsBard, uint16 caster_id) {
	return value;
		...
		if (spells[spellid].effectid[i] == SE_ResourceTap) {
			...
			if (damage) {
				...
				if (spells[spellid].base2[i] == 0) { // HP Tap
					if (damage > 0)
						HealDamage(damage * mitigation());
					else
						Damage(this, -damage * mitigation(), 0, EQ::skills::SkillEvocation, false);
				}
