	  ...
	  m_GuardPointSaved(0, 0, 0, 0)
{
	...
	memset(&NPCTypedata_copy, 0, sizeof(NPCType));
	if(npc_type_data)
			memcpy(&NPCTypedata_copy, npc_type_data, sizeof(NPCType));
	...
	if (zone && zone->GetZoneID() == RuleI(World, LoadZoneID) && npc_type_data->merchanttype > 0)
		merchant_open = true;
	...
	scale_tier = npc_type_data->scale_tier;
	scale_difficulty = npc_type_data->scale_difficulty;
	...
	ignore_scaling = npc_type_data->ignore_scaling;
	m_targetable   = !npc_type_data->untargetable;

	SetHP(max_hp);
	if(!IsTypePet()) //pets should be scaled through their masters' stats, i think
...
int64 NPC::CalcEnduranceRegen(bool bCombat)
{
	int64 base = 0;
...
bool NPC::Process()
{
	...
	if (mob_close_scan_timer.Check()) {
		...
		if (moving) {
			...
			mob_close_scan_timer.Start(RandomTimer(3000, 6000));
		}
		else {
			mob_close_scan_timer.Disable();
			mob_close_scan_timer.Start(RandomTimer(6000, 60000));
		}
	}
...
)
{
	...
	if (npc_type_id) {
		query = StringFormat("INSERT INTO npc_types (id, name, level, race, class, hp, gender, "
				     ...
				     spawn->GetHelmTexture(), spawn->GetSize(), spawn->GetLoottableID().empty() ? 0 : spawn->GetLoottableID().at(0),
		...
		query = StringFormat("INSERT INTO npc_types (name, level, race, class, hp, gender, "
				     ...
				     spawn->GetLoottableID().empty() ? 0 : spawn->GetLoottableID().at(0), spawn->MerchantType, 0, spawn->GetRunspeed(), 28, 28);
		auto results = QueryDatabase(query);
		if (!results.Success()) {
			return false;
		}
...
uint32 ZoneDatabase::UpdateNPCTypeAppearance(Client *client, NPC *spawn)
{
	std::string query =
	    StringFormat("UPDATE npc_types SET name = '%s', level = '%i', race = '%i', class = '%i', "
			 ...
			 spawn->GetGender(), spawn->GetTexture(), spawn->GetHelmTexture(), spawn->GetSize(),
			  spawn->GetLoottableID().empty() ? 0 : spawn->GetLoottableID().at(0), spawn->MerchantType, spawn->GetLuclinFace(), spawn->GetNPCTypeID());
	auto results = QueryDatabase(query);
...
void NPC::ModifyNPCStat(const char *identifier, const char *new_value)
{
	...
	if (id == "ac") {
		...
		loottable_id.push_back(atof(val.c_str()));
		return;
	}
...
void NPC::RescaleNPC(float modifier) { 
	npc_scale_manager->ScaleNPC(this, modifier);
}
...
void NPC::DoQuestPause(Mob *other) {
	if(IsMoving() && !IsOnHatelist(other)) {
		...
		{
			if (GetRace() != RT_PORTAL && GetRace() != RT_CHEST)
				FaceTarget(other);
		}
	} else if(!IsMoving()) {
