...
public:
	...
	int AreaRampageOnPets(Mob *caster, Mob *target, int count, ExtraAttackOptions *opts);
	...
	void AddEntToHateList(Mob *ent, int64 in_hate = 0, int64 in_damage = 0, bool in_is_frenzied = false, bool add_to_hate_list_if_not_exist = true);
	...
	void SetHateOwner(Mob *new_hate_owner);
	void SpellCast(Mob *caster, uint32 spell_id, float range, Mob *ae_center = nullptr, bool isproc = false);
