...
Lua_Mob lua_unique_spawn(int npc_type, int grid, int unused, double x, double y, double z, double heading, uint32 character_id) {
	auto position = glm::vec4(x, y, z, heading);
	return Lua_Mob(quest_manager.unique_spawn(npc_type, grid, unused, position, luaClient));
}
...
uint32 lua_create_instance(std::string zone, uint32 version, uint32 duration, uint32 group_id) {
	return quest_manager.CreateInstance(zone.c_str(), version, duration, group_id);
}

uint32 lua_create_instance(std::string zone, uint32 version, uint32 duration, uint32 group_id, int64 scale_cap) {
	return quest_manager.CreateInstance(zone.c_str(), version, duration, group_id, scale_cap);
}

uint32 lua_create_instance(uint32 zone, uint32 version, uint32 duration, uint32 group_id) {
	return quest_manager.CreateInstance(zone, version, duration, group_id);
}

uint32 lua_create_instance(uint32 zone, uint32 version, uint32 duration, uint32 group_id, int64 scale_cap) {
	return quest_manager.CreateInstance(zone, version, duration, group_id, scale_cap);
}
...
uint32 lua_get_zone_id_from_name(std::string short_name) {
	return quest_manager.GetZoneID(short_name.c_str());
}
...
bool lua_check_instance_by_group_id(uint32 group_id) {
	return quest_manager.CheckInstanceByGroupID(group_id);
}
...
luabind::adl::object lua_get_loot_lockouts(lua_State* L, Lua_Client client) {
	luabind::adl::object ret = luabind::newtable(L);
	...
	auto ll_map = c->loot_lockouts;
	for (auto ll_iter : ll_map)
	{
		LootLockout lootLockout;
		lootLockout.character_id = ll_iter.second.character_id;
		lootLockout.ip_addr = ll_iter.second.ip_addr;
		lootLockout.expirydate = ll_iter.second.expirydate;
		lootLockout.zone_id = ll_iter.second.zone_id;
		lootLockout.npctype_id = ll_iter.second.npctype_id;
		Lua_LootLockout ll_entry = Lua_LootLockout(lootLockout);
		luabind::adl::object l_ll_o = luabind::adl::object(L, ll_entry);
		l_ll_o.push(L);
		ret[(ll_iter).first] = l_ll_o;
...
void lua_create_npc(luabind::adl::object table, float x, float y, float z, float heading) {
	...
	LuaCreateNPCParse(current_hp, int64, 30);
	LuaCreateNPCParse(max_hp, int64, 30);
	...
	LuaCreateNPCParse(hp_regen, int64, 1);
...
luabind::scope lua_register_general() {
	...
	[
		...
		luabind::def("unique_spawn", (Lua_Mob(*)(int,int,int,double,double,double,double,Lua_Client)) & lua_unique_spawn),
		...
		luabind::def("create_instance", (uint32(*)(std::string, uint32, uint32, uint32))&lua_create_instance),
		luabind::def("create_instance", (uint32(*)(std::string, uint32, uint32, uint32, int64))&lua_create_instance),
		luabind::def("create_instance", (uint32(*)(uint32, uint32, uint32, uint32))&lua_create_instance),
		luabind::def("create_instance", (uint32(*)(uint32, uint32, uint32, uint32, int64))&lua_create_instance),
		luabind::def("destroy_instance", &lua_destroy_instance),
		luabind::def("get_zone_id_from_name", &lua_get_zone_id_from_name),
		...
		luabind::def("check_instance_by_group_id", (uint32(*)(uint32))&lua_check_instance_by_group_id),
		...
		luabind::def("get_lootlockouts", (luabind::adl::object(*)(lua_State*, Lua_Client))& lua_get_loot_lockouts),
