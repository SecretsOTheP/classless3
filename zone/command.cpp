...
int command_init(void)
{
	...
	if (
		...
		command_add("genocide", "[aggro] - Aggro every mob in the zone with X aggro. Default is 0. Not recommend if you're not invulnerable.", 100, command_genocide) ||
		...
		command_add("reloadedge", "Reloads Edge-specific data", 80, command_reloadedge) ||
...
void command_summon(Client *c, const Seperator *sep)
{
	...
	else if (t->IsClient())
	{
		...
		t->CastToClient()->SetPosition(c->GetX(), c->GetY(), c->GetZ());
	}
}
	...
	if (c->Admin() >= commandInvSnapshot) {
		...
		if (strcmp(sep->arg[1], "parse") == 0) {
			...
			for (auto iter : parse_list) {
				...
				std::string window_line = StringFormat("%i: %u - %s<br>", iter.first, iter.second, (item_data.ID ? item_data.Name : "[error]"));
...
void command_gmzone(Client *c, const Seperator *sep)
{
	...
	if (instance_id == 0) {
		...
		if (!database.CreateInstance(instance_id, zone_id, zone_version, duration, 0)) {
...
void command_scribespells(Client *c, const Seperator *sep)
{
	...
	for ( ; spell_id < SPDAT_RECORDS && book_slot < EQ::spells::SPELLBOOK_SIZE; ++spell_id) {
		...
		while (true) {
			...
			if (!t->HasSpellScribed(spell_id)) { // isn't a discipline & we don't already have it scribed
...
void command_scribespell(Client *c, const Seperator *sep) {
	...
	if(IsValidSpell(spell_id)) {
		...
		if (spells[spell_id].classes[WARRIOR] != 0 && spells[spell_id].classes[t->GetPP().class_ - 1] > 0) {
...
void command_summonitem(Client *c, const Seperator *sep)
{
	...
	if (item_status > c->Admin())
		...
		c->SummonItem(itemid, atoll(sep->arg[2]));
	else if (sep->argnum == 3)
		c->SummonItem(itemid, atoll(sep->arg[2]), atoll(sep->arg[3]));
	else if (sep->argnum == 4)
		c->SummonItem(itemid, atoll(sep->arg[2]), atoll(sep->arg[3]), atoll(sep->arg[4]));
	else if (sep->argnum == 5)
		c->SummonItem(itemid, atoll(sep->arg[2]), atoll(sep->arg[3]), atoll(sep->arg[4]), atoll(sep->arg[5]));
	else if (sep->argnum == 6)
		c->SummonItem(itemid, atoll(sep->arg[2]), atoll(sep->arg[3]), atoll(sep->arg[4]), atoll(sep->arg[5]), atoll(sep->arg[6]));
	else if (sep->argnum == 7)
		c->SummonItem(itemid, atoll(sep->arg[2]), atoll(sep->arg[3]), atoll(sep->arg[4]), atoll(sep->arg[5]), atoll(sep->arg[6]), atoll(sep->arg[7]));
	else if (sep->argnum == 8)
		c->SummonItem(itemid, atoll(sep->arg[2]), atoll(sep->arg[3]), atoll(sep->arg[4]), atoll(sep->arg[5]), atoll(sep->arg[6]), atoll(sep->arg[7]), atoll(sep->arg[8]));
	else {
		c->SummonItem(itemid);
	}
...
void command_itemsearch(Client *c, const Seperator *sep)
{
	...
	else
	{
		...
		char* pdest;
		uint32 it = 0;
		const EQ::ItemData* data = nullptr;
		while ((data = database.IterateItems(&it))) {
			strn0cpy(sName, data->Name, sizeof(sName));
			strupr(sName);
			pdest = strstr(sName, sCriteria);
			if (pdest != nullptr) {
				linker.SetItemData(*data);

				c->Message(Chat::White, "%i: %s",  data->ID, linker.GenerateLink().c_str());
...
void command_setaapts(Client *c, const Seperator *sep)
{
	...
	if(sep->arg[1][0] == '\0' || sep->arg[2][0] == '\0')
		...
		c->Message(Chat::White, "You must have a number greater than 0 for points.");
...
{	if (!c->GetTarget() || !c->GetTarget()->IsNPC()) {
	...
	if (strcasecmp(sep->arg[1], "help") == 0) {
		...
		c->Message(Chat::White, "#npcedit loot_lockout - Enables loot lockouts, sets the timer. Params: [1, 0]");
		c->Message(Chat::White, "#npcedit loot_lockout_timer - Enables loot lockouts, sets the timer. Params: [time in hours]");
	}
	...
	if (strcasecmp(sep->arg[1], "loot_lockout") == 0) {
		c->Message(15, "NPCID %u now has a loot lockout value of [%s]", npcTypeID, atoi(sep->argplus[2]) != 0 || strcasecmp(sep->argplus[2],"false") != 0 ? "true (1)" : "false (0)");
		std::string query = StringFormat("UPDATE npc_types SET loot_lockout = '%s' WHERE id = %i", sep->argplus[2], npcTypeID);
		database.QueryDatabase(query);
		return;
	}

	if (strcasecmp(sep->arg[1], "loot_lockout_timer") == 0) {
		c->Message(15, "NPCID %u now has a loot lockout timer of [%s] hours. Use #npcedit loot_lockout to disable the timer.", npcTypeID, sep->argplus[2]);
		std::string query = StringFormat("UPDATE npc_types SET loot_lockout = 1 WHERE id = %i", npcTypeID);
		std::string query2 = StringFormat("UPDATE npc_types SET loot_lockout_timer = '%s' WHERE id = %i", sep->argplus[2], npcTypeID);
		database.QueryDatabase(query);
		database.QueryDatabase(query2);
		return;
	}
...
void command_reloadinstscale(Client *c, const Seperator *sep) {
	c->Message(0, "Reloading instance scalerate data...");
	zone->ResetInstanceScaleCap();
}

void command_reloadedge(Client *c, const Seperator *sep) {
	c->Message(0, "Reloading edge spell data...");
	zone->LoadEdgeSpells();
	zone->ResetInstanceScaleCap();
}
...
void command_genocide(Client *c, const Seperator *sep) {
	if(!c)
		return;

	uint32 hate = atoi(sep->arg[1]); //should default to 0 if we don't enter anything
	entity_list.Genocide(c, hate);
}
...
void command_instance(Client *c, const Seperator *sep)
{
	...
	else if(strcasecmp(sep->arg[1], "create") == 0)
	{
		...
		if(!database.CreateInstance(id, zone_id, version, duration, 0))
		{
...
{ // - Owner only command..non-targetable to eliminate malicious or mischievious activities.
	if (!c)
		...
		EQ::ItemData FakeItem = database.GetItem(itemid);
...
void command_merchantopenshop(Client *c, const Seperator *sep)
{
	...
	if (!merchant || !merchant->IsNPC() || merchant->CastToNPC()->MerchantType == 0 || merchant->GetClass() != MERCHANT && zone && zone->GetZoneID() != RuleI(World, LoadZoneID)) {
