...
Mob* QuestManager::spawn2(int npc_type, int grid, int unused, const glm::vec4& position, uint32 questgranter) {
	const NPCType* tmp = 0;
	if (tmp = content_db.LoadNPCTypesData(npc_type))
	{
		...
		npc->QuestSpawned = true;
		npc->QuestLootEntity = questgranter;
...
Mob* QuestManager::unique_spawn(int npc_type, int grid, int unused, const glm::vec4& position, uint32 questgranter) {
...
Mob *QuestManager::spawn_from_spawn2(uint32 spawn2_id, uint32 questgranter)
{
	...
	//		npc->CheckGlobalLootTables();
		...
		if (spawn_group->roambox[0] != 0.0f && spawn_group->roambox[1] != 0.0f && spawn_group->roambox[2] != 0.0f && spawn_group->roambox[3] != 0.0f) {
			npc->AI_SetRoambox(
...
uint16 QuestManager::scribespells(uint8 max_level, uint8 min_level) {
	...
	for ( ; spell_id < SPDAT_RECORDS && book_slot < EQ::spells::SPELLBOOK_SIZE; ++spell_id) {
		...
		while (true) {
			...
			if (!initiator->HasSpellScribed(spell_id)) { // isn't a discipline & we don't already have it scribed
				if (SpellGlobalRule) {
...
void QuestManager::setnexthpevent(int64 at) {
	QuestManagerCurrentQuestVars();
	if (owner)
...
void QuestManager::MerchantSetItem(uint32 NPCid, uint32 itemid, uint32 quantity) {
	...
	if (merchant == 0 || !merchant->IsNPC() || !merchant->CastToNPC()->MerchantType || (merchant->GetClass() != MERCHANT) && zone && zone->GetZoneID() != RuleI(World, LoadZoneID))
...
uint32 QuestManager::CreateInstance(uint32 zone_id, int16 version, uint32 duration, uint32 group_id, int64 scale_cap)
{
	QuestManagerCurrentQuestVars();
	if(initiator)
	{
		...
		if(!database.CreateInstance(id, zone_id, version, duration, group_id, scale_cap))
		{
			initiator->Message(Chat::Red, "Server was unable to create a new instance.");
			return 0;
		}
...
void QuestManager::DestroyInstance(uint32 instance_id)
{
	database.DeleteInstance(instance_id);
}

void QuestManager::UpdateInstanceTimer(uint32 instance_id, uint32 new_duration)
{
...
uint32 QuestManager::GetInstanceTimerByID(uint32 instance_id) {
	if (instance_id == 0)
		return 0;
...
uint32 QuestManager::GetInstanceID(const char *zone, int16 version)
{
	QuestManagerCurrentQuestVars();
	if (initiator)
	{
...
void QuestManager::AssignToInstance(uint32 instance_id)
{
	QuestManagerCurrentQuestVars();
	if (initiator)
	{
...
void QuestManager::AssignToInstanceByCharID(uint32 instance_id, uint32 char_id) {
	database.AddClientToInstance(instance_id, char_id);
}

void QuestManager::AssignGroupToInstance(uint32 instance_id)
{
	QuestManagerCurrentQuestVars();
	if (initiator)
	{
...
void QuestManager::AssignRaidToInstance(uint32 instance_id)
{
	QuestManagerCurrentQuestVars();
	if (initiator)
	{
...
void QuestManager::RemoveFromInstance(uint32 instance_id)
{
	QuestManagerCurrentQuestVars();
	if (initiator)
	{
...
void QuestManager::RemoveFromInstanceByCharID(uint32 instance_id, uint32 char_id) {
	database.RemoveClientFromInstance(instance_id, char_id);
}

uint32 QuestManager::CheckInstanceByGroupID(uint32 group_id) {
	return database.CharacterInInstanceGroup(database.GetGroupIDByCharID(group_id));
}

void QuestManager::RemoveAllFromInstance(uint32 instance_id)
{
	QuestManagerCurrentQuestVars();
	if (initiator)
	{
...
void QuestManager::CrossZoneMoveInstanceByCharID(int character_id, uint32 instance_id) {
...
void QuestManager::CrossZoneMoveInstanceByGroupID(int group_id, uint32 instance_id) {
...
void QuestManager::CrossZoneMoveInstanceByRaidID(int raid_id, uint32 instance_id) {
...
void QuestManager::CrossZoneMoveInstanceByGuildID(int guild_id, uint32 instance_id) {
...
void QuestManager::WorldWideMoveInstance(uint32 instance_id, uint8 min_status, uint8 max_status) {
...
void QuestManager::ReloadAllRules()
{
...
void QuestManager::ScaleNPC(uint16 entityId) {

	NPC* npc = entity_list.GetNPCByID(entityId);
...
void QuestManager::UpdateZoneHeader(std::string type, std::string value) {
	...
	else if (strcasecmp(type.c_str(), "zone_exp_mod") == 0)
		zone->newzone_data.zone_exp_multiplier = atof(value.c_str());
