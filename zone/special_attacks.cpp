...
{
#ifndef TLP_STYLE
	return EQ::skills::GetBaseDamage(skill);
#else
...
void Mob::DoSpecialAttackDamage(Mob *who, EQ::skills::SkillType skill, int64 base_damage, int64 min_damage,
	int64 hate_override, int ReuseTime)
{
	...
	my_hit.offense = offense(EQ::skills::Skill1HBlunt);
	my_hit.tohit = GetTotalToHit(EQ::skills::Skill1HBlunt, 0);
...
void Client::OPCombatAbility(const CombatAbility_Struct *ca_atk)
{
	...
	if (!MaxSkill(static_cast<EQ::skills::SkillType>(ca_atk->m_skill)) && !CanBypassSkillCheck && ca_atk->m_skill != EQ::skills::SkillArchery && ca_atk->m_skill != EQ::skills::SkillThrowing)
		return;
...
void Client::RangedAttack(Mob* other, bool CanDoubleAttack) {
	...
	EQ::ItemData AmmoItem = Ammo ? database.GetItem(Ammo->GetID()) : database.GetItem(8005);
	...
	bool bIsActuallyAmmo = true;

	if (AmmoItem.ItemType != EQ::item::ItemTypeArrow) {
		AmmoItem = database.GetItem(8005);
		bIsActuallyAmmo = false;
	}
	...
	if(Ammo && Ammo->GetCharges() == 1) {
		...
		if(!found) {
			...
			int32 aslot = m_inv.HasItem(Ammo->GetID(), 1, invWherePersonal);
...
void Client::ThrowingAttack(Mob* other, bool CanDoubleAttack) { //old was 51
	...
	CheckIncreaseSkill(EQ::skills::SkillThrowing, GetTarget());
	CommonBreakInvisibleFromCombat();
}
...
void Mob::Taunt(NPC *who, bool always_succeed, int chance_bonus, bool FromSpell, int32 bonus_hate)
{
	...
	if (IsPet() && GetUltimateOwner() && GetUltimateOwner()->IsClient())
		return;
