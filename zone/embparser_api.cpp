...
XS(XS__setnexthpevent) {
	dXSARGS;
	if (items != 1)
		Perl_croak(aTHX_ "Usage: quest::setnexthpevent(int64 at_mob_percentage)");

	int64 at = (int64)SvIV(ST(0));
...
XS(XS__checksum) {
	dXSARGS;
	if (items > 1)
		Perl_croak(aTHX_ "Usage: quest::checksum(int accid)");
	...
	uint32 accid;
	if (items == 1)
		accid = (uint32)SvIV(ST(0));
	else
		accid = 0;

	RETVAL = quest_manager.checksum(accid);
	XSprePUSH;
	PUSHu((IV)RETVAL);
...
XS(XS__CreateInstance) {
	...
	uint32 id       = quest_manager.CreateInstance(zone, version, duration, 0);
...
XS(XS__CheckInstanceByGroupID);
XS(XS__CheckInstanceByGroupID) {
	dXSARGS;
	if (items != 1) {
		Perl_croak(aTHX_ "Usage: quest::CheckInstanceByCharID(uint32 instance_id, uint32 char_id)");
	}
	...
	uint32 char_id = (int) SvUV(ST(1));
	RETVAL = quest_manager.CheckInstanceByGroupID(char_id);
	XSprePUSH;
	PUSHu((IV) RETVAL);
...
XS(XS__reloadallrules);
XS(XS__reloadallrules) {
	dXSARGS;

	quest_manager.ReloadAllRules();
...
XS(XS__scalenpc) {
	...
	if (items != 1) {
		Perl_croak(aTHX_ "Usage: quest::scalenpc(int entityId)");
	}
	else {
		int32 entityId = (int32)SvIV(ST(0));
		quest_manager.ScaleNPC(entityId);
	}
...
EXTERN_C XS(boot_quest) {
	...
	newXS(strcpy(buf, "CheckInstanceByGroupID"), XS__CheckInstanceByGroupID, file);
	...
	newXS(strcpy(buf, "checksum"), XS__checksum, file);
	...
	newXS(strcpy(buf, "reloadallrules"), XS__reloadzonestaticdata, file);
	...
	newXS(strcpy(buf, "scalenpc"), XS__scalenpc, file);
