...
public:
	...
	uint8 IsHotzone() const { return (is_hotzone); }
	bool IsLoaded();
	...
	inline uint32	GetInstanceGroupID() { return instance_group_id; }
	inline int64	GetInstanceScaleCap() { return instance_scale_cap; }
	inline void		ResetInstanceScaleCap() { instance_scale_cap = database.GetInstanceScaleCap(instanceid); }
	...
	std::map<uint8, std::list<EdgeSpell_Struct>> edge_spells_table;
	...
	void LoadEdgeSpells();
	std::list<int32> GetEdgeSpellsForClass(uint8 edge_class, uint32 PlayerLevel = 255, bool PrimaryOnly = false);
	...
	void LoadZoneBlockedSpells(uint32 zoneid);
...
private:
	...
	uint32    instance_group_id;
	int64    instance_scale_cap;
	uint32    instanceid;
