...
uint32 ZoneDatabase::GetZoneForage(uint32 ZoneID, uint32 skill) {
...
uint32 ZoneDatabase::GetZoneFishing(uint32 ZoneID, uint32 skill, uint32 &npc_id, uint8 &npc_chance)
{
	uint32 item[50];
	uint32 chance[50];
	uint32 npc_ids[50];
...
void Client::GoFish()
{
	...
	fishing_skill += m_pp.intoxication / 4;
	...
	if (zone->random.Int(0, 399) < fishing_skill) {
		...
		EQ::ItemData food_item = database.GetItem(food_id);

		if (food_item.ID != 0 && food_item.ItemType != EQ::item::ItemTypeFood) {
		...
		if (inst != nullptr) {
			...
			EQ::ItemInstance* inst = database.CreateItem(food_item.ID, 1);
			if (inst != nullptr) {
				if (CheckLoreConflict(inst->GetItem()))
				{
	...
	else
	{
		...
		if (zone->random.Int(0, 4) == 1) {
			...
			if (zone->random.Int(0, 4) == 1)	//give about a 1 in 15 chance to spill your beer. we could make this a rule, but it doesn't really seem worth it
	...
	if (CheckIncreaseSkill(EQ::skills::SkillFishing, nullptr, 5))
	{
		if (title_manager.IsNewTradeSkillTitleAvailable(EQ::skills::SkillFishing, GetRawSkill(EQ::skills::SkillFishing)))
			...
			AddEXP(100 * GetRawSkill(EQ::skills::SkillFishing), 0);
	}
}
...
void Client::ForageItem(bool guarantee) {
	...
	if (guarantee || zone->random.Int(0,199) < skill_level) {
		...
		EQ::ItemData food_item = database.GetItem(foragedfood);
