...
{
	auto hate_to_add = in_hate;

	if (other && other->IsPet() && other->GetUltimateOwner() && other->GetUltimateOwner()->IsClient())
	{
		hate_to_add = 0;
	}
...
void HateList::AddEntToHateList(Mob *in_entity, int64 in_hate, int64 in_damage, bool in_is_entity_frenzied, bool iAddIfNotExist)
{
	...
	if (in_entity && in_entity->IsPet() && in_entity->GetUltimateOwner() && in_entity->GetUltimateOwner()->IsClient())
	{
		hate_to_add = 0;
	}
	...
	if (entity)
	{
		...
		entity->stored_hate_amount += hate_to_add;
...
bool HateList::RemoveEntFromHateList(Mob *in_entity)
{
	...
	if (hate_owner && list.empty() && (!hate_owner->GetUltimateOwner()->IsClient()))
	{
		hate_owner->SetHP(hate_owner->GetMaxHP());
		hate_owner->BuffFadeDetrimental();
	}
...
void HateList::IncursionScale()
{
	if (hate_owner && (hate_owner->IsPetOwnerClient() || hate_owner->GetUltimateOwner()->IsClient()))
		return;

	Mob *ScaleTarget = nullptr;
	if (hate_owner && hate_owner->IsNPC())
	{
		uint64 highestpower = 0;
		...
		if (!list.empty())
		{
			for (auto iter : list) {
				if (!iter->entity_on_hatelist)
					continue;
				if (iter->entity_on_hatelist->GetUltimateOwner()->IsClient())
				{
					FoundClient = true;
					ScaleTarget = iter->entity_on_hatelist->GetScaleTarget();
					if (ScaleTarget)
					{
						highestpower = ScaleTarget->m_POWER * ScaleTarget->m_POWER_NORMAL;
					}
					//Group *kg = entity_list.GetGroupByClient(iter->entity_on_hatelist->GetUltimateOwner()->CastToClient());
					//Raid *kr = entity_list.GetRaidByClient(iter->entity_on_hatelist->GetUltimateOwner()->CastToClient());

					//if(kr && kr->GetLeader())
					//{
					//	highestpower = kr->GetLeader()->m_POWER;
					//}
					//else if(kg && kg->GetLeader())
					//{
					//	highestpower = kg->GetLeader()->m_POWER;
					//}
					//else
					//{
					//	if(iter->entity_on_hatelist->GetUltimateOwner() && iter->entity_on_hatelist->GetUltimateOwner()->m_POWER > highestpower)
					//	{
					//		highestpower = iter->entity_on_hatelist->m_POWER;
					//	}
					//}
				}
			}

			if (FoundClient && ScaleTarget && ScaleTarget->IsClient() && !ScaleTarget->CastToClient()->GetPVP(false))
			{
				if (highestpower > 0)
				{
					if (isScaled)
					{
						if (highestpower > hate_owner->GetLevel())
						{
							npc_scale_manager->ScaleNPC(hate_owner->CastToNPC(), highestpower);
						}
					}
					else
					{
			...
			{
				npc_scale_manager->ScaleNPC(hate_owner->CastToNPC());
				isScaled = false;
			}
		}
		else
...
Mob *HateList::GetEntWithMostHateOnList(Mob *center, Mob *skip, bool skip_mezzed)
{
	...
	if (RuleB(Aggro, SmartAggroList)){
		...
		/*	if (!isTopClientType) {
			...
			if (!isTopClientType && !center->IsRooted())
...
int HateList::AreaRampageOnPets(Mob *caster, Mob *target, int count, ExtraAttackOptions *opts)
{
	if (!target || !caster)
		return 0;
	...
	for (auto &h : list) {
		if (h->entity_on_hatelist && h->entity_on_hatelist != caster && h->entity_on_hatelist != target &&
			caster->CombatRange(h->entity_on_hatelist) && h->entity_on_hatelist->IsPet())
		{
			if (h->entity_on_hatelist->IsNPC() && 
				h->entity_on_hatelist->CastToNPC()->GetPetSpellID() != 1936 &&
				h->entity_on_hatelist->CastToNPC()->GetPetSpellID() != 43300)
				id_list.push_back(h->entity_on_hatelist->GetID());
		}
		if (count != -1 && id_list.size() > count)
			break;
...
void HateList::SpellCast(Mob *caster, uint32 spell_id, float range, Mob* ae_center, bool isproc)
{
	if (!caster)
		return;
	...
	while (iter != id_list.end())
	{
		...
		if (cur)
		{
			caster->SpellOnTarget(spell_id, cur, false, false, 0, isproc);
		}
		iter++;
	}
}
...
void HateList::SetHateOwner(Mob *new_hate_owner)
{
	hate_owner = new_hate_owner; 
	isScaled = false; 
}
