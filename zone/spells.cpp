...
bool Mob::CastSpell(uint16 spell_id, uint16 target_id, CastingSlot slot,
	int32 cast_time, int64 mana_cost, uint32* oSpellWillFinish, uint32 item_slot,
	...
	uint32 aa_id)
{
	...
	)
	{
		...
		if (casting_spell_id && IsMerc())
			CastToMerc()->AI_Event_MercSpellCastFinished(false, casting_spell_id);
		return(false);
	}
	...
	if (IsClient()) {
		...
		if (IsEffectInSpell(spell_id, SE_SummonServerGroup))
		{
			if (HasGroup() && strcmp(GetGroup()->GetLeaderName(), GetCleanName()) != 0)
			{
	...
	if(item_slot && IsClient() && (slot == CastingSlot::Item || slot == CastingSlot::PotionBelt))
	{
		...
		if( itm && itm->GetItem()->Classes != 65535 ) {
			if ((itm->GetItem()->Click.Type == EQ::item::ItemEffectEquipClick) && !(itm->GetItem()->Classes & bitmask) && GetClass() != CLASSLESS) {
			...
			if ((itm->GetItem()->Click.Type == EQ::item::ItemEffectClick2) && !(itm->GetItem()->Classes & bitmask) && GetClass() != CLASSLESS) {
				if (CastToClient()->ClientVersion() < EQ::versions::ClientVersion::SoF) {
...
bool Mob::DoCastSpell(uint16 spell_id, uint16 target_id, CastingSlot slot,
					int32 cast_time, int64 mana_cost, uint32* oSpellWillFinish,
					...
					int16 resist_adjust, uint32 aa_id)
{
	...
	{
		int64 my_curmana = GetMana();
		int64 my_maxmana = GetMaxMana();
...
void Mob::CastedSpellFinished(uint16 spell_id, uint32 target_id, CastingSlot slot,
	int64 mana_used, uint32 inventory_slot, int16 resist_adjust)
{
		...
		if (IsBardSong(spell_id)) {
			if (spells[spell_id].buffduration == 0xFFFF) {
				...
				if (spells[spell_id].recast_time == 0) {
					bardsong = spell_id;
					bardsong_stoptimer.Start(30000, true);
					bardsong_slot = slot;
	...
	else if (!IsBardSong(spell_id))// not bard, check movement
	{
				...
				)
		{
			...
			if (IsClient())
			{
				...
				//channelchance = 85;
				channelchance -= attacked_count;
				channelchance = std::max(channelchance, 75.f);
			}
			...
			for(int t_count = 0; t_count < 4; t_count++) {
				...
				// handle the components for traditional casters
					...
					else if(c->GetInv().HasItem(component, component_count, invWhereWorn|invWherePersonal) == -1) // item not found
					{
						...
						EQ::ItemData item = database.GetItem(component);
						if(item.ID) {
...
// single target spells
		...
		case ST_SummonedPet:
		{
			...
			if(!spell_target || isTargetingPet ||
		...
		case ST_GroupClientAndPet:
		{
			...
			if(spell_target != this)
			{
				...
				bool bIsPetTarget = false;
				for (auto mob : petlist)
				{
					if (mob == spell_target)
					{
						CastAction = SingleTarget;
...
bool Mob::SpellFinished(uint16 spell_id, Mob *spell_target, CastingSlot slot, int64 mana_used,
						uint32 inventory_slot, int16 resist_adjust, bool isproc, int level_override)
{
	...
	if (IsClient() && slot < CastingSlot::MaxGems && !CastToClient()->FindMemmedSpellBySlot((int)slot)) {
		Message(Chat::SpellFailure, "You cannot cast that spell without a spell in that gem.");
		return false;
	}

	if (IsClient() && slot < CastingSlot::MaxGems && CastToClient()->FindMemmedSpellBySlot((int)slot) != spell_id) {
		Message(Chat::SpellFailure, "You cannot cast that spell without having it memmed first.");
		return false;
	}

	if ((spell_id == SPELL_HARM_TOUCH || spell_id == SPELL_HARM_TOUCH2) && IsClient() && !CastToClient()->GetGM() && spell_target && spell_target->IsNPC() && spell_target->CastToNPC()->GetNPCScaleTier() > 1)
	{
		Message(Chat::SpellFailure, "You cower in fear as you begin to harm touch your enemy and decide doing so probably isn't a good idea.");
		return false;
	}
	...
	{
		if (IsNPC() && CastToNPC()->GetNPCScaleTier() > 2 && !CombatRange(spell_target))
		{
			LogSpells("Spell [{}]: cannot see target [{}]", spell_id, spell_target->GetName());
			MessageString(Chat::Red,CANT_SEE_TARGET);
			return false;
		}
		else if(!IsNPC() || IsNPC() && CastToNPC()->GetNPCScaleTier() <= 2)
		{
			return false;
		}
	}
	...
	switch(CastAction)
	{
		...
		case AETarget:
		{
			...
			if(ae_center->IsBeacon()) {
				...
				ae_center->CastToBeacon()->AELocationSpell(this, spell_id, resist_adjust, isproc);
			} else {
				// unsure if we actually need this? Need to find some spell examples
				if(ae_center && ae_center == this && IsBeneficialSpell(spell_id))
					SpellOnTarget(spell_id, this, false, false, 0, isproc);
				...
				if (spells[spell_id].targettype == ST_AETargetHateList)
					hate_list.SpellCast(this, spell_id, spells[spell_id].aoerange, ae_center, isproc);
				else
					entity_list.AESpell(this, ae_center, spell_id, affect_caster, resist_adjust, nullptr, isproc);
			}
			break;
		}
		...
		case AllPets:
		{
			if (spell_target)
			{
				std::list<Mob*> pet_iterator_list;

				for (auto pet : spell_target->petlist)
				{
					pet_iterator_list.push_back(pet);
				}

				for(auto p : pet_iterator_list)
				{
					SpellOnTarget(spell_id, p, false, false, 0, isproc);
				}
			}
			break;
		}
		...
		case GroupSpell:
		{
			...
			{
				SpellOnTarget(spell_id, this, false, false, 0, isproc);
				entity_list.MassGroupBuff(this, this, spell_id, true, isproc);
			}
			else
			{
				...
				if(spell_target->IsGrouped())
				{
					...
					if(target_group)
					{
						target_group->CastGroupSpell(this, spell_id, isproc);
					}
				}
				else if(spell_target->IsRaidGrouped() && spell_target->IsClient())
				{
					...
					if(target_raid){
						...
						if(gid < 12)
							target_raid->CastGroupSpell(this, spell_id, gid, isproc);
						else
							SpellOnTarget(spell_id, spell_target, false, false, 0, isproc);
					}
				}
				else
				{
		...
		case CAHateList:
		{
			if(!IsClient())
			{
				hate_list.SpellCast(this, spell_id, spells[spell_id].range > spells[spell_id].aoerange ? spells[spell_id].range : spells[spell_id].aoerange, nullptr, isproc);
			}
			break;
		}
		...
		case TargetRing:
		{
			entity_list.AESpell(this, nullptr, spell_id, false, resist_adjust, nullptr, isproc);
			break;
		}
	}
	...
	{
		mana_used = GetActSpellCost(spell_id, mana_used) * GetHaste(spells[spell_id].skill) / 100.0f;
		if (mgb) {
			mana_used *= 2;
		}
	...
	if (spells[spell_id].EndurCost && !isproc) {
		int64 end_cost = spells[spell_id].EndurCost;
		if (mgb)
			end_cost *= 2;
	...
	if (IsMerc())
		CastToMerc()->AI_Event_MercSpellCastFinished(true, spell_id);
	return true;
}
...
int Mob::CalcBuffDuration(Mob *caster, Mob *target, uint16 spell_id, int32 caster_level_override)
{
	...
	if(target->IsClient())
	{
		if(IsDetrimentalSpell(spell_id) && res > 30)
			res = 30;
	}
			...
			bool isproc, int level_override)
{
	...
	if(IsDetrimentalSpell(spell_id) && !IsAttackAllowed(spelltar, true) && !IsResurrectionEffects(spell_id) && !IsEffectInSpell(spell_id, SE_SumCorpsePet)) {
		if(!IsClient() || !CastToClient()->GetGM()) {
	...
	if(SkipSpellPacket)
	{
		DoAnim(spells[spell_id].CastingAnim, 0, false);
	}
	else
	{
		...
		entity_list.QueueCloseClients(
			...
			this, /* Skip this Mob */
			true, /* Packet ACK */
			FilterNone // fixes spell effects for non caster/target
		);
	}
	...
	if(!(IsClient() && CastToClient()->GetGM()) && !IsHarmonySpell(spell_id))	// GMs can cast on anything
	{
		...
		if(IsBeneficialSpell(spell_id))
		{
				...
				spelltar != this)
			{
				...
				bool targetingOurPet = false;

				for (auto pet : petlist)
				{
					if (pet == spelltar)
						targetingOurPet = true;
				}
				...
				if((!IsAllianceSpellLine(spell_id) && !IsBeneficialAllowed(spelltar)) ||
					(IsGroupOnlySpell(spell_id) &&
						!(
							...
							(targetingOurPet) //should be able to cast grp spells on self and pet despite grped status.
						)
					)
				)
				{
		...
		else if	( !IsAttackAllowed(spelltar, true) && !IsResurrectionEffects(spell_id) && !IsEffectInSpell(spell_id, SE_SumCorpsePet)) // Detrimental spells - PVP check
		{
	...
	if(spelltar && spelltar->TryReflectSpell(spell_id) && !reflect && IsDetrimentalSpell(spell_id) && this != spelltar) {
		...
		if (reflect_chance) {
			...
			// spell
			spelltar->SpellOnTarget(spell_id, this, true, use_resist_adjust, resist_adjust, isproc);
			safe_delete(action_packet);
			return false;
		}
	}
		...
		if(spell_effectiveness < 100)
		{
			if(spell_effectiveness == 0 || !IsPartialCapableSpell(spell_id) )
			{
				...
				if (spelltar->IsAIControlled()) {
					int64 aggro = CheckAggroAmount(spell_id, spelltar);
					if (aggro > 0) {
	...
	if (spelltar->IsAIControlled() && IsDetrimentalSpell(spell_id) && !IsHarmonySpell(spell_id)) {
		int64 aggro_amount = CheckAggroAmount(spell_id, spelltar, isproc);
	...
	if(!spelltar->SpellEffect(this, spell_id, spell_effectiveness, level_override, isproc))
	{
	...
	// EDGE - Fixes spells not showing up for non-casters?
	entity_list.QueueCloseClients(spelltar, action_packet, false, 200, 0, true, spelltar->IsClient() ? FilterPCSpells : FilterNPCSpells);
...
void Mob::BuffFadeBeneficialByCaster(Mob *caster)
{
	if (!caster)
		return;
...
bool Client::IsItemRecastExpired(int16 slot)
{
	if(slot == -1)
		return false;
	...
	if (itm && itm->GetItem()->RecastDelay > 0 && itm->GetItem()->RecastType > 0)
	{
		if (GetPTimers().Expired(&database, (pTimerItemStart + itm->GetItem()->RecastType), false)) {
			bExpired = true;
		}
	}
	return bExpired;
...
int32 Client::GetCharmEffectID(int16 slot)
{
	if (slot == -1)
		return -1;
...
bool Mob::IsImmuneToSpell(uint16 spell_id, Mob *caster)
{
	...
	if (IsClient())
	{
		if (IsEffectInSpell(spell_id, SE_Root) &&
			CastToClient()->GetCharmEffectID(EQ::invslot::slotCharm) == CharmSpeedChangeImmmunity
			&& CastToClient()->IsItemRecastExpired(EQ::invslot::slotCharm))
		{
				Log(Logs::Detail, Logs::Spells, "We are immune to Root spells.");
				caster->MessageString(Chat::Red, IMMUNE_MOVEMENT);
				return true;
		}
		...
		if (IsEffectInSpell(spell_id, SE_SumClone) && IsDetrimentalSpell(spell_id) && 
			CastToClient()->GetCharmEffectID(EQ::invslot::slotCharm) == CharmEnchantImmunity
			&& CastToClient()->IsItemRecastExpired(EQ::invslot::slotCharm))
		{
				Log(Logs::Detail, Logs::Spells, "We are immune to Clone spells.");
				caster->MessageString(Chat::Red, SPELL_NO_HOLD);
				return true;
		}

		if (IsEffectInSpell(spell_id, SE_Mez) && IsDetrimentalSpell(spell_id) && 
			CastToClient()->GetCharmEffectID(EQ::invslot::slotCharm) == CharmEnchantImmunity
			&& CastToClient()->IsItemRecastExpired(EQ::invslot::slotCharm))
		{
		...
		if (IsEffectInSpell(spell_id, SE_Fear) && IsDetrimentalSpell(spell_id) && 
			CastToClient()->GetCharmEffectID(EQ::invslot::slotCharm) == CharmFearImmunity
			&& CastToClient()->IsItemRecastExpired(EQ::invslot::slotCharm))
		{
...
void Client::Stun(int duration)
{
	...
	{
		if (
			CastToClient()->GetCharmEffectID(EQ::invslot::slotCharm) == CharmStunImmunity
			&& CastToClient()->IsItemRecastExpired(EQ::invslot::slotCharm))
		{
			Log(Logs::Detail, Logs::Spells, "We are immune to Stun spells.");
...
void Client::UnmemSpell(int slot, bool update_client)
{
	...
	uint32 forgotten_id = m_pp.mem_spells[slot];
	...
	if (HasActiveSong() && IsBardSong(forgotten_id) && bardsong == forgotten_id)
	{
		_StopSong();
	}

	entity_list.BuffFadeBySpellIDAndCaster(forgotten_id, this->GetID());

	std::list<Mob*> iteration_list;

	for (auto pet : petlist)
	{
		if (pet->CastToNPC()->GetPetSpellID() == forgotten_id)
			iteration_list.push_back(pet);
	}
	for (auto iter : iteration_list)
	{
		iter->SayString(this, Chat::PetResponse, PET_GETLOST_STRING);
		RemovePet(iter);
	}
...
void Client::UnscribeSpell(int slot, bool update_client)
{
	...
	auto spell_id = m_pp.spell_book[slot];
	...
	if (HasActiveSong() && IsBardSong(spell_id) && bardsong == spell_id)
	{
		_StopSong();
	}

	entity_list.BuffFadeBySpellIDAndCaster(spell_id, this->GetID());
...
int Client::GetNextAvailableSpellMemSlot(int starting_slot) {
	for (int i = starting_slot; i < EQ::spells::SPELL_GEM_COUNT; i++) {	//using starting_slot should help speed this up when we're iterating through a bunch of spells
...
int Client::FindMemSlotBySpellID(uint16 spellid) {
	for (int i = 0; i < EQ::spells::SPELL_GEM_COUNT; i++) {
...
bool Mob::UseBardSpellLogic(uint16 spell_id, int slot)
{
	...
	(
		...
		slot != -1 &&
		(GetClass() == BARD || GetClass() == CLASSLESS) &&
...
void Client::SendBuffDurationPacket(Buffs_Struct &buff, int slot)
{
	...
	sbf->buff.duration = buff.ticsremaining/3;
	if (buff.dot_rune)
...
EQApplicationPacket *Mob::MakeBuffsPacket(bool for_target)
{
	...
	buff->count = count;
	buff->all_buffs = 1;
	buff->tic_timer = tic_timer.GetRemainingTime()/3;
	...
	for(int i = 0; i < buff_count; ++i)
	{
		if(buffs[i].spellid != SPELL_UNKNOWN)
		{
			...
			buff->entries[index].tics_remaining = buffs[i].ticsremaining/3;
...
void Client::InitializeBuffSlots()
{
	...
	{
		memset(&buffs[x], 0, sizeof(Buffs_Struct));
...
void Mob::BeamDirectional(uint16 spell_id, int16 resist_adjust, bool isproc)
{
	...
	while (iter != targets_in_range.end()) {
		...
		if (d <= spells[spell_id].aoerange) {
			if (CheckLosFN((*iter)) || spells[spell_id].npc_no_los || IsNPC() && CastToNPC()->GetNPCScaleTier() > 2 && CombatRange(*iter, false)) {
				(*iter)->CalcSpellPowerDistanceMod(spell_id, 0, this);
				SpellOnTarget(spell_id, (*iter), false, true, resist_adjust, isproc);
				maxtarget_count++;
			}
...
void Mob::ConeDirectional(uint16 spell_id, int16 resist_adjust, bool isproc)
{
	int maxtarget_count = 0;
	bool beneficial_targets = false;
	...
	while (iter != targets_in_range.end()) {
		...
		if (angle_start > angle_end) {
			if ((heading_to_target >= angle_start && heading_to_target <= 360.0f) ||
				...
				if (CheckLosFN((*iter)) || spells[spell_id].npc_no_los || IsNPC() && CastToNPC()->GetNPCScaleTier() > 2 && CombatRange(*iter)) {
					(*iter)->CalcSpellPowerDistanceMod(spell_id, 0, this);
					SpellOnTarget(spell_id, (*iter), false, true, resist_adjust, isproc);
					maxtarget_count++;
				}
