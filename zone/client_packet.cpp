...
void Client::UpdateEdgeSpells()
{
	UnmemSpellAll(client_state == CLIENT_CONNECTED);
	UnscribeSpellAll(client_state == CLIENT_CONNECTED);

	if (zone)
	{
		//First, primary class:
		std::list<int32> pri_spells = zone->GetEdgeSpellsForClass(m_epp.edge_pri_class_id, GetLevel(), true);
		for (auto spell : pri_spells)
		{
			ScribeSpell(spell, GetNextAvailableSpellBookSlot(), client_state == CLIENT_CONNECTED);
			MemSpell(spell, GetNextAvailableSpellMemSlot(), client_state == CLIENT_CONNECTED);
		}
		//Then, secondary
		std::list<int32> sec_spells = zone->GetEdgeSpellsForClass(m_epp.edge_sec_class_id, GetLevel(), false);
		for (auto spell : sec_spells)
		{
			ScribeSpell(spell, GetNextAvailableSpellBookSlot(), client_state == CLIENT_CONNECTED);
			MemSpell(spell, GetNextAvailableSpellMemSlot(), client_state == CLIENT_CONNECTED);
		}
		//Third spec's a charm
		std::list<int32> ter_spells = zone->GetEdgeSpellsForClass(m_epp.edge_ter_class_id, GetLevel(), false);
		for (auto spell : ter_spells)
		{
			ScribeSpell(spell, GetNextAvailableSpellBookSlot(), client_state == CLIENT_CONNECTED);
			MemSpell(spell, GetNextAvailableSpellMemSlot(), client_state == CLIENT_CONNECTED);
		}
...
void Client::CompleteConnect()
{
	...
	if (m_ClientVersionBit & EQ::versions::maskUFAndLater && !petlist.empty()) {
...
void Client::Handle_Connect_OP_ReqClientSpawn(const EQApplicationPacket *app)
{
	...
	if (strncasecmp(zone->GetShortName(), "load", 4) == 0)
		SendBazaarWelcome();
...
void Client::Handle_Connect_OP_ZoneEntry(const EQApplicationPacket *app)
{
	...
	*/
	...
	database.LoadCharacterLootLockouts(loot_lockouts, AccountID(), GetIP()); /* Load CharacterTribute */
	validMercCharacters = database.GetCharactersOnAccount(AccountID());
	checkedValidAltCharacters = true;
	/* Load AdventureStats */
	...
	bool bSentPetInfo = false;
	for (auto petmap : m_petinfo)
	{
		PetInfo petinfo = petmap.second;

		if (petinfo.SpellID > 1 && petinfo.SpellID <= SPDAT_RECORDS) {
			Mob* pet = MakePoweredPet(petinfo.SpellID, spells[petinfo.SpellID].teleport_zone, petinfo.petpower, petinfo.Name, petinfo.size);
			if (pet && pet->IsNPC()) {
				pet->CastToNPC()->SetPetState(petinfo.Buffs, petinfo.Items);
				pet->CastToNPC()->RecalcBonuses();
				pet->SetHP(petinfo.HP);
				pet->SetMana(petinfo.Mana);
				if (bSentPetInfo == false)
				{
					pet->SendPetBuffsToClient();
					pet->SendAppearancePacket(AT_Pet, GetID(), true, true);
					bSentPetInfo = true;
				}
			}
			petinfo.SpellID = 0;
...
void Client::Handle_OP_AltCurrencySell(const EQApplicationPacket *app)
{
	...
	if (tar) {
		...
		for (itr = merlist.begin(); itr != merlist.end(); ++itr) {
			MerchantList ml = *itr;
			EQ::ItemData item = database.GetItem(ml.item);
...
void Client::Handle_OP_ApplyPoison(const EQApplicationPacket *app)
{
	...
	{
		Message(13, "Poison must be in a main inventory slot to be used.");
	}
...
void Client::Handle_OP_AutoFire(const EQApplicationPacket *app)
{
	...
	bool prev_auto_state = auto_attack;
	bool prev_af_state = auto_fire;
	auto_attack = *af;
	auto_fire = *af;
	if (prev_af_state == true && prev_auto_state == true && auto_fire == false)
		auto_attack = false;
	else
		auto_attack = true;
	SetAttackTimer();
...
void Client::Handle_OP_Barter(const EQApplicationPacket *app)
{
	...
	case Barter_BarterItemInspect:
	{
		...
		EQ::ItemData item = database.GetItem(bislr->ItemID);
...
void Client::Handle_OP_BazaarInspect(const EQApplicationPacket *app)
{
	...
	EQ::ItemData item = database.GetItem(bis->ItemID);
...
void Client::Handle_OP_CastSpell(const EQApplicationPacket *app)
{
	...
	else if (slot == CastingSlot::Item || slot == CastingSlot::PotionBelt)	// ITEM or POTION cast
	{
		if (m_inv.SupportsClickCasting(castspell->inventoryslot) || slot == CastingSlot::PotionBelt)	// sanity check
		{
			...
			if (castspell->inventoryslot == EQ::invslot::slotCharm && GetCharmEffectID(EQ::invslot::slotCharm) >= 0 ||
				castspell->inventoryslot == EQ::invslot::slotPowerSource && GetCharmEffectID(EQ::invslot::slotPowerSource) >= 0)
			{
				Message(13, "This item cannot be activated in this manner. Simply wearing it long enough should work.");
...
void Client::Handle_OP_ClientUpdate(const EQApplicationPacket *app) {
	...
	glm::vec3 newPosition(ppu->x_pos, ppu->y_pos, ppu->z_pos);
	bool bSkip = false;
	if (m_RewindLocation == glm::vec3())
	{
		m_RewindLocation = newPosition;
	...
	double dist = Distance(m_RewindLocation, newPosition);
	double distFromExpected = Distance(ExpectedRewindPos, newPosition);
	...
	*/
	...
	//if (shouldTryHackCheck)
	//{
	if (distDivTime >= 100.0 && !is_exempt_correct && !GetGM() && client_state == CLIENT_CONNECTED)
	{
		...
		worldserver.SendEmoteMessage(0, 0, 250, Chat::White , "%s - entity moving too fast: %lf %lf - is_exempt_correct %s", GetCleanName(), dist, distDivTime, std::to_string(is_exempt_correct).c_str());
		database.SetHackerFlag(this->account_name, this->name, warped.c_str());
	}


		/*if ((rewind_x_diff > 10000) || (rewind_y_diff > 10000) && !GetGM())
		{
				//entity_list.AggroZone(this, 100000);
				database.SetHackerFlag(this->account_name, this->name, "longwarp");
				ppu->x_pos = m_RewindLocation.x;
				ppu->y_pos = m_RewindLocation.y,
				ppu->z_pos = m_RewindLocation.z;
				GMMove(m_RewindLocation.x, m_RewindLocation.y, m_RewindLocation.z, EQ12toFloat(ppu->heading));
...
void Client::Handle_OP_EnvDamage(const EQApplicationPacket *app)
{
	...
	else if (zone->GetZoneID() == 183 || zone->GetZoneID() == RuleI(World, LoadZoneID)) {
...
void Client::Handle_OP_GroupMakeLeader(const EQApplicationPacket *app)
{
	...
	if (NewLeader && g && !g->IsGroupInCombat())
	{
		if (g->IsLeader(this))
...
void Client::Handle_OP_GroupUpdate(const EQApplicationPacket *app)
{
	...
	case groupActMakeLeader:
	{
		...
		if (newleader && group) {
			...
			if (group->IsLeader(this) && !group->IsGroupInCombat())
...
void Client::Handle_OP_ItemLinkClick(const EQApplicationPacket *app)
{
	...
	EQ::ItemData item = database.GetItem(ivrs->item_id);
	if (!item.ID) {
...
void Client::Handle_OP_ItemName(const EQApplicationPacket *app)
{
	...
	EQ::ItemData item;
	memset(&item, 0, sizeof(item));
	if ((item = database.GetItem(p->item_id)).ID != 0) {
...
void Client::Handle_OP_ItemPreview(const EQApplicationPacket *app)
{
	...
	EQ::ItemData item = database.GetItem(ips->itemid);
...
void Client::Handle_OP_ItemVerifyRequest(const EQApplicationPacket *app)
{
	...
	if (m_inv.SupportsClickCasting(slot_id) || ((item->ItemType == EQ::item::ItemTypePotion || item->PotionBelt) && m_inv.SupportsPotionBeltCasting(slot_id))) // sanity check
	{
		...
		if ((spell_id <= 0) && (item->ItemType != EQ::item::ItemTypeFood && item->ItemType != EQ::item::ItemTypeDrink && item->ItemType != EQ::item::ItemTypeAlcohol && item->ItemType != EQ::item::ItemTypeSpell))
		{
			...
			SendSpellBarEnable(item->Scroll.Effect);
			return;
		}
		else if (inst->IsClassCommon())
		{
			if (item->ItemType == EQ::item::ItemTypeSpell && !HasSpellScribed(item->Scroll.Effect) && CanUseSpell(item->Scroll.Effect, GetClass(), GetLevel()))
			{
				DeleteItemInInventory(slot_id, 1, true);
				ScribeSpell(item->Scroll.Effect, GetNextAvailableSpellBookSlot(), true);
			}
			else if (item->ItemType == EQ::item::ItemTypeSpell)
			{
				MessageString(13, 12052); // already memmed spell
				SendSpellBarEnable(item->Scroll.Effect);
				return;
			}
			else if ((item->Click.Type == EQ::item::ItemEffectClick) || (item->Click.Type == EQ::item::ItemEffectExpendable) || (item->Click.Type == EQ::item::ItemEffectEquipClick) || (item->Click.Type == EQ::item::ItemEffectClick2))
			{
				...
				{
					if (!CastToClient()->GetPTimers().Expired(&database, (pTimerItemStart + item->RecastType), false)) {
				...
				if (slot_id == EQ::invslot::slotCharm && GetCharmEffectID(EQ::invslot::slotCharm) >= 0 ||
					slot_id == EQ::invslot::slotPowerSource && GetCharmEffectID(EQ::invslot::slotPowerSource) >= 0)
				{
					Message(13, "This item cannot be activated in this manner. Simply wearing it long enough should work.");
					SendSpellBarEnable(spell_id);
					return;
				}
				else if (inst->GetCharges() == 0)
				{
...
{
	SetEndurance(GetEndurance() - (GetMaxEndurance() * 0.10f));
	return;
}
...
void Client::Handle_OP_MercenaryCommand(const EQApplicationPacket *app)
{
	...
	if (option >= 0)
	{
		...
		if (merc)
		{
			...
			{
				merc->SetStance((EQ::constants::StanceType)(option + 1));
...
void Client::Handle_OP_PetCommands(const EQApplicationPacket *app)
{
	...
	bool bFollowingYouMaster = false;

	std::list<Mob*> iterator_list;

	for (Mob* mypet : petlist)
	{
		if (mypet == nullptr)
			continue;
		...
		*/
		...
		case PET_ATTACK: {
			if (!target)
				break;
			if (target->IsMezzed()) {
				if (!bFollowingYouMaster)
				{
					bFollowingYouMaster = true;
		...
		case PET_QATTACK: {
			...
			if (!mypet->IsAttackAllowed(GetTarget())) {
				...
				{
					bFollowingYouMaster = true;
					mypet->SayString(this, Chat::PetResponse, NOT_LEGAL_TARGET);
				}
				break;
			}
...
void Client::Handle_OP_PotionBelt(const EQApplicationPacket *app)
{
	...
	if (mptbs->Action == 0) {
		EQ::ItemData BaseItem = database.GetItem(mptbs->ItemID);
		if (BaseItem.ID) {
...
void Client::Handle_OP_ShopPlayerBuy(const EQApplicationPacket *app)
{
	...
	for (itr = merlist.begin(); itr != merlist.end(); ++itr) {
		...
		if (item.ItemType == 20 && tmp->GetClass() != ALT_CURRENCY_MERCHANT && GetClass() == CLASSLESS)
			continue;
...
void Client::Handle_OP_ShopPlayerSell(const EQApplicationPacket *app)
{
	...
	if (RuleB(QueryServ, PlayerLogMerchantTransactions)) {
		...
		qsaudit->items[0].aug_1 = m_inv[mp->itemslot]->GetAugmentItemID(0);
		qsaudit->items[0].aug_2 = m_inv[mp->itemslot]->GetAugmentItemID(1);
		qsaudit->items[0].aug_3 = m_inv[mp->itemslot]->GetAugmentItemID(2);
		qsaudit->items[0].aug_4 = m_inv[mp->itemslot]->GetAugmentItemID(3);
		qsaudit->items[0].aug_5 = m_inv[mp->itemslot]->GetAugmentItemID(4);
