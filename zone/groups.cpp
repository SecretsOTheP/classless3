...
bool Group::IsGroupInCombat()
{
	for (uint32 i = 0; i < MAX_GROUP_MEMBERS; ++i)
	{
		if (members[i] && members[i]->IsClient() && members[i]->CastToClient()->GetAggroCount() > 0)
		{
			return true;
		}
	}
...
bool Group::AddMember(Mob* newmember, const char *NewMemberName, uint32 CharacterID, bool ismerc)
{
	...
	if(InZone)
	{
		...
		if(newmember->IsMerc())
		{
			...
			Group* group = newmember->CastToMerc()->GetGroup();
			if (group) {
				group->SendHPManaEndPacketsTo(newmember);
				group->SendHPPacketsFrom(newmember);
			}
...
bool Group::DelMember(Mob* oldmember, bool ignoresender)
{
	...
	if(oldmember->IsClient())
	{
		database.SetGroupID(oldmember->GetName(), 0, oldmember->CastToClient()->CharacterID(), false);
	}
...
void Group::CastGroupSpell(Mob* caster, uint16 spell_id, bool isproc) {
	uint32 z;
...
//	caster->SpellOnTarget(spell_id, caster);
	...
	{
		if(members[z] == caster) {
			caster->SpellOnTarget(spell_id, caster, false, false, 0, isproc);
		...
		else if(members[z] != nullptr)
		{
			...
			if(distance <= range2 && distance >= min_range2) {
				...
				caster->SpellOnTarget(spell_id, members[z], false, false, 0, isproc);
#ifdef GROUP_BUFF_PETS
				for (auto pet : members[z]->petlist)
				{
					pet_iterator_list.push_back(pet);
				}
#endif
	...
	for (auto petiter : pet_iterator_list)
	{
		caster->SpellOnTarget(spell_id, petiter, false, false, 0, isproc);
	}
	castspell = false;
	disbandcheck = true;
}
...
void Group::GroupBardPulse(Mob* caster, uint16 spell_id) {
	...
	for (auto petiter : pet_iterator_list)
	{
		petiter->BardPulse(spell_id, caster);
	}
}
...
uint64 Group::GetTotalGroupDamage(Mob* other) {
	uint64 total = 0;
...
std::list<std::string>& Group::GetClientNames() const
{
...
Mob* Group::GetHighest()
{
	...
	for (i = 0; i < MAX_GROUP_MEMBERS; i++)
	{
		if (members[i])
		{
			if(members[i]->IsMerc())
			{
				if(members[i]->m_POWER * members[i]->m_POWER_NORMAL > power)
				{
					power = members[i]->m_POWER * members[i]->m_POWER_NORMAL;
					highest = members[i];
				}
			}
			else
			{
				if(members[i]->m_POWER > power)
				{
					power = members[i]->m_POWER;
					highest = members[i];
				}
			}
		}
...
void Group::TeleportGroup(Mob* sender, uint32 zoneID, uint16 instance_id, float x, float y, float z, float heading)
{
	...
	for (i = 0; i < MAX_GROUP_MEMBERS; i++)
	{
		if (members[i] != nullptr && members[i]->IsClient())
		{
			members[i]->CastToClient()->MovePC(zoneID, instance_id, x, y, z, heading, 1, ZoneSolicited);
		}
	}
}
...
void Group::HealGroup(int64 heal_amt, Mob* caster, float range)
{
	if (!caster)
		return;
...
void Group::BalanceHP(int64 penalty, float range, Mob* caster, int64 limit)
{
	if (!caster)
		return;
...
void Group::BalanceMana(int64 penalty, float range, Mob* caster, int64 limit)
{
	if (!caster)
		return;
	...
	int64 manataken = 0, numMem = 0, manataken_tmp = 0;
	unsigned int gi = 0;
	for(; gi < MAX_GROUP_MEMBERS; gi++)
	{
...
void Group::MarkNPC(Mob* Target, int Number)
{
	...
	if(m)
		sprintf(mnpcs->Name, "%s", m->GetName());
