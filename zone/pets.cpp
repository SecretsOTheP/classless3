...
Mob* Mob::MakePet(uint16 spell_id, const char* pettype, const char *petname) {
	...
	return MakePoweredPet(spell_id, pettype, -1, petname);
}
...
Mob* Mob::MakePoweredPet(uint16 spell_id, const char* pettype, int16 petpower,
	...
	npc_type->scale_difficulty = 100;
	npc_type->scale_tier = 1;
	...
	if(record.petnaming == 2)
	{
		...
		case WOOD_ELF:
			...
			break;
		case HIGH_ELF:
			npc_type->race = 69; // WILLOWISP
			break;
		case DARK_ELF:
			npc_type->race = 37; // SNEK
			break;
		case HALF_ELF:
		...
		case OGRE:
			...
			break;
		case HALFLING:
			npc_type->race = 79; // BIXIE
			npc_type->size *= 0.5f;
			break;
		case GNOME:
			npc_type->race = 38; // SPIDER
			npc_type->texture = 3;
			break;
		case IKSAR:
			npc_type->race = WOLF;
		...
		case VAHSHIR:
			...
			break;
		case FROGLOK:
			npc_type->race = 104; // LEECH
			break;
		case DRAKKIN:
			npc_type->race = 96; // COCKATRICE
			break;
		default:
			npc_type->race = GetBaseRace();
			npc_type->gender = GetBaseGender();
			npc_type->texture = 0;
			npc_type->size *= 0.5f;
		}
	}
...
Mob* Mob::MakePoweredNPCPet(uint16 spell_id, uint32 npcTypeId, int16 petpower,
	...
	record.equipmentset = -1;
	record.monsterflag = 0;
	record.npc_type = npcTypeId;
	record.petcontrol = petNecro;
	record.petpower = 0;
	record.temporary = 0;
	record.petnaming = 4;
...
Mob* Mob::MakePoweredPCPet(uint16 spell_id, Client* affected, int16 petpower,
	const char *petname, float in_size) {
	// Sanity and early out checking first.
	if (affected == nullptr)
		return nullptr;
	...
	// handle monster summoning pet appearance
	npc_type->race = affected->GetRace();
	npc_type->size = affected->GetSize();

	auto feetItem = database.GetItem(affected->CastToClient()->GetEquippedItemFromTextureSlot(EQ::textures::armorFeet));
	auto helmItem = database.GetItem(affected->CastToClient()->GetEquippedItemFromTextureSlot(EQ::textures::armorHead));
	auto chestItem = database.GetItem(affected->CastToClient()->GetEquippedItemFromTextureSlot(EQ::textures::armorChest));
	auto wristItem = database.GetItem(affected->CastToClient()->GetEquippedItemFromTextureSlot(EQ::textures::armorWrist));
	auto armsItem = database.GetItem(affected->CastToClient()->GetEquippedItemFromTextureSlot(EQ::textures::armorArms));
	auto handsItem = database.GetItem(affected->CastToClient()->GetEquippedItemFromTextureSlot(EQ::textures::armorHands));
	auto legsItem = database.GetItem(affected->CastToClient()->GetEquippedItemFromTextureSlot(EQ::textures::armorLegs));
	auto primaryModel = database.GetItem(affected->CastToClient()->GetEquippedItemFromTextureSlot(EQ::textures::weaponPrimary));
	auto secondaryModel = database.GetItem(affected->CastToClient()->GetEquippedItemFromTextureSlot(EQ::textures::weaponSecondary));



	npc_type->feettexture = feetItem.ID ? feetItem.Material : 0;
	npc_type->helmtexture = helmItem.ID ? helmItem.Material : 0;
	npc_type->texture = chestItem.ID ? chestItem.Material : 0;
	npc_type->bracertexture = wristItem.ID ? wristItem.Material : 0;
	npc_type->armtexture = armsItem.ID ? armsItem.Material : 0;
	npc_type->legtexture = legsItem.ID ? legsItem.Material : 0;
	npc_type->handtexture = handsItem.ID ? handsItem.Material : 0;

	npc_type->gender = affected->GetGender();
	npc_type->luclinface = affected->luclinface;
	npc_type->beard = affected->beard;
	npc_type->beardcolor = affected->beardcolor;
	npc_type->eyecolor1 = affected->eyecolor1;
	npc_type->eyecolor2 = affected->eyecolor2;

	auto primary_id = 0;
	if (secondaryModel.ID && strlen(secondaryModel.IDFile) > 2) {
		primary_id = atoi(&secondaryModel.IDFile[2]);
	}

	auto sec_id = 0;
	if (primaryModel.ID && strlen(primaryModel.IDFile) > 2) {
		primary_id = atoi(&primaryModel.IDFile[2]);
	}

	npc_type->d_melee_texture1 = primary_id;
	npc_type->d_melee_texture2 = sec_id;
...
uint32 Mob::GetPetCount() {
		return petlist.size();
}

std::vector<Mob*> Mob::GetSwarmPets() {

	std::vector<Mob*> ret_list;
	std::unordered_map<uint16, NPC*> iteration_list = entity_list.GetNPCList();

	for (std::pair<uint16, NPC*> pet : iteration_list)
	{
		NPC* tmp = pet.second;
		if (tmp == nullptr) {
			continue;
		...
		if (!tmp->GetSwarmInfo() || tmp->GetSwarmInfo() && tmp->GetSwarmOwner() != GetID()) {
			continue;
		}
...
void Mob::RemovePet(Mob* ptr)
{
	if (ptr == nullptr)
		return;

	if (contains(petlist, ptr))
	{
	...
	if (IsClient()) {
		if (!petlist.empty())
		{
			...
			petlist.front()->CreateDespawnPacket(outapp, false);
			petlist.front()->CreateSpawnPacket(outapp2, this);
			CastToClient()->QueuePacket(outapp);
			CastToClient()->QueuePacket(outapp2);
			petlist.front()->SendAppearancePacket(AT_Pet, petlist.front()->GetID(), true, true);
			petlist.front()->SendPetBuffsToClient();
			safe_delete(outapp);
			safe_delete(outapp2);
		}
		else
		{
			SendAppearancePacket(AT_Pet, 0, true, false);
		}
	}
...
void Mob::RemoveAllPets()
{
	...
	for (auto mobptr : iteration_list)
	{
		RemovePet(mobptr);
	}
...
int Pet::_GetWalkSpeed() {
	...
	if (GetOwner() != nullptr)
	{
		auto ownerMob = GetOwner();

		auto oSpeed = ownerMob->_GetWalkSpeed();
		auto ourSpeed = Mob::_GetWalkSpeed();
		if (oSpeed != 0 && ourSpeed != 0)
		{
			if (oSpeed > ourSpeed)
			{
				oSpeed += spellbonuses.movementspeed;
				oSpeed += itembonuses.movementspeed;
				return oSpeed;
			}
		}
	}
	return Mob::_GetWalkSpeed();
}

int Pet::_GetRunSpeed() {
	...
	if (GetOwner() != nullptr)
	{
		...
		auto oSpeed = ownerMob->_GetRunSpeed();
		auto ourSpeed = Mob::_GetRunSpeed();
		if (oSpeed != 0 && ourSpeed != 0)
		{
			if (oSpeed > ourSpeed)
			{
	...
	return Mob::_GetRunSpeed();
}

int Pet::_GetFearSpeed() {
	...
	if (GetOwner() != nullptr)
	{
		...
		auto oSpeed = ownerMob->_GetFearSpeed();
		auto ourSpeed = Mob::_GetFearSpeed();
		if (oSpeed != 0 && ourSpeed != 0)
		{
			if (oSpeed > ourSpeed)
			{
