...
bool Mob::CheckWillAggro(Mob *mob) {
	...
	FACTION_VALUE fv = mob->GetReverseFactionCon(this);
	if (!IsPet() && mob && !mob->IsPet() && zone && zone->GetInstanceID() != 0)
		fv = FACTION_SCOWLS;
...
//	NPC *npc1, *npc2;
	...
	Mob *target_owner = target->GetUltimateOwner();
	Mob *our_owner = GetUltimateOwner();
	if(target_owner && target_owner == this)
		return false;
	else if(our_owner && our_owner == target)
		return false;
	else if (our_owner == target_owner && our_owner != nullptr && target_owner != nullptr)
		return false;
	...
	do
	{
		if(_CLIENT(mob1))
		{
			if(_CLIENT(mob2))					// client vs client
			{
				...
				(
					c1->GetPVP() &&
					c2->GetPVP() && (zone->GetZoneID() != RuleI(World, LoadZoneID))
				)
					return true;
...
bool Mob::IsBeneficialAllowed(Mob *target)
{
	...
	// first figure out if we're pets. we always look at the master's flags.
	// no need to compare pets to anything
	mob1 = this->GetUltimateOwner();
	mob2 = target->GetUltimateOwner();
...
bool Mob::CombatRange(Mob* other, bool CheckLOS)
{
	if(!other)
		return(false);
	...
	if (_DistNoRoot <= size_mod)
	{
		...
		if (flymode != GravityBehavior::Flying && _zDist > 500 && !CheckLastLosState() && CheckLOS) {
			return false;
		}
...
int64 Mob::CheckAggroAmount(uint16 spell_id, Mob *target, bool isproc)
{
	if (NoDetrimentalSpellAggro(spell_id))
		return 0;

	int64 AggroAmount = 0;
	int64 nonModifiedAggro = 0;
	uint16 slevel = GetLevel();
	bool dispel = false;
	bool on_hatelist = target ? target->CheckAggro(this) : false;
	int64 proc_cap = RuleI(Aggro, MaxScalingProcAggro);
	...
	for (int o = 0; o < EFFECT_COUNT; o++) {
		switch (spells[spell_id].effectid[o]) {
			...
			case SE_CurrentHP: {
				int64 val = CalcSpellEffectValue_formula(spells[spell_id].formula[o], spells[spell_id].base[o], spells[spell_id].max[o], slevel, spell_id);
				if(val < 0)
					AggroAmount -= val;
				break;
			}
	...
	if (!on_hatelist && AggroAmount > 0) {
		int64 HateMod = RuleI(Aggro, SpellAggroMod);
...
int64 Mob::CheckHealAggroAmount(uint16 spell_id, Mob *target, uint64 heal_possible)
{
	int64 AggroAmount = 0;
	...
	return std::max((int64)0, AggroAmount);
}
