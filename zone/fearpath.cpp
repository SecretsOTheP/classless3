...
void Mob::CheckFlee()
{
	...
	float fleeratio = GetSpecialAbility(FLEE_PERCENT); // if a special flee_percent exists
	...
	if (GetLevelCon(hate_top->GetLevel(), GetLevel()) == CON_GRAY && fleeratio == 0.0f && RuleB(Combat, FleeGray) &&
	...
	bool mob_has_low_enough_health_to_flee = hpratio >= fleeratio;
